...
async function handleAnalyzeMatch(data, chatId, userId, redis, services) {
  try {
    ...
    // Prefer an analyzer service if available
    if (services.multiSportAnalyzer && typeof services.multiSportAnalyzer.analyzeMatch === 'function') {
      try {
        const analysis = await services.multiSportAnalyzer.analyzeMatch(m, { userId });
        const formatted = services.multiSportAnalyzer.formatForTelegram ? services.multiSportAnalyzer.formatForTelegram(analysis) : JSON.stringify(analysis, null, 2);
