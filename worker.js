...
if (redis && typeof redis.on === "function") {
  redis.on("error", (err) => {
    try {
      ...
      console.error("[redis] Error event (stringify failed)", e);
    }
  });
}
...
const HOSTS = {
  ...
  freeFootball:
    process.env.FREE_FOOTBALL_HOST || "free-football-data.p.rapidapi.com",
...
function requireParam(val, label) {
  if (!val)
    return `Missing ${label}. Usage: provide ${label} after the command.`;
  return null;
}

function ensureKey() {
  if (!RAPIDAPI_KEY)
    return "RapidAPI key missing. Set RAPIDAPI_KEY in environment.";
  return null;
}

async function callGet(host, path) {
  const keyMissing = ensureKey();
  ...
  try {
    ...
    if (!ct.includes("application/json"))
      return `Non-JSON response: ${await res.text()}`;
...
async function getSofaSportOdds(matchId) {
  const miss = requireParam(matchId, "matchId");
  if (miss) return miss;
  return await callGet(HOSTS.sofasport, `/odds/${encodeURIComponent(matchId)}`);
}

async function getTournamentSeasons(tournamentId) {
  const miss = requireParam(tournamentId, "tournamentId");
  if (miss) return miss;
  return await callGet(
    HOSTS.osSports,
    `/tournament/${encodeURIComponent(tournamentId)}/seasons`,
  );
}

async function runAIAnalysis(prompt) {
  const miss = requireParam(prompt, "prompt");
  if (miss) return miss;
...
async function getSportsbookAdvantages(sport) {
  const miss = requireParam(sport, "sport");
  if (miss) return miss;
  return await callGet(
    HOSTS.sportsbook,
    `/v0/advantages/${encodeURIComponent(sport)}`,
  );
}

async function searchPlayer(name) {
  const miss = requireParam(name, "player name");
  if (miss) return miss;
  return await callGet(
    HOSTS.freeFootball,
...
async function getScores(fixtureId) {
  const miss = requireParam(fixtureId, "fixtureId");
  if (miss) return miss;
  return await callGet(
    HOSTS.oddsApi,
    `/scores/${encodeURIComponent(fixtureId)}`,
  );
}
...
async function getTeamTransfers(teamId) {
  const miss = requireParam(teamId, "teamId");
  if (miss) return miss;
  return await callGet(
    HOSTS.allSportsApi,
    `/api/team/${encodeURIComponent(teamId)}/transfers`,
  );
}

async function getFootballPrediction(query) {
  const miss = requireParam(query, "query");
  if (miss) return miss;
...
async function getMarkets(eventId) {
  const miss = requireParam(eventId, "eventId");
  if (miss) return miss;
  return await callGet(
    HOSTS.oddsFeed,
    `/markets/feed?eventId=${encodeURIComponent(eventId)}`,
  );
}
  ...
  try {
    switch (cmd) {
      ...
      case "/odds":
        ...
        break;
      case "/seasons":
        reply = await getTournamentSeasons(args[0]);
        break;
      case "/ai":
        reply = await runAIAnalysis(argstr);
        break;
      case "/advantages":
        reply = await getSportsbookAdvantages(args[0]);
        break;
      case "/player":
        reply = await searchPlayer(argstr);
        break;
      case "/copilot":
        reply = await runCopilot(argstr);
        break;
      case "/ai4":
        reply = await runChatGPT4(argstr);
        break;
      case "/scores":
        reply = await getScores(args[0]);
        break;
      case "/news":
        reply = await getMBBNews();
        break;
      case "/chat":
        reply = await runChatCompletion(argstr);
        break;
      case "/transfers":
        reply = await getTeamTransfers(args[0]);
        break;
      case "/predict":
        reply = await getFootballPrediction(argstr);
        break;
      case "/markets":
        reply = await getMarkets(args[0]);
        break;
      default:
