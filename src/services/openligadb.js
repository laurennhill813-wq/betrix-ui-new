...
class OpenLigaDBService {
  ...
  async getAvailableLeagues() {
    ...
    if (this.cache) {
      ...
      const allowed = await this.cache
        .rateLimit("openligadb:leagues", this.rateLimit, this.rateWindow)
        .catch(() => false);
      if (!allowed) {
    ...
    if (!res.ok)
      throw new Error(`OpenLigaDB leagues fetch failed: ${res.status}`);
    const json = await res.json();
    if (this.cache)
      await this.cache.set(cacheKey, json, this.cacheTtl).catch(() => {});
    return json;
  }

  // Fetch match data for a league + season + group (matchday)
  async getMatchData(leagueShortcut, season, groupOrderId) {
    if (!leagueShortcut || !season)
      throw new Error("leagueShortcut and season required");
    ...
    const res = await fetch(url);
    if (!res.ok)
      throw new Error(`OpenLigaDB matchdata fetch failed: ${res.status}`);
    const json = await res.json();
    if (this.cache)
      await this.cache.set(cacheKey, json, this.cacheTtl).catch(() => {});
    return json;
  ...
  async getRecentMatches(leagueShortcut, season, groups = 4) {
    ...
    for (let g = 1; g <= groups; g++)
      promises.push(
        this.getMatchData(leagueShortcut, season, g).catch(() => []),
      );
