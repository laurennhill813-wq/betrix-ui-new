...
export class SportsAggregator {
  constructor(redis, extras = {}) {
    ...
    this.sportsgameodds =
      this._isAllowedSync("SPORTSGAMEODDS") &&
      CONFIG.SPORTSGAMEODDS &&
      CONFIG.SPORTSGAMEODDS.KEY
        ? new SportGameOddsService(redis)
        : null;
    ...
    this.isports =
      this._isAllowedSync("API_SPORTS") &&
      CONFIG.API_FOOTBALL &&
      CONFIG.API_FOOTBALL.KEY
        ? new ISportsService(redis)
        : null;
    // Sportradar adapter (optional)
    this.sportradar =
      this._isAllowedSync("SPORTRADAR") &&
      CONFIG.SPORTRADAR &&
      CONFIG.SPORTRADAR.KEY &&
      PROVIDERS.sportradar &&
      typeof PROVIDERS.sportradar.handler === "function"
  ...
  /**
   * Get live matches for a non-football sport using configured providers (e.g., Sportradar)
   * Returns normalized match objects: { id, sport, league, homeTeam, awayTeam, startTime, status, venue, externalId }
   */
  async getLiveBySport(sport = "nba", options = {}) {
    try {
      const cacheKey = `live:sport:${sport}`;
      if (this.cache.has(cacheKey)) {
        const cached = this.cache.get(cacheKey);
        if (Date.now() - cached.timestamp < 60 * 1000) {
          // 1 min cache for multi-sport live
          return cached.data;
        }
      }

      // Prefer Sportradar if enabled
      if (this.sportradar && (await this._isProviderEnabled("SPORTRADAR"))) {
        try {
          const date = options.date || new Date().toISOString().slice(0, 10);
          const sr = await this.sportradar(
            sport,
            "matches_by_date",
            { date },
            options,
          ...
          if (sr && sr.ok && sr.data) {
            // try to extract matches array from common fields
            let rawMatches =
              sr.data.matches ||
              sr.data.games ||
              sr.data.events ||
              ...
              sr.data ||
              [];
            if (!Array.isArray(rawMatches) && rawMatches && rawMatches.matches)
              rawMatches = rawMatches.matches;
            rawMatches = Array.isArray(rawMatches) ? rawMatches : [];
            const formatted = rawMatches
              .map((r) => this._normalizeSportradarEntry(r, sport))
              .filter(Boolean);
            this._setCached(cacheKey, formatted);
            await this._recordProviderHealth(
              "sportradar",
              true,
              `Found ${formatted.length} ${sport} matches`,
            );
            return formatted;
          }
        } catch (e) {
          logger.warn(
            `Sportradar ${sport} live fetch failed`,
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      // Fallback to other generic sources (AllSports, SportsData, etc.)
      // Try SportsData for certain sports if configured
      try {
        const alt = await this._getLiveFromSportsData();
        if (alt && alt.length > 0) {
          const formatted = alt
            .map((r) => this._formatMatches([r], "sportsdata"))
            .flat();
          this._setCached(cacheKey, formatted);
          return formatted;
        }
      ...
      // No data
      logger.info(`No live matches found for sport ${sport}`);
      return [];
    } catch (err) {
      logger.error("getLiveBySport failed:", err?.message || String(err));
      return [];
    }
  }

  /**
   * Get upcoming matches for a non-football sport using providers like Sportradar
   */
  async getUpcomingBySport(sport = "nba", options = {}) {
    try {
      const cacheKey = `upcoming:sport:${sport}`;
      if (this.cache.has(cacheKey)) {
        const cached = this.cache.get(cacheKey);
        ...
        try {
          ...
          if (sr && sr.ok && sr.data) {
            ...
            await this._recordProviderHealth(
              "sportradar",
              true,
              `Found ${formatted.length} upcoming ${sport} matches`,
            );
            return formatted;
          }
        } catch (e) {
          logger.warn(
            `Sportradar ${sport} upcoming fetch failed`,
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      logger.error("getUpcomingBySport failed:", err?.message || String(err));
      return [];
    }
  }

  // Normalize a Sportradar entry into the canonical match shape
  _normalizeSportradarEntry(raw, sport = "unknown") {
    try {
      if (!raw) return null;
      const id =
        raw.id ||
        ...
        raw.match_id ||
        raw.event_id ||
        (raw.external_id && String(raw.external_id)) ||
        null;
      const league =
      ...
      const home =
        raw.home?.name ||
        raw.home_team?.name ||
        (raw.teams && raw.teams.home && raw.teams.home.name) ||
        (raw.teams &&
      ...
      const away =
        raw.away?.name ||
        raw.away_team?.name ||
        (raw.teams && raw.teams.away && raw.teams.away.name) ||
        (raw.teams &&
          Array.isArray(raw.teams) &&
          raw.teams[1] &&
          raw.teams[1].name) ||
      ...
      const status =
        ...
        raw.game_status ||
        raw.state ||
        (raw.live ? "LIVE" : "SCHEDULED");
      const venue =
        raw.venue?.name ||
        raw.location ||
        raw.venue ||
        (raw.stadium && raw.stadium.name) ||
        null;
      return {
        id: id ? String(id) : null,
        sport: String(sport || "unknown"),
        league: league || null,
        home: home || null,
        away: away || null,
      ...
      logger.debug(
        "Failed to normalize Sportradar entry",
        e?.message || String(e),
      );
      return null;
    }
  ...
  async _recordProviderHealth(name, ok, message = "") {
    try {
      ...
      // Non-fatal, just log
      logger.warn(
        `Failed to write provider health for ${name}`,
        e?.message || String(e),
      );
    }
  ...
  async _isProviderEnabled(name) {
    try {
      ...
      if (
        Array.isArray(this.allowedProviders) &&
        this.allowedProviders.length > 0
      ) {
  ...
  _isAllowedSync(name) {
    if (!this.allowedProviders || !Array.isArray(this.allowedProviders))
      return true;
    ...
    try {
      ...
      if (this.sportsgameodds) {
        try {
          ...
          const sgoLeagues =
            typeof this.sportsgameodds.getLeagues === "function"
              ? await this.sportsgameodds.getLeagues()
              : [];
          if (sgoLeagues && sgoLeagues.length > 0) {
            ...
            await this._recordProviderHealth(
              "sportsgameodds",
              true,
              `Found ${sgoLeagues.length} leagues`,
            );
            return sgoLeagues;
          }
        } catch (e) {
          logger.warn(
            "SportGameOdds league fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (this.sportmonks) {
        try {
          ...
          const smLeagues =
            typeof this.sportmonks.getLeagues === "function"
              ? await this.sportmonks.getLeagues()
              : [];
          if (smLeagues && smLeagues.length > 0) {
            ...
            await this._recordProviderHealth(
              "sportsmonks",
              true,
              `Found ${smLeagues.length} leagues`,
            );
            return smLeagues;
          }
        } catch (e) {
          logger.warn(
            "SportMonks league fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
        ...
        try {
          ...
          if (fdLeagues && fdLeagues.length > 0) {
            ...
            await this._recordProviderHealth(
              "footballdata",
              true,
              `Found ${fdLeagues.length} leagues`,
            );
            return fdLeagues;
          }
        } catch (e) {
          logger.warn(
            "Football-Data league fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (this.sportradar && (await this._isProviderEnabled("SPORTRADAR"))) {
        try {
          ...
          if (
            ...
            sr.ok &&
            sr.data &&
            Array.isArray(sr.data.competitions) &&
            sr.data.competitions.length > 0
          ) {
            const mapped = sr.data.competitions.map((c) => ({
              id: c.id || null,
              name: c.name || "Unknown",
            ...
            await this._recordProviderHealth(
              "sportradar",
              true,
              `Found ${mapped.length} competitions`,
            );
            return mapped;
          }
        } catch (e) {
          logger.warn(
            "Sportradar league fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
  ...
  async getLiveMatches(leagueId, options = {}) {
    try {
      ...
      if (CONFIG.FOOTBALLDATA && CONFIG.FOOTBALLDATA.KEY) {
        try {
          logger.debug(
            `📡 Fetching live matches from Football-Data for league ${leagueId}`,
          );
          const fdMatches = await this._getLiveFromFootballData(leagueId);
          if (fdMatches && fdMatches.length > 0) {
            logger.info(
              `✅ Football-Data: Found ${fdMatches.length} live matches`,
            );
            await this.dataCache.storeLiveMatches("footballdata", fdMatches);
            const formatted =
              this._formatMatches(fdMatches, "footballdata") || [];
            const liveCount = formatted.filter(
              (m) => String(m.status || "").toUpperCase() === "LIVE",
            ).length;
            logger.info(
              `🔍 Football-Data DIAGNOSTIC [league:${leagueId}]: raw:${fdMatches.length} | formatted:${formatted.length} | live:${liveCount}`,
            );
            this._setCached(cacheKey, formatted);
            await this._recordProviderHealth(
              "footballdata",
              true,
              `Found ${formatted.length} live matches`,
            );
            return formatted;
          }
        } catch (e) {
          logger.debug(
            "Football-Data live fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (CONFIG.SPORTSMONKS && CONFIG.SPORTSMONKS.KEY) {
        try {
          logger.debug(
            `📡 Fallback: Fetching live matches from SportMonks for league ${leagueId}`,
          );
          const smMatches = await this.sportmonks.getLivescores(leagueId);
          if (smMatches && smMatches.length > 0) {
            logger.info(
              `✅ SportMonks (fallback): Found ${smMatches.length} live matches (raw)`,
            );
            await this.dataCache.storeLiveMatches("sportsmonks", smMatches);
            const formatted =
              this._formatMatches(smMatches, "sportsmonks") || [];
            const liveOnly = formatted.filter(
              (m) => String(m.status || "").toUpperCase() === "LIVE",
            );
            logger.info(
              `🔍 SportMonks DIAGNOSTIC [league:${leagueId}]: raw:${smMatches.length} | formatted:${formatted.length} | live:${liveOnly.length}`,
            );
            if (liveOnly.length > 0) {
              ...
              await this._recordProviderHealth(
                "sportsmonks",
                true,
                `Found ${liveOnly.length} live matches (fallback)`,
              );
              return liveOnly;
            }
            this._setCached(cacheKey, formatted);
            await this._recordProviderHealth(
              "sportsmonks",
              false,
              `No matches marked LIVE (raw:${smMatches.length})`,
            );
          }
        } catch (e) {
      ...
      if (this.isports) {
        try {
          logger.debug(
            `📡 Fallback: Fetching live matches from ISports for league ${leagueId}`,
          );
      ...
      if (this.sportradar && (await this._isProviderEnabled("SPORTRADAR"))) {
        try {
          logger.debug(
            "📡 Trying Sportradar for global live matches (matches_by_date)",
          );
          ...
          if (sr && sr.ok) {
            ...
            let rawMatches =
              (sr.data && (sr.data.matches || sr.data.summaries || sr.data)) ||
              [];
            if (!Array.isArray(rawMatches) && rawMatches && rawMatches.matches)
              rawMatches = rawMatches.matches;
            if (Array.isArray(rawMatches) && rawMatches.length > 0) {
              logger.info(
                `✅ Sportradar: Found ${rawMatches.length} matches from matches_by_date`,
              );
              await this.dataCache.storeLiveMatches("sportradar", rawMatches);
              const formatted =
                this._formatMatches(rawMatches, "sportradar") || [];
              const liveOnly = formatted.filter(
                (m) => String(m.status || "").toUpperCase() === "LIVE",
              );
              if (liveOnly.length > 0) {
                ...
                await this._recordProviderHealth(
                  "sportradar",
                  true,
                  `Found ${liveOnly.length} live matches`,
                );
                return liveOnly;
              }
              ...
              if (formatted.length > 0) {
                ...
                await this._recordProviderHealth(
                  "sportradar",
                  true,
                  `Found ${formatted.length} matches`,
                );
                return formatted;
              }
            }
      ...
      logger.debug(
        `📚 Attempting to read live matches from cache for league ${leagueId}`,
      );
      try {
        ...
        const fdForLeague = (fdCached || []).filter((m) => {
          const leagueFromMatch =
            m.competition?.id || m.competition?.league_id || m.league?.id;
          return leagueFromMatch === leagueId;
        });
        if (fdForLeague && fdForLeague.length > 0) {
          logger.info(
            `📚 Using cached Football-Data live matches (${fdForLeague.length} matches)`,
          );
          const formatted =
            this._formatMatches(fdForLeague, "footballdata") || [];
          this._setCached(cacheKey, formatted);
          return formatted;
        }
        ...
        if (smForLeague && smForLeague.length > 0) {
          logger.info(
            `📚 Using cached SportMonks live matches (${smForLeague.length} matches)`,
          );
          const formatted =
            this._formatMatches(smForLeague, "sportsmonks") || [];
          this._setCached(cacheKey, formatted);
          return formatted;
        }
      } catch (cacheErr) {
        logger.debug(
          "Failed to read live matches from RawDataCache",
          cacheErr?.message,
        );
      }

      logger.warn(
        "⚠️  No live matches available from SportMonks or Football-Data",
      );
      return [];
    } catch (err) {
  ...
  async getAllLiveMatches() {
    try {
      ...
      if (CONFIG.FOOTBALLDATA && CONFIG.FOOTBALLDATA.KEY) {
        try {
          logger.debug(
            "📡 Fetching live matches from Football-Data (GLOBAL endpoint)",
          );
          const fdGlobal = await this._getLiveFromFootballDataGlobal();
          logger.debug(
            `getAllLiveMatches: FD global returned ${fdGlobal ? fdGlobal.length : 0} matches`,
          );
          if (fdGlobal && fdGlobal.length > 0) {
            const formatted = this._formatMatches(fdGlobal, "football-data");
            logger.info(
              `✅ Football-Data (global): Found ${formatted.length} live matches`,
            );
            ...
            await this._recordProviderHealth(
              "footballdata",
              true,
              `Found ${formatted.length} live matches (global)`,
            );
            return formatted;
          }
          ...
          logger.debug(
            "📡 Football-Data global returned no live matches; skipping per-league to avoid rate limits",
          );
      ...
      if (
        ...
        try {
          ...
          if (smMatches && smMatches.length > 0) {
            logger.info(
              `✅ SportMonks: Found ${smMatches.length} total live matches globally (fallback)`,
            );
            await this.dataCache.storeLiveMatches("sportsmonks", smMatches);
            const formatted =
              this._formatMatches(smMatches, "sportsmonks") || [];
            ...
            logger.info(
              `🔍 SportMonks DIAGNOSTIC: raw:${smMatches.length} | formatted:${formatted.length} | live:${liveOnly.length}`,
            );
            if (liveOnly.length > 0) {
              this._setCached(cacheKey, liveOnly);
              await this._recordProviderHealth(
      ...
      try {
        ...
        if (fdCached && fdCached.length > 0) {
          logger.info(
            `📚 Using cached Football-Data live matches (${fdCached.length} matches)`,
          );
          const formatted =
            this._formatMatches(fdCached, "football-data") || [];
          this._setCached(cacheKey, formatted);
          return formatted;
        }
        ...
        if (smCached && smCached.length > 0) {
          logger.info(
            `📚 Using cached SportMonks live matches (${smCached.length} matches)`,
          );
      ...
      logger.warn(
        "⚠️  No live matches available globally from either provider",
      );
      return [];
    } catch (err) {
  ...
  async getFixtures(leagueId = null, options = {}) {
    try {
      // If caller passed a sport name (e.g., 'nba','nfl'), route to sport-specific upcoming
      if (leagueId && typeof leagueId === "string" && isNaN(Number(leagueId))) {
        return await this.getUpcomingBySport(leagueId, options);
      }
      if (!leagueId) {
        ...
        for (const compId of competitions) {
          try {
            const fixtures = await this.getUpcomingMatches(compId, {
              sport: "football",
            });
    ...
    try {
      ...
      if (this.sportsgameodds) {
        try {
          ...
          const sgoFixtures =
            await this.sportsgameodds.getUpcomingFixtures(leagueId);
          if (sgoFixtures && sgoFixtures.length > 0) {
            logger.info(
              `✅ SportGameOdds: Found ${sgoFixtures.length} upcoming matches`,
            );
            await this.dataCache.storeFixtures(
              "sportsgameodds",
              leagueId,
              sgoFixtures,
            );
            const formatted =
              this._formatMatches(sgoFixtures, "sportsgameodds") || [];
            this._setCached(cacheKey, formatted);
            await this._recordProviderHealth(
              "sportsgameodds",
              true,
              `Found ${formatted.length} upcoming matches`,
            );
            return formatted;
          }
        } catch (e) {
          logger.warn(
            "SportGameOdds upcoming fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (CONFIG.SPORTSMONKS && CONFIG.SPORTSMONKS.KEY) {
        try {
          ...
          const smFixtures = await this.sportmonks.getFixtures({
            league_id: leagueId,
          });
          if (smFixtures && smFixtures.length > 0) {
            logger.info(
              `✅ SportMonks: Found ${smFixtures.length} upcoming matches`,
            );
            ...
            const formatted =
              this._formatMatches(smFixtures, "sportsmonks") || [];
            this._setCached(cacheKey, formatted);
            await this._recordProviderHealth(
              "sportsmonks",
              true,
          ...
          logger.warn(
            "SportMonks upcoming fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (CONFIG.FOOTBALLDATA && CONFIG.FOOTBALLDATA.KEY) {
        try {
          ...
          if (fdMatches && fdMatches.length > 0) {
            logger.info(
              `✅ Football-Data: Found ${fdMatches.length} upcoming matches`,
            );
            // Store raw data for audit/diagnostics
            await this.dataCache.storeFixtures(
              "footballdata",
          ...
          logger.warn(
            "Football-Data upcoming fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (this.isports) {
        try {
          logger.debug(
            "📡 Fallback: Fetching upcoming matches from ISports (API-Football)",
          );
      ...
      // 🔄 FALLBACK: Try to read from RawDataCache (prefetched data)
      logger.debug(
        `📚 Attempting to read upcoming fixtures from cache for league ${leagueId}`,
      );
      try {
        // Try both sources from cache
        const smCached = await this.dataCache.getFixtures(
          "sportsmonks",
          leagueId,
        );
        if (smCached && smCached.length > 0) {
          logger.info(
            `📚 Using cached SportMonks fixtures (${smCached.length} matches)`,
          );
          this._setCached(cacheKey, smCached);
          return this._formatMatches(smCached, "sportsmonks");
        }

        const fdCached = await this.dataCache.getFixtures(
          "footballdata",
          leagueId,
        );
        if (fdCached && fdCached.length > 0) {
          logger.info(
            `📚 Using cached Football-Data fixtures (${fdCached.length} matches)`,
          );
      ...
      logger.warn(
        "⚠️  No upcoming matches available from SportMonks or Football-Data",
      );
      return [];
    } catch (err) {
  ...
  async getOdds(leagueId, options = {}) {
    try {
      ...
      logger.info(
        "ℹ️  No odds available from SportMonks (Football-Data does not provide odds)",
      );
      return [];
    } catch (err) {
  ...
  async getStandings(leagueId, season = null) {
    try {
      ...
      if (
        this.sportmonks &&
        typeof this.sportmonks.getStandings === "function"
      ) {
        try {
          logger.debug("📡 Fetching standings via SportMonks");
          const smStandings = await this.sportmonks.getStandings(
            leagueId,
            season,
          );
          if (smStandings && smStandings.length > 0) {
            logger.info(
              `✅ SportMonks: Found ${smStandings.length} standings entries`,
            );
            this._setCached(cacheKey, smStandings);
            await this._recordProviderHealth(
              "sportsmonks",
              true,
              `Found ${smStandings.length} standings`,
            );
            return smStandings;
          }
        } catch (e) {
          logger.warn(
            "SportMonks standings fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
      ...
      if (CONFIG.FOOTBALLDATA && CONFIG.FOOTBALLDATA.KEY) {
        try {
          ...
          const fdStandings = await this._getStandingsFromFootballData(
            leagueId,
            season,
          );
          if (fdStandings && fdStandings.length > 0) {
            logger.info(
              `✅ Football-Data: Found ${fdStandings.length} standings entries`,
            );
            this._setCached(cacheKey, fdStandings);
            await this._recordProviderHealth(
              "footballdata",
              true,
              `Found ${fdStandings.length} standings`,
            );
            return fdStandings;
          }
        } catch (e) {
          logger.warn(
            "Football-Data standings fetch failed",
            e?.message || String(e),
          );
          try {
            await this._recordProviderHealth(
    ...
    try {
      ...
      if (typeof this.sportmonks._fetch === "function") {
        ...
        for (const r of records) {
          ...
          else if (Number(a) > Number(h)) awayWins++;
          else draws++;
        }
  ...
  async _fetchApiSports(path, options = {}, retries = 2) {
    ...
    if (!this._isAllowedSync("API_FOOTBALL")) {
      throw new Error(
        "API-Football is not allowed by the SportsAggregator configuration",
      );
    }
    ...
    if (!CONFIG.API_FOOTBALL.KEY) {
      throw new Error(
        "API_FOOTBALL_KEY or API_SPORTS_KEY environment variable not set",
      );
    }
    ...
    const strategies = this._apiSportsStrategy
      ? [
          this._apiSportsStrategy,
          ...this._apiSportsStrategies.filter(
            (s) => s.name !== this._apiSportsStrategy.name,
          ),
        ]
    ...
    for (const strat of strategies) {
      ...
      try {
        ...
        lastError.err = e;
        logger.debug(
          `API-Sports strategy failed (${strat.name}): ${e.message}`,
        );
        // try next strategy
        continue;
      }
  ...
  async _getLiveFromApiSports(leagueId) {
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (response.response && response.response.length > 0) {
          logger.info(
            `API-Sports (${strategy.name}): Got ${response.response.length} matches`,
          );
    ...
    logger.warn(
      `API-Sports live matches for league ${leagueId}: No data from any strategy`,
    );
    return [];
  }
  ...
  async _getOddsFromApiSports(leagueId) {
    ...
    for (const strategy of strategies) {
      try {
        ...
        if (response.response && response.response.length > 0) {
          logger.info(
            `API-Sports odds (${strategy.name}): Got ${response.response.length} matches`,
          );
    ...
    logger.warn(
      `API-Sports odds for league ${leagueId}: No data from any strategy`,
    );
    return [];
  }
  ...
  async _getLiveFromFootballDataGlobal() {
    try {
      ...
      logger.debug(
        `Football-Data global response: ${response ? `${(response.matches || []).length} total matches` : "no response"}`,
      );
      const matches = (response.matches || []).filter(
        (m) => m.status === "LIVE" || m.status === "IN_PLAY",
      );
      logger.debug(
        `Football-Data global: ${matches.length} live/in_play matches found`,
      );
      return matches.slice(0, 200);
    } catch (e) {
      logger.warn(
        "Football-Data global live fetch failed",
        e?.message || String(e),
      );
      return [];
    }
  ...
  async _getLiveFromFootballData(leagueId) {
    ...
    try {
      ...
      logger.warn(
        `Football-Data live matches for league ${fdLeagueId} failed: ${e.message}`,
      );
      return [];
    }
  }
  ...
  async _getStandingsFromFootballData(leagueId, season) {
    ...
    try {
      ...
      logger.warn(
        `Football-Data standings for league ${fdLeagueId} failed: ${e.message}`,
      );
      return [];
    }
  }
  ...
  async _getUpcomingFromFootballData(leagueId) {
    ...
    try {
      ...
      logger.warn(
        `Football-Data upcoming matches for league ${fdLeagueId} failed: ${e.message}`,
      );
      return [];
    }
  }
  ...
  async _getLiveFromSportsData() {
    ...
    try {
      ...
      return (Array.isArray(response) ? response : response.games || []).slice(
        0,
        10,
      );
    } catch (e) {
  ...
  async _getOddsFromSportsData() {
    ...
    try {
      ...
      return (Array.isArray(response) ? response : response.odds || []).slice(
        0,
        10,
      );
    } catch (e) {
  ...
  async _getStandingsFromSportsData(leagueId) {
    ...
    try {
      ...
      return (
        Array.isArray(response) ? response : response.standings || []
      ).slice(0, 20);
    } catch (e) {
  ...
  async _getLiveFromSportsMonks(sport = "football") {
    try {
      ...
      if (!Array.isArray(items) || items.length === 0) {
        logger.warn(
          `⚠️  SportMonks returned no live matches (${items.length} items)`,
        );
        return [];
      }
      ...
      const normalized = items.slice(0, 10).map((it) => {
        ...
        const status =
          it.result_info ||
          it.state ||
          `Starts ${it.starting_at || it.starting_at_timestamp || ""}`;

        return {
          id:
            it.id ||
            `${it.fixture_id || Math.random().toString(36).slice(2, 9)}`,
          home,
          away,
          home_team: home,
          away_team: away,
  ...
  async _fetchWithRetry(url, options = {}, retries = 3) {
      ...
      try {
        ...
        if (resp && resp.ok) {
          try {
            ...
            logger.warn(
              `Failed to parse JSON from ${url}, returning text fallback`,
            );
            return txt;
          }
        }
        ...
        const err = new Error(
          `HTTP ${resp.status} ${resp.statusText} - ${body ? body.substring(0, 200) : ""}`,
        );
        lastErr = err;
        // exponential backoff
    ...
    logger.warn(
      `_fetchWithRetry failed for ${url}: ${lastErr?.message || lastErr}`,
    );
  ...
  _normalizeOpenLigaMatch(m) {
    ...
    try {
      const home =
        (m.Team1 && (m.Team1.TeamName || m.Team1.Name)) || m.home || "Home";
      const away =
        (m.Team2 && (m.Team2.TeamName || m.Team2.Name)) || m.away || "Away";
      const homeScore =
        (m.MatchResults &&
          m.MatchResults[0] &&
          m.MatchResults[0].PointsTeam1) ||
        null;
      const awayScore =
        (m.MatchResults &&
          m.MatchResults[0] &&
      ...
      const status = m.MatchIsFinished
        ? "FINISHED"
        : m.MatchDateTime && new Date(m.MatchDateTime) > new Date()
          ? "SCHEDULED"
          : "LIVE";
      ...
      return {
        ...
        venue:
          m.Location && m.Location.LocationCity
            ? `${m.Location.LocationCity}, ${m.Location.LocationStadium || ""}`
            : "TBA",
        provider: "openligadb",
        raw: m,
      };
  ...
  _formatMatches(matches, source) {
    ...
    const mapped = matches.map((m) => {
      ...
      if (source === "api-sports") {
        return {
          id:
            (m.fixture && (m.fixture.id || m.fixture.fixture_id)) ||
            m.id ||
            null,
          ...
          homeScore:
            (m.goals &&
              (typeof m.goals.home === "number"
                ? m.goals.home
                : m.goals.home || null)) ||
            (m.score && m.score.fulltime && m.score.fulltime.home) ||
            null,
          awayScore:
            (m.goals &&
              (typeof m.goals.away === "number"
                ? m.goals.away
                : m.goals.away || null)) ||
            (m.score && m.score.fulltime && m.score.fulltime.away) ||
            null,
          status: safe(
            m.fixture &&
              m.fixture.status &&
          ...
          time:
            m.fixture && m.fixture.status === "LIVE" && m.fixture.elapsed
              ? `${m.fixture.elapsed}'`
              : safe(
                  m.fixture && (m.fixture.date || m.fixture.timestamp),
                  "TBA",
                ),
          venue: safe(
            m.fixture && m.fixture.venue && m.fixture.venue.name,
            "TBA",
          ),
          provider: "api-sports",
          raw: m,
      ...
      if (source === "football-data" || source === "footballdata") {
        ...
        let homeName = safe(
          m.homeTeam &&
            (m.homeTeam.name || m.homeTeam.fullName || m.homeTeam.shortName),
          "Home",
        );
        let awayName = safe(
          m.awayTeam &&
            (m.awayTeam.name || m.awayTeam.fullName || m.awayTeam.shortName),
          "Away",
        );

        // Fallback to direct team properties if nested structure missing
        if (homeName === "Home" && m.homeTeamName)
          homeName = safe(m.homeTeamName, "Home");
        if (awayName === "Away" && m.awayTeamName)
          awayName = safe(m.awayTeamName, "Away");
        ...
        if (m.score && typeof m.score === "object") {
          if (m.score.fullTime) {
            ...
            homeScore =
              typeof m.score.current.home === "number"
                ? m.score.current.home
                : null;
            awayScore =
              typeof m.score.current.away === "number"
                ? m.score.current.away
                : null;
          }
        }

        // Fallback for direct score properties
        if (homeScore === null && typeof m.homeTeamScore === "number")
          homeScore = m.homeTeamScore;
        if (awayScore === null && typeof m.awayTeamScore === "number")
          awayScore = m.awayTeamScore;

        logger.debug(
          `[FOOTBALLDATA_FORMAT] ${homeName} vs ${awayName} | status:${m.status} | score:${homeScore}:${awayScore}`,
        );
        ...
        return {
          ...
          league: safe(
            m.competition && (m.competition.name || m.competition.shortName),
            "Unknown",
          ),
      ...
      if (source === "sportsmonks") {
        ...
        if (!m._logged) {
          logger.info(
            `[SPORTSMONKS_RAW] Match ID: ${m.id} | Keys: ${Object.keys(m).join(", ")}`,
          );
          try {
            if (m.participants)
              logger.info(
                `[SPORTSMONKS_RAW] Participants structure: ${JSON.stringify(m.participants?.slice(0, 2))}`,
              );
            if (m.teams)
              logger.info(
                `[SPORTSMONKS_RAW] Teams structure: ${JSON.stringify(m.teams)}`,
              );
          } catch (e) {
            logger.info(
              "[SPORTSMONKS_RAW] Failed to stringify nested structures",
            );
          }
        ...
        if (
          ...
          const rawTitle = String(
            m.name || m.title || m.fixture_title || "",
          ).trim();
          if (rawTitle) {
        ...
        if (
          !["LIVE", "SCHEDULED", "FINISHED", "POSTPONED"].includes(
            String(status),
          )
        ) {
          const textState = (m.state || m.result_info || "")
            .toString()
            .toLowerCase();
          if (
            textState.includes("live") ||
            textState.includes("in progress") ||
            textState.includes("ht") ||
            textState.includes("1st")
          ) {
            status = "LIVE";
          }
        }
        ...
        logger.debug(
          `[SPORTSMONKS_FORMAT] ${homeName} vs ${awayName} | status:${status} | home:${homeScore} away:${awayScore}`,
        );
        logger.info(
          `[SPORTSMONKS_FORMAT] FORMATTED => home:${homeName} | away:${awayName} | status:${status}`,
        );
        ...
        return {
          ...
          time: timeStr,
          league:
            (m.league && (m.league.name || m.league.fullName)) ||
            m.league_id ||
            "Unknown",
      ...
      if (source === "espn") {
        return {
          ...
          homeScore:
            m.home && typeof m.home.score === "number" ? m.home.score : null,
          awayScore:
            m.away && typeof m.away.score === "number" ? m.away.score : null,
          status: safe(m.status, "UNKNOWN"),
          ...
          for (const k of keys) {
            ...
            if (k.indexOf(".") > -1) {
              ...
              if (ok && (typeof cur === "string" || typeof cur === "number"))
                return cur;
              if (ok && cur && typeof cur === "object" && cur.name)
                return cur.name;
              continue;
            }
      ...
      if (source === "sportradar") {
        ...
        const pickNameFromCompetitors = (arr, sidePreferred) => {
          try {
            ...
            return (
              found.name ||
              (found.team && (found.team.name || found.team.fullName)) ||
              null
            );
          } catch (e) {
            return null;
        ...
        const homeFromCompetitors = pickNameFromCompetitors(
          m.competitors || m.participants || m.teams,
          "home",
        );
        const awayFromCompetitors = pickNameFromCompetitors(
          m.competitors || m.participants || m.teams,
          "away",
        );
        ...
        const homeName = safe(
          homeFromCompetitors ||
            (m.home && (m.home.name || m.home.team_name)) ||
            (m.home_team && (m.home_team.name || m.home_team.fullName)) ||
            (m.localteam && m.localteam.name),
          "Home",
        );
        const awayName = safe(
          awayFromCompetitors ||
            (m.away && (m.away.name || m.away.team_name)) ||
            (m.away_team && (m.away_team.name || m.away_team.fullName)) ||
            (m.visitorteam && m.visitorteam.name),
          "Away",
        );
        ...
        const status = safe(
          m.status ||
            m.match_status ||
            (m.state && m.state.status) ||
            (m.result && m.result.status),
          "UNKNOWN",
        );
        const time =
          m.schedule && m.schedule.start_time
            ? safe(m.schedule.start_time)
            : safe(
                m.match_time ||
                  m.generated_at ||
                  m.date ||
        ...
        const league = safe(
          (m.tournament && (m.tournament.name || m.tournament.fullName)) ||
            (m.competition && (m.competition.name || m.competition.fullName)) ||
            m.league,
          "Unknown",
        );
      ...
      return {
        ...
        home: safe(
          m.home ||
            ...
            (m.title && m.title.split && String(m.title).split(" - ")[0]),
          "Home",
        ),
        away: safe(
          m.away ||
            (m.teams && m.teams.away && m.teams.away.name) ||
            (m.title && m.title.split && String(m.title).split(" - ")[1]),
          "Away",
        ),
        ...
        time: safe(
          m.time || (m.fixture && m.fixture.elapsed) || m.date || "TBA",
        ),
        venue: safe(
          m.venue ||
            (m.fixture && m.fixture.venue && m.fixture.venue.name) ||
    ...
    return mapped.map((o) => {
      const home = String(
        o.home || (o.teams && o.teams.home && o.teams.home.name) || "Home",
      );
      const away = String(
        o.away || (o.teams && o.teams.away && o.teams.away.name) || "Away",
      );
    ...
    try {
      ...
      const findIn = (arr) => {
        ...
        return arr.find(
          (m) =>
            ...
            (m.raw &&
              (String(m.raw.id) === String(matchId) ||
                String(m.raw.match_id) === String(matchId))),
        );
      };
      ...
      if (sport === "soccer" || sport === "football") {
        ...
        try {
          ...
          logger.debug(
            "Aggregator SportMonks live lookup failed",
            e?.message || String(e),
          );
        }
        ...
        if (this.sportmonks && CONFIG.SPORTSMONKS && CONFIG.SPORTSMONKS.KEY) {
          try {
            ...
            logger.debug(
              "SportMonks service match lookup failed",
              e?.message || String(e),
            );
          }
        }
