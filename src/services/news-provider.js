...
export async function getNewsHeadlines({ query = "football", max = 10 } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `News fetch failed: ${res.status} ${res.statusText} - ${text.slice(0, 200)}`,
    );
  }
...
export async function getRedditHeadlines({
  ...
  const headers = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) BetrixBot/1.0",
  };
  const urls = [
  ...
  for (const url of urls) {
    try {
      ...
      if (!res.ok) {
        lastErr = new Error(
          `Reddit RSS fetch failed: ${res.status} ${res.statusText}`,
        );
        continue;
      }
      const rss = await res.text();
