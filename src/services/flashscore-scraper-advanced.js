 ...
 */
async function renderFlashscorePage(
  url,
  waitSelector = '[class*="event__"]',
  timeoutMs = 10000,
) {
  if (!puppeteer || !browserInstance) {
    // Fallback to HTML scraping
    logger.debug(
      `Puppeteer unavailable; falling back to HTML scraping for ${url}`,
    );
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `Selector ${waitSelector} not found within 3s, proceeding with available content`,
        );
      }
    ...
    logger.warn(
      `Puppeteer rendering failed for ${url}: ${e.message}; falling back to HTML`,
    );
    return await fetchWithRetry(url);
  }
}
 ...
 */
export async function getLiveMatchesFlashscoreAdvanced(
  leagueId = "1",
  usePuppeteer = true,
) {
  try {
    ...
    logger.info(
      `Scraped ${matches.length} matches from Flashscore${usePuppeteer ? " (Puppeteer)" : " (HTML)"}`,
    );
    return matches;
  } catch (e) {
...
export async function getFlashscoreMatchDetailsAdvanced(matchId) {
  try {
    ...
    const homeTeam = $('[class*="homeTeam__name"], [class*="home"]')
      .first()
      .text()
      .trim();
    const awayTeam = $('[class*="awayTeam__name"], [class*="away"]')
      .first()
      .text()
      .trim();
    const scoreHome = $('[class*="homeScore"], [class*="score"]')
      .first()
      .text()
      .trim();
    const scoreAway = $('[class*="awayScore"], [class*="score"]')
      .last()
      .text()
      .trim();
    ...
    $('[class*="stat"]').each((i, elem) => {
      try {
        const $row = $(elem);
        const statName = $row
          .find('[class*="stat__name"]')
          .text()
          .toLowerCase()
          .trim();
        const homeVal = $row
          .find('[class*="stat__value"]')
          .first()
          .text()
          .trim();
        const awayVal = $row
        ...
        if (statName.includes("possess"))
          stats.possession = { home: homeVal, away: awayVal };
        else if (statName.includes("shot"))
          stats.shots = { home: homeVal, away: awayVal };
        else if (statName.includes("pass"))
          stats.passes = { home: homeVal, away: awayVal };
