...
export class MultiSportAnalyzer {
  ...
  async analyzeMatch(
    ...
    try {
      ...
      if (!matchedSport) {
        return {
          status: "error",
          message: `Sport '${sport}' not supported. Available: ${Object.values(
            sports,
          )
            .map((s) => s.name)
            .join(", ")}`,
      ...
      const matchData = await this._getMatchData(
        matchedSport,
        homeTeam,
        awayTeam,
        leagueId,
  ...
  _normalizeFootballData(match) {
    return {
      ...
      injuries: [{ team: "home", player: "Key Player", status: "OUT" }],
      weather: {
        temp: 22,
        condition: "Clear",
        windSpeed: 12,
  ...
  _generateMockStats(sport) {
    ...
    if (sport === "football") {
      return {
        ...baseStats,
        goalsFor: Math.random() * 2.5,
        goalsAgainst: Math.random() * 1.5,
      };
    } else if (sport === "basketball") {
      return {
        ...baseStats,
        pointsFor: Math.random() * 50 + 80,
        pointsAgainst: Math.random() * 50 + 80,
      };
    }
    return baseStats;
  }
  ...
  _analyzeForm(matchData) {
    ...
    return {
      ...
      analysis:
        `${matchData.homeTeam} in ${homeForm} form vs ${matchData.awayTeam} in ${awayForm} form. ` +
  ...
  _analyzeHeadToHead(matchData) {
    ...
    return {
      ...
      draws,
      analysis:
        `${matchData.homeTeam} leads ${homeWins}-${awayWins} against ${matchData.awayTeam} ` +
  ...
  _analyzeStatistics(matchData, sport) {
    ...
    if (sport === "football") {
      ...
      return {
        ...
        analysis:
          `${matchData.homeTeam} typically dominates possession (${homePossession}%) ` +
  ...
  _analyzeInjuries(matchData) {
    ...
    return {
      ...
      analysis:
        injuries
          .map((i) => `${i.team} missing ${i.player} (${i.status})`)
          .join(". ") +
        ". " +
        (injuries.some((i) => i.team === "home")
          ? "Home team strength affected."
          : "Away team strength affected."),
    };
  }
  ...
  _generateRecommendations(matchData, markets) {
    ...
    // Find best markets
    if (markets.length > 0) {
      const sortedByConfidence = [...markets].sort(
  ...
  formatForTelegram(analysis) {
    ...
    if (analysis.alternativeMarkets.length > 0) {
      ...
      analysis.alternativeMarkets.slice(0, 3).forEach((market, idx) => {
        text +=
          `${idx + 1}. *${market.market}:* ${market.prediction?.outcome || "TBD"} ` +
  ...
  predictHalftimeFulltime(matchData) {
    try {
      ...
      return {
        htft,
        confidence,
        reasoning: `Heuristic based on possession (${hPoss}% vs ${aPoss}%) and shots on target (${hSOT} vs ${aSOT}).`,
      };
    } catch (e) {
  ...
  predictCorrectScores(matchData) {
    try {
      ...
      const factor = Math.max(
        0.6,
        Math.min(1.4, lambdaHome / Math.max(0.1, lambdaAway)),
      );
      const adjusted = candidates.map((c) => ({
        score: c.score,
        prob: Math.min(0.95, Math.max(0.01, c.prob * factor)),
      }));

      // Derive odds approximate
      const result = adjusted.map((c) => ({
        score: c.score,
        confidence: Math.round(c.prob * 100),
        odds: (1 / Math.max(0.01, c.prob)).toFixed(2),
      }));
...
class FootballAnalyzer {
  ...
  async _analyze1X2(matchData, homeTeam, awayTeam) {
    ...
    return {
      ...
      confidence,
      reasoning:
        `${homeTeam} (${matchData.homeForm}) plays ${awayTeam} (${matchData.awayForm}). ` +
  ...
  async _analyzeOverUnder(matchData) {
    const totalGoals =
      (matchData.homeStats?.goalsFor || 1.5) +
      (matchData.awayStats?.goalsFor || 1);
    const threshold = 2.5;
    ...
    return {
      ...
      confidence,
      reasoning:
        `Expected goals: ${totalGoals.toFixed(2)}. ${matchData.homeTeam} scores ${matchData.homeStats?.goalsFor?.toFixed(2)} avg, ` +
  ...
  async _analyzeCorners(matchData) {
    ...
    return {
      ...
      confidence,
      reasoning:
        `${matchData.homeTeam} averages ${homeCorners.toFixed(1)} corners. ` +
  ...
  async _analyzeCards(matchData) {
    const homeCards =
      (matchData.homeStats?.yellowCards || 0) +
      (matchData.homeStats?.redCards || 0) * 2;
    const awayCards =
      (matchData.awayStats?.yellowCards || 0) +
      (matchData.awayStats?.redCards || 0) * 2;
    ...
    return {
      ...
      confidence,
      reasoning:
        `Card history: ${matchData.homeTeam} ${homeCards} cards vs ${matchData.awayTeam} ${awayCards} cards. ` +
  ...
  async _analyzeBothScore(matchData) {
    ...
    return {
      ...
      confidence,
      reasoning:
        `${matchData.homeTeam} scores ${homeGoals.toFixed(2)} goals/game. ` +
  ...
  async _analyzeFirstGoal(matchData) {
    const homeAttack =
      (matchData.homeStats?.shotsOnTarget || 0) /
      Math.max(1, matchData.homeStats?.possession || 50);
    const awayAttack =
      (matchData.awayStats?.shotsOnTarget || 0) /
      Math.max(1, matchData.awayStats?.possession || 50);
    ...
    return {
      ...
      confidence,
      reasoning:
        `Attack efficiency: ${matchData.homeTeam} vs ${matchData.awayTeam}. ` +
