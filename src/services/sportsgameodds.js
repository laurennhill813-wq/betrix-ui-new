...
class SportGameOddsService {
  constructor(redis = null) {
    this.redis = redis;
    this.base = (CONFIG.SPORTSGAMEODDS && CONFIG.SPORTSGAMEODDS.BASE) || 'https://api.sportsgameodds.com/v1';
    this.key = (CONFIG.SPORTSGAMEODDS && CONFIG.SPORTSGAMEODDS.KEY) || null;
    this.cacheTTL = (CONFIG.SPORTSGAMEODDS && CONFIG.SPORTSGAMEODDS.CACHE_TTL) || 600;
  }
  ...
  async _fetch(path, params = {}) {
    ...
    for (const q of queryVariants) {
      for (const h of headerVariants) {
        ...
        try {
          const resp = await axios.get(url, { headers, params: q, timeout: 15000 });
          return resp.data;
        } catch (e) {
          lastErr = e;
    ...
    logger.warn('SportGameOdds fetch failed', lastErr?.message || String(lastErr));
    throw lastErr;
  }
    ...
    try {
      ...
      for (const p of candidates) {
        try {
          ...
          const items = (data && (data.data || data.events || data.results || data.items)) || data || [];
          if (Array.isArray(items) && items.length > 0) {
            // Normalise minimal shape: { home, away, start_ts, league }
            const normalized = items.map(it => {
              const homeRaw = it.home_team || it.home || (it.teams && it.teams.home) || null;
              const awayRaw = it.away_team || it.away || (it.teams && it.teams.away) || null;
              const home = (homeRaw && typeof homeRaw === 'object') ? (homeRaw.name || homeRaw.fullName || homeRaw.title || homeRaw.teamName || null) : homeRaw;
              const away = (awayRaw && typeof awayRaw === 'object') ? (awayRaw.name || awayRaw.fullName || awayRaw.title || awayRaw.teamName || null) : awayRaw;

              return {
                id: it.id || it.event_id || it.fixture_id || null,
                home: home || null,
                away: away || null,
                start: it.start || it.start_time || it.kickoff || it.date || it.utcDate || null,
                raw: it,
              };
            });
            return normalized;
