...
export class SportMonksAPI {
  ...
  async getLiveMatches(sportSlug = "football", limit = 20) {
    ...
    try {
      const response = await axios.get(
        `${this.baseUrl}/${sportSlug}/fixtures/inplay`,
        {
          params: {
      ...
      return (response.data?.data || []).map((match) => ({
        ...
        score: match.scores?.data
          ? `${match.scores.data[0]?.score || 0}-${match.scores.data[1]?.score || 0}`
          : null,
      ...
      logger.error(
        `Failed to fetch live matches from SportMonks (${sportSlug}):`,
        error.message,
      );
      return [];
    }
  ...
  async getLeagues(sportSlug = "football") {
    ...
    try {
      ...
      logger.error(
        `Failed to fetch leagues from SportMonks (${sportSlug}):`,
        error.message,
      );
      return [];
    }
  ...
  async getStandings(leagueId) {
    ...
    try {
      ...
      logger.error(
        `Failed to fetch standings from SportMonks (${leagueId}):`,
        error.message,
      );
      return [];
    }
  ...
  async getMatchDetails(matchId) {
    ...
    try {
      const response = await axios.get(
        `${this.baseUrl}/football/fixtures/${matchId}`,
        {
          params: {
            api_token: this.apiKey,
      ...
      logger.error(
        `Failed to fetch match details from SportMonks (${matchId}):`,
        error.message,
      );
      return null;
    }
  ...
  async searchUpcomingMatches(searchTerm, days = 7) {
    ...
    try {
      ...
      const response = await axios.get(
        `${this.baseUrl}/football/fixtures/between`,
        {
          params: {
            api_token: this.apiKey,
      ...
      logger.error(
        `Failed to search upcoming matches from SportMonks:`,
        error.message,
      );
      return [];
    }
