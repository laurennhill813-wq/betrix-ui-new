...
export async function signUrl(url, opts = {}) {
  const endpoint = process.env.SIGN_ENDPOINT || process.env.SPORTRADAR_SIGN_ENDPOINT || process.env.SPORTRADAR_SIGNER_ENDPOINT;
  if (!endpoint) throw new Error('SIGN_ENDPOINT (or SPORTRADAR_SIGN_ENDPOINT) not configured');
