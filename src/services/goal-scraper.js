...
export async function getLiveMatchesFromGoal(league = "premier-league") {
  try {
    ...
    $(
      '[data-testid="fixture-card"], .fixture-card, [class*="FixtureCard"]',
    ).each((i, elem) => {
      try {
        const $card = $(elem);
        ...
        const homeTeam =
          $card
            .find('[class*="homeTeam"], [data-testid*="home"]')
            .text()
            .trim() || $card.find("span").first().text().trim();
        const awayTeam =
          $card
            .find('[class*="awayTeam"], [data-testid*="away"]')
            .text()
            .trim() || $card.find("span").last().text().trim();
        ...
        const oddsText = $card
          .find('[class*="odd"], [data-testid*="odd"]')
          .text()
          .trim();
