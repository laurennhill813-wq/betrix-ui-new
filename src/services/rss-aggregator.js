  ...
  async fetchFeed(url) {
    ...
    if (this.cache) {
      ...
      const allowed = await this.cache
        .rateLimit(`rss:${host}`, this.rateLimit, this.rateWindow)
        .catch(() => false);
      if (!allowed) {
    ...
    $("item").each((i, el) => {
      ...
      const media =
        $(el)
          .find("media\\:thumbnail, thumbnail, enclosure")
          .first()
          .attr("url") || null;
    ...
    if (this.cache)
      await this.cache.set(cacheKey, out, this.ttl).catch(() => {});
    return out;
  }

  async fetchMultiple(urls = []) {
    const promises = urls.map((u) =>
      this.fetchFeed(u).catch((e) => ({ url: u, error: e.message })),
    );
