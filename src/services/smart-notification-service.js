  ...
  async checkOddsMovement(userId, chatId, matchId, oddsData) {
    try {
      ...
      const delta = Math.abs(
        (oddsData.homeOdds - parseFloat(previousOdds)) /
          parseFloat(previousOdds),
      );
      const threshold = parseFloat(prefs.oddsDelta || 0.1);

      if (delta >= threshold) {
        const direction =
          oddsData.homeOdds > previousOdds ? "📈 UP" : "📉 DOWN";
        const message =
          `⚠️ <b>Odds Movement!</b>\n\n` +
          `Match: ${matchId}\n` +
    ...
    try {
      ...
      if (streak === 5) {
        await this.telegram.sendMessage(
          chatId,
          `🔥 Amazing! You're on a 5-win streak! Keep it going!`,
        );
      } else if (streak === 10) {
        await this.telegram.sendMessage(
          chatId,
          `🏆 Incredible! 10-win streak! You're a legend!`,
        );
      } else if (streak % 5 === 0) {
        await this.telegram.sendMessage(
          chatId,
          `✨ You just hit a ${streak}-win streak! Fantastic!`,
        );
      }
