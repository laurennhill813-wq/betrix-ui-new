  ...
  try {
    ...
    const safeMetadata = (() => {
      try {
        ...
        if (typeof payment.metadata === 'string') return JSON.parse(payment.metadata);
        if (typeof payment.metadata === 'object') return payment.metadata;
        return { value: String(payment.metadata) };
      } catch (e) {
        logger.warn('createPaymentRecord: malformed metadata, coercing to object', e?.message || String(e));
    ...
    const row = {
      ...
      provider: payment.provider || payment.paymentMethod || payment.processor || null,
      provider_ref: payment.providerRef || payment.provider_ref || payment.transactionId || payment.transaction_id || null,
      amount: payment.totalAmount || payment.amount || null,
      currency: payment.currency || (payment.meta && payment.meta.currency) || (safeMetadata && safeMetadata.currency) || null,
    ...
    if (dbClient && dbClient.type === 'drizzle' && dbClient.db) {
      try {
        ...
        const res = await dbClient.db.insert(payments).values({
          user_id: row.user_id,
        ...
        if (row.provider_ref && r && r.setex) {
          try { await r.setex(`payment:by_provider_ref:${row.provider}:${row.provider_ref}`, 30 * 24 * 60 * 60, JSON.stringify({ id: inserted.id })); } catch (e) { /* ignore */ }
        }
        return inserted;
      } catch (e) {
        logger.warn('Drizzle insert failed, falling back to SQL', e?.message || String(e));
      }
    }
    ...
    if (dbClient && (dbClient.pool || dbClient.query)) {
      ...
      const vals = [row.user_id, row.provider, row.provider_ref, row.amount, row.currency, row.status];
      const clientQuery = dbClient.query ? dbClient.query : (sql, params) => pool.query(sql, params);
      const result = await clientQuery(q, vals);
      const inserted = result && result.rows ? result.rows[0] : null;
      if (row.provider_ref && r && r.setex) {
        try { await r.setex(`payment:by_provider_ref:${row.provider}:${row.provider_ref}`, 30 * 24 * 60 * 60, JSON.stringify({ id: inserted && inserted.id })); } catch (e) { /* ignore */ }
      }
      return inserted;
    }
    ...
    if (r && r.setex) {
      ...
      if (row.provider_ref) await r.setex(`payment:by_provider_ref:${row.provider}:${row.provider_ref}`, 30 * 24 * 60 * 60, key);
...
export async function updatePaymentRecord(dbClient, lookup = {}, updates = {}) {
  try {
    ...
    if (dbClient && dbClient.type === 'drizzle' && dbClient.db) {
      ...
      else if (provider && provider_ref) q = q.where(payments.provider.equals(provider)).where(payments.provider_ref.equals(provider_ref));
    ...
    if (transaction_id) { sets.push(`transaction_id = $${idx++}`); vals.push(transaction_id); }
    if (metadata) { sets.push(`metadata = jsonb_strip_nulls(coalesce(metadata, '{}'::jsonb) || $${idx++}::jsonb)`); vals.push(metadata); }

    if (!sets.length) throw new Error('No updates provided');

    let where = '';
    if (id) {
      where = `WHERE id = $${idx++}`; vals.push(id);
    } else if (provider && provider_ref) {
      where = `WHERE provider = $${idx++} AND provider_ref = $${idx++}`; vals.push(provider, provider_ref);
    } else {
