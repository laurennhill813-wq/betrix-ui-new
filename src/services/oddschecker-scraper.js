 ...
 */
export async function getOddscheckerOdds(
  homeTeam,
  awayTeam,
  ...
  try {
    ...
    $('[class*="odds-table"], [class*="bet-odds"], table').each(
      (tableIdx, table) => {
        const $table = $(table);
        ...
        $table.find('tr, [class*="bet-row"]').each((i, row) => {
          try {
            ...
            if (!rowText.includes(homeTeam) && !rowText.includes(awayTeam))
              return;

            // Extract bookmaker name
            const bookmaker = $row
              .find('[class*="bookmaker"], td:first-child')
              .text()
              .trim()
              .substring(0, 20);

            // Extract odds values
            const oddsElements = $row.find(
              '[class*="odds"], td[class*="price"], td[data-odds]',
            );
            const oddsValues = [];
 ...
 */
export async function getBestOddscheckerOdds(
  homeTeam,
  awayTeam,
  outcome = "homeWin",
  league = "premier-league",
