...
export async function getBetfairExchangeOdds(eventId = null) {
  try {
    ...
    $(
      '[class*="runner"], [class*="market-row"], [data-testid*="selection"]',
    ).each((i, elem) => {
      try {
        const $elem = $(elem);
        const name = $elem
          .find('[class*="name"], [class*="runner-name"]')
          .text()
          .trim();

        // Extract back odds (first price)
        const backOdds = $elem
          .find('[class*="back"], [class*="price"]')
          .first()
          .text()
          .trim();
        // Extract lay odds (second price)
        const layOdds = $elem
          .find('[class*="lay"], [class*="price"]')
          .last()
          .text()
          .trim();
    ...
    $('[class*="market"], [class*="competition"]').each((i, elem) => {
      try {
        ...
        const marketName = $market
          ...
          .text()
          .trim();
        const eventCount = $market.find(
          '[class*="event"], [class*="runner"]',
        ).length;
...
export function formatBetfairOdds(odds) {
  ...
  return {
    ...
    recommendation:
      odds.spread && odds.spread < 0.05
        ? "✅ Tight spread - good odds"
        : "⚠️ Wide spread",
    source: "betfair.com",
  };
}
