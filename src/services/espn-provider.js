...
export async function getEspnLiveMatches({ sport = "football" } = {}) {
  ...
  const sportSlugs =
    sport === "soccer"
      ? ["soccer", "football_world_cup", "football_uefa_champ_league"]
      : sport === "football"
        ? ["football", "nfl"]
        : [sport];
  ...
  for (const slug of sportSlugs) {
    try {
      ...
      if (!res.ok) {
        lastError = new Error(
          `ESPN fetch failed: ${res.status} ${res.statusText}`,
        );
        continue;
      }
      ...
      const events = (json.events || []).map((ev) => {
        ...
        return {
          ...
          league:
            ...
            (json.sport && json.sport.slug),
          name:
            ev.fullName ||
            ev.shortName ||
            `${(home.team && (home.team.displayName || home.team.name)) || home.displayName || ""} vs ${(away.team && (away.team.displayName || away.team.name)) || away.displayName || ""}`,
          status:
            (ev.status && ev.status.type && ev.status.type.description) ||
