...
export async function getOddsFromBetExplorer({ sport = "football" } = {}) {
  ...
  try {
    ...
    return fixtures.length > 0
      ? fixtures
      : {
          message: "No fixtures found on BetExplorer",
          raw: html.slice(0, 500),
        };
  } catch (err) {
    return {
      error: err.message,
      note: "BetExplorer scrape failed (may be temporarily unavailable or blocked)",
    };
  }
}

export async function getOddsFromOddsPortal({
