...
export async function fetchAndCacheImage(url) {
  ...
  if (st) {
    ...
    if (now - st.mtimeMs < CACHE_TTL_MS) {
      ...
      try {
        ...
        return { path: cue, contentType: meta.contentType || 'application/octet-stream' };
      } catch (e) {
  ...
  let fetchUrl = url;
  const isSportradar = String(url || '').toLowerCase().includes('sportradar');
  if (isSportradar && SPORTRADAR_KEY && !fetchUrl.includes('api_key=')) {
    fetchUrl = fetchUrl + (fetchUrl.includes('?') ? `&api_key=${SPORTRADAR_KEY}` : `?api_key=${SPORTRADAR_KEY}`);
  }

  // Try multiple header strategies for protected providers (Sportradar, Reuters, Getty)
  const baseHeaders = { 'User-Agent': 'BetrixBot/1.0 (+https://betrix.example)', Accept: '*/*' };
  const headerStrategies = [];
  // default
  headerStrategies.push(baseHeaders);
  // X-API-Key header (some provider endpoints prefer header)
  if (SPORTRADAR_KEY) headerStrategies.push({ ...baseHeaders, 'X-API-Key': SPORTRADAR_KEY });
  // Referer + Origin variations
  headerStrategies.push({ ...baseHeaders, Referer: 'https://developer.sportradar.com/' });
  headerStrategies.push({ ...baseHeaders, Referer: 'https://sportradar.com/' });
  // Combined strategies
  if (SPORTRADAR_KEY) headerStrategies.push({ ...baseHeaders, 'X-API-Key': SPORTRADAR_KEY, Referer: 'https://developer.sportradar.com/' });

  let res = null;
  let lastErr = null;
  for (const headers of headerStrategies) {
    try {
      res = await fetch(fetchUrl, { redirect: 'follow', headers, timeout: 15000 });
    } catch (err) {
      lastErr = err;
      // try next header strategy
      console.warn('[image-proxy] fetch error for', fetchUrl, 'headers=', headers, 'err=', err && err.message);
      res = null;
    }
    if (res) {
      if (!res.ok) {
  ...
  const ct = res.headers && res.headers.get ? (res.headers.get('content-type') || 'application/octet-stream') : 'application/octet-stream';
    ...
    try { if (await statSafe(cue)) await fs.promises.unlink(cue); } catch (e) {}
