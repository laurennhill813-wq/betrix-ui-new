...
const DOMAIN_MIN_INTERVAL_MS = Number(
  process.env.LIVE_SCRAPER_MIN_INTERVAL_MS || 500,
);
...
async function retryFetchJson(url, opts = {}, attempts = 3, baseDelay = 300) {
  for (let i = 0; i < attempts; i++) {
    try {
      ...
      if (proxies.length > 0) {
        ...
        if (proxy) {
          ...
          if (agent) {
            ...
            logger.debug(
              `Using proxy: ${proxy.replace(/:[^:]*@/, ":***@")} for ${new URL(url).hostname}`,
            );
          }
        }
      }
      ...
      if (res.status === 429) {
        logger.warn(
          `Rate limited on ${url} (HTTP 429), attempt ${i + 1}/${attempts}`,
        );
      ...
      logger.warn(
        `retryFetchJson attempt ${i + 1} failed for ${url}: ${e.message || e}`,
      );
  ...
  try {
    ...
    if (box.teams && Array.isArray(box.teams)) {
      box.teams.forEach((t) => {
        ...
        stats[
          teamId || t.team?.displayName || t.team?.name || t.name || t.team
        ] = (t.statistics || []).map((s) => ({
    ...
    try {
      minute =
        (status && (status.detail || status.shortDetail)) ||
        (comp.status && comp.status.type && comp.status.type.name) ||
        null;
    } catch (e) {
      minute = null;
    }
    ...
    return {
      eventId,
      status: status.description || status.detail || null,
      minute,
      competitors,
      stats,
    };
  ...
  try {
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        logger.warn(
          `ScoreBat fetch attempt ${i + 1} failed for ${url}: ${e.message || e}`,
        );
      ...
      try {
        ...
        if (!m.liveStats && (m.title || (m.home && m.away))) {
          return {
            ...m,
            liveStats: {
              note: "No detailed live stats available",
              title: m.title || `${m.home} vs ${m.away}`,
            },
          };
        }
        return m;
        ...
        logger.warn(
          "enrichMatchesWithLiveStats failed for match",
          e.message || e,
        );
        return m;
      }
...
export default {
  getEspnMatchStats,
  getScorebatMatchDetails,
  enrichMatchesWithLiveStats,
};
