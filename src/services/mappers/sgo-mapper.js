...
export function mapSgoOdds(raw, { sport, league } = {}) {
  ...
  return events.map((event) => {
    const homeTeam =
      event.homeTeam?.name || event.home_team || event.home || null;
    const awayTeam =
      event.awayTeam?.name || event.away_team || event.away || null;
    ...
    return normalizeOddsRecord({
      ...
      lastUpdated:
        event.updatedAt || event.lastUpdated || new Date().toISOString(),
    });
  });
}
