...
class ClaudeService {
  constructor(apiKey, model = 'claude-haiku-4.5', timeoutMs = 15000) {
    ...
    this.apiVersion = process.env.ANTHROPIC_API_VERSION || process.env.ANTHROPIC_VERSION || '2023-06-01';
  }
  ...
  async chat(message, context = {}) {
    ...
    const body = {
      ...
      temperature: typeof context.temperature === 'number' ? context.temperature : 0.2,
      stop_sequences: ['\n\nHuman:'],
    };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 400 && (lowCreditMsg.includes('credit') || lowCreditMsg.includes('balance') || lowCreditMsg.includes('billing') || lowCreditMsg.includes('low'))) {
          err.code = 'billing';
        }
        throw err;
      }
