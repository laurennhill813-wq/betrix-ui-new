 ...
 */
export async function callProvider({ base, path, auth = {}, key }, { timeoutMs = 30000, attempts = 3, backoffMs = 300 } = {}) {
  let url = `${base}${path}`;
    ...
    try {
      ...
      if (shouldRetryStatus(res.status) && attempt < attempts) {
        ...
        if (ra) {
          ...
          if (!Number.isNaN(raNum) && raNum > 0) wait = Math.max(wait, raNum * 1000);
        }
        // jitter
