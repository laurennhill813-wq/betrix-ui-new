...
export async function fetchRandomMediaItem({ sport }) {
  ...
  for (const provider of providers) {
    try {
      ...
      if (item) {
        try {
          await cacheSet(cacheKey, item, DEFAULT_TTL);
        } catch (e) {}
        return item;
      }
    } catch (err) {
      ...
      console.warn(
        `media provider ${provider.id} failed: ${err && err.message ? err.message : err}`,
      );
      continue;
    }
  }
  ...
  switch (provider.id) {
    case "reuters":
      return callProvider({
        ...
        path: `/search?query=${encodeURIComponent(sport)}`,
        auth: { method: "query", queryParam: provider.auth.param },
        key: process.env[provider.auth.keyEnv],
      });
    case "getty":
      return callProvider({
        base: provider.baseUrl,
        path: `/search/images/editorial?phrase=${encodeURIComponent(sport)}`,
        auth: { method: "header", headerName: provider.auth.headerName },
        key: process.env[provider.auth.keyEnv],
      });
    case "ap_content":
    case "ap_images":
      return callProvider({
        base: provider.baseUrl,
        path: `/images?sport=${encodeURIComponent(sport)}`,
        auth: { method: "query", queryParam: provider.auth.param },
        key: process.env[provider.auth.keyEnv],
      });
    case "imagn":
      return callProvider({
        base: provider.baseUrl,
        path: `/images/search?sport=${encodeURIComponent(sport)}`,
        auth: { method: "query", queryParam: provider.auth.param },
        key: process.env[provider.auth.keyEnv],
      });
    case "sportradar_images":
      ...
      return callProvider({
        base: provider.baseUrl,
        path: `/flags/${encodeURIComponent(sport)}`,
        auth: { method: "query", queryParam: provider.auth.param },
        key: process.env[provider.auth.keyEnv],
      });
    default:
...
async function pickRandomMedia(res, providerId, fallbackSport) {
  ...
  const url =
    ...
    raw.preview_url ||
    raw.thumbnail_url ||
    (raw.display_sizes && raw.display_sizes[0] && raw.display_sizes[0].uri);
  if (!url) return null;
