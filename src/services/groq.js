...
export default class GroqService {
  constructor(apiKey, model = null, timeoutMs = 20000, baseUrl = 'https://api.groq.com') {
    this.apiKey = apiKey;
  ...
  async chat(message, context = {}) {
    ...
    const messages = [];
    if (context && context.system) messages.push({ role: 'system', content: context.system });
    // if context includes few-shot examples, optionally append them
    if (context && context.few_shot_examples && Array.isArray(context.few_shot_examples)) {
    ...
    try {
      ...
      try {
        ...
        const out = (choice && (choice.message && choice.message.content)) || json.output || json.text || '';
