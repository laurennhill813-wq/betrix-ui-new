  ...
  try {
    ...
    for (const hk of healthKeys.slice(0, 20)) {
      try {
        ...
        healthDetails.push(
          `${hk.replace("betrix:provider:health:", "")}: ${parsed && parsed.ok ? "OK" : "FAIL"} ${parsed && parsed.message ? `(${parsed.message})` : ""}`,
        );
      } catch (e) {
    ...
    for (const sk of strategies.slice(0, 20)) {
      try {
        ...
        stratDetails.push(
          `${sk.replace("betrix:provider:strategy:", "")}: ${val}`,
        );
      } catch (e) {
    ...
    return ctx.reply(
      "Failed to fetch prefetch status: " + String(err.message || err),
    );
  }
}
