...
export async function handleBasketballOdds(ctx) {
  ...
  const events = await getUnifiedOddsWithFair({ sport: 'basketball', league: 'nba' });

  if (!events || events.length === 0) {
    return ctx.reply('No NBA odds available right now. Try again in a few minutes.');
  }
  ...
  const lines = top.map(ev => {
    ...
    const freeUpsell = tier === 'free' ? '🔓 VIP unlocks true fair odds, consensus and best-book edges for NBA.' : null;

    return [header, basicLine, consensusLine, vipLines || freeUpsell].filter(Boolean).join('\n');
  });

  const prefix = tier === 'vip' ? '🏀 NBA Fair Odds & Edges (VIP)' : '🏀 NBA Odds Snapshot';
