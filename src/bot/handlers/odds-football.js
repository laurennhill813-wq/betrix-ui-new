...
export async function handleFootballOdds(ctx) {
  ...
  if (!events || events.length === 0) {
    return ctx.reply(
      "No NFL odds available right now. Try again in a few minutes.",
    );
  }
  ...
  const lines = top.map((ev) => {
    ...
    const freeUpsell =
      tier === "free"
        ? "🔓 Upgrade to VIP for fair odds, consensus and edge intel."
        : null;
  ...
  const prefix =
    tier === "vip" ? "🏈 NFL Fair Odds & Edges (VIP)" : "🏈 NFL Odds Snapshot";
