...
export async function handleFootballOdds(ctx) {
  ...
  const events = await getUnifiedOddsWithFair({ sport: 'football', league: 'nfl' });

  if (!events || events.length === 0) {
    return ctx.reply('No NFL odds available right now. Try again in a few minutes.');
  }

  const top = events.slice(0, 5);

  const lines = top.map(ev => {
    const header = `🏈 NFL | ${ev.homeTeam} vs ${ev.awayTeam}\n🕒 ${ev.startsAt}`;
    const providerSample = ev.providers?.[0];

    const basicLine = providerSample && providerSample.markets?.moneyline
    ...
    const consensusLine = ev.fair?.consensus
      ? `📊 Consensus (raw): H ${(ev.fair.consensus.homeProb * 100).toFixed(1)}% | A ${(ev.fair.consensus.awayProb * 100).toFixed(1)}%`
      : null;

    const vipLines = tier === 'vip' && ev.fair?.fair && ev.fair?.bestOffers
      ? [
    ...
    const freeUpsell = tier === 'free' ? '🔓 Upgrade to VIP for fair odds, consensus and edge intel.' : null;

    return [header, basicLine, consensusLine, vipLines || freeUpsell].filter(Boolean).join('\n');
  });

  const prefix = tier === 'vip' ? '🏈 NFL Fair Odds & Edges (VIP)' : '🏈 NFL Odds Snapshot';
