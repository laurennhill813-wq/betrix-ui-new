...
export async function initiateStkPush({ user_id, msisdn, amount = 300 }) {
  ...
  if (process.env.LIPANA_SECRET || process.env.LIPANA_API_KEY) {
    try {
      ...
      if (checkout && resp.status >= 200 && resp.status < 300) {
        await updatePaymentStatus(tx_ref, "initiated", checkout, {
          provider_checkout_id: checkout,
          lipana_response: resp.raw,
        });
      }
  ...
  if (process.env.MPESA_CONSUMER_KEY && process.env.MPESA_CONSUMER_SECRET) {
    try {
      ...
      const checkout =
        resp?.raw?.CheckoutRequestID || resp?.raw?.MerchantRequestID || null;
      if (checkout) {
        await updatePaymentStatus(tx_ref, "initiated", checkout, {
          provider_checkout_id: checkout,
          mpesa_response: resp.raw,
        });
      }
...
export async function handleMpesaCallback({
  ...
  const normalized =
    status === "SUCCESS" || status === "success" || status === "OK"
      ? "success"
  ...
  try {
    ...
    console.warn(
      "handleMpesaCallback fallback - DB update failed or payment not found",
