...
const ADMIN_USER_ID = process.env.ADMIN_TELEGRAM_ID
...
function payKeyboard() {
  return Markup.inlineKeyboard([
    [
      Markup.button.callback("M-Pesa (Lipana)", "pay:method:lipana"),
      Markup.button.callback("M-Pesa (Daraja)", "pay:method:mpesa"),
    ],
  ]);
}
...
async function pollPaymentStatusAndNotify(ctx, tx_ref, timeoutSeconds = 180) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      ...
      if (status && (status === "success" || status === "failed")) {
        if (status === "success") {
          await ctx.reply(
            "Payment confirmed — thank you! Your account is now active.",
          );
          try {
            await upsertUser({ user_id: ctx.from.id, status: "active" });
          } catch (e) {
            /* ignore */
          }
        } else {
  ...
  await ctx.reply(
    "No confirmation received yet. We will notify you when the payment completes.",
  );
  return null;
}
...
bot.action("pay:method:lipana", async (ctx) => {
  ...
  await ctx.reply(
    "You chose Lipana M-Pesa. We will use M-Pesa STK to collect 300 KES.",
  );
  ...
  await (async function trigger() {
    ...
    if (!msisdn) {
      await ctx.reply(
        "Please send your phone number first (e.g. 2547XXXXXXXX)",
      );
      ctx.session.payAfterNumber = true;
      return;
    }
    await ctx.reply(
      "Initiating STK push — please complete the prompt on your phone. I will wait for confirmation...",
    );
    try {
      const { tx_ref } = await initiateStkPush({
        user_id: userId,
        msisdn,
        amount: 300,
      });
      ...
      await ctx.reply(
        `STK Push initiated. Reference: ${tx_ref}. Please complete the prompt on your phone.`,
...
bot.action("pay:method:mpesa", async (ctx) => {
  ...
  await ctx.reply(
    "You chose Daraja M-Pesa. We will use M-Pesa STK to collect 300 KES.",
  );
...
bot.action(/match:(.+):football/, async (ctx) => {
  ...
  const found = all.find(
    (m) =>
      ...
      `${m.home?.id || ""}:${m.away?.id || ""}` === String(matchId),
  );
  if (!found) {
    await ctx.editMessageText(`Match not found. Showing list instead.`, {
      reply_markup: Markup.inlineKeyboard([
        [Markup.button.callback("🔙 Back", "sport:football")],
      ]).reply_markup,
    });
    return;
  }
...
bot.on("text", async (ctx, next) => {
  ...
  if (s.step === 3) {
    ...
    try {
      ...
      await ctx.reply(
        "Profile saved! You can now pay to unlock full access.",
        payKeyboard(),
      );
    } catch (err) {
      console.error("Error saving user", err);
      await ctx.reply(
        "Sorry, something went wrong saving your profile. Try again later.",
      );
    }
...
bot.action("pay:stk", async (ctx) => {
  ...
  try {
    ...
    ctx.session.lastPayment = { tx_ref, amount: 300 };
    await ctx.reply(
      `STK Push initiated. Reference: ${tx_ref}. Complete the prompt on your phone.`,
      Markup.inlineKeyboard([
        [Markup.button.callback("❌ Cancel payment", "pay:cancel")],
        [Markup.button.callback("🔁 Retry payment", "pay:retry")],
      ]),
...
bot.hears(/^[0-9]{9,12}$/, async (ctx) => {
  ...
  if (ctx.session?.payAfterNumber) {
    ...
    try {
      ...
      ctx.session.lastPayment = { tx_ref, amount: 300 };
      await ctx.reply(
        `STK Push initiated. Reference: ${tx_ref}. Check your phone.`,
        Markup.inlineKeyboard([
          [Markup.button.callback("❌ Cancel payment", "pay:cancel")],
          [Markup.button.callback("🔁 Retry payment", "pay:retry")],
        ]),
...
bot.action("pay:cancel", async (ctx) => {
  ...
  try {
    await updatePaymentStatus(last.tx_ref, "failed", null, {
      cancelled_by: ctx.from.id,
    });
    ...
    await ctx.reply(
      "Could not cancel the payment. Please try again or contact support.",
    );
  }
});
...
bot.action("pay:retry", async (ctx) => {
  ...
  if (!msisdn) {
    try {
      const u = await getUserById(ctx.from.id);
      if (u && u.msisdn) msisdn = u.msisdn;
    } catch (e) {
      /* ignore */
    }
  ...
  if (!msisdn) {
    await ctx.reply(
      "No phone number found. Please send your phone number (e.g. 2547XXXXXXXX) to retry.",
    );
    ctx.session.payAfterNumber = true;
    return;
  }
  try {
    const { tx_ref } = await initiateStkPush({
      user_id: ctx.from.id,
      msisdn,
      amount: last.amount || 300,
    });
    ctx.session.lastPayment = { tx_ref, amount: last.amount || 300 };
    await ctx.reply(
      `Retry initiated. Reference: ${tx_ref}. Please complete the prompt on your phone.`,
      Markup.inlineKeyboard([
        [Markup.button.callback("❌ Cancel payment", "pay:cancel")],
        [Markup.button.callback("🔁 Retry payment", "pay:retry")],
      ]),
...
bot.use(async (ctx, next) => {
  ...
  try {
    ...
    if (user && user.status && user.status !== "active") {
      ...
      await ctx.reply(
        "Your account is not active. Please pay 300 KES to unlock full access.",
        payKeyboard(),
      );
  ...
  try {
    const parts =
      ctx.message && ctx.message.text
        ? ctx.message.text.trim().split(/\s+/).filter(Boolean)
        : [];
    const fixtureId = parts[1];
    if (!fixtureId)
      return await ctx.reply("Usage: /analyse_fixture <fixtureId>");
    ...
    try {
      ...
      fixture = (all || []).find(
        (f) =>
          String(f.id) === String(fixtureId) ||
          String(f.fixture?.id) === String(fixtureId),
      );
    } catch (e) {
    ...
    if (!fixture) {
      ...
      try {
        const live =
          (await sportsAgg.getAllLiveMatches?.()) ||
          (await sportsAgg.getLiveMatches?.()) ||
          [];
        fixture = (live || []).find(
          (m) =>
            String(m.id) === String(fixtureId) ||
            String(m.fixture?.id) === String(fixtureId),
        );
      } catch (e) {
        /* ignore */
      }
    ...
    if (!fixture)
      return await ctx.reply(
        "I couldn't find that fixture. Check the ID and try again.",
      );
    ...
    try {
      ...
      if (!analysis || String(analysis).trim().length === 0)
        return await ctx.reply(
          "BETRIXX could not generate analysis right now. Try again shortly.",
        );
    ...
    try {
      await ctx.reply("Error: analysis failed.");
    } catch (_) {}
  }
});
...
bot.command("odds", async (ctx) => {
  try {
    const parts =
      ctx.message && ctx.message.text
        ? ctx.message.text.split(" ").filter(Boolean)
        : [];
    if (parts.length > 1) {
      ...
      if (
        arg.startsWith("nfl") ||
        arg.startsWith("football") ||
        arg.startsWith("soccer")
      )
        return await handleFootballOdds(ctx);
    }
  ...
  try {
    ...
    if (hasAnalysisKeyword || mentionsVs || looksLikeFixtureId) {
      ...
      if (idMatch) {
        const fid = idMatch[1];
        await ctx.reply(
          "🧠 Looking up that fixture and running a BETRIXX analysis...",
        );
        let fixture = null;
        try {
          const all = await sportsAgg.getFixtures();
          fixture = (all || []).find(
            (f) => String(f.id) === fid || String(f.fixture?.id) === fid,
          );
        } catch (e) {
          /* ignore */
        }
        if (!fixture) {
          try {
            const live =
              ...
              [];
            fixture = (live || []).find(
              (m) => String(m.id) === fid || String(m.fixture?.id) === fid,
            );
          } catch (e) {
            /* ignore */
          }
        ...
        if (!fixture) {
          await ctx.reply(
            "I couldn't find that fixture ID. Send `/analyse_fixture <fixtureId>` or paste the match like 'Liverpool vs Man City'.",
          );
          return;
        }
        try {
          ...
          await ctx.reply(
            "Failed to generate analysis. Try /analyse_fixture <fixtureId>",
          );
          return;
        }
      }
      ...
      if (mentionsVs || hasAnalysisKeyword) {
        await ctx.reply(
          'Do you want a quick BETRIXX take on a match? Send a fixture ID or type like "Liverpool vs Man City" and I will analyse it.',
        );
        return;
      }
    }
    ...
    try {
      ...
      if (resp && String(resp).trim().length > 0) {
        ...
        await ctx.reply(
          "Sorry — I could not think of a reply right now. Try again in a moment.",
        );
      }
    } catch (err) {
    ...
    try {
      await ctx.reply("Sorry, something went wrong while I tried to reply.");
    } catch (_) {}
  }
});
...
app.post("/webhook/mpesa", async (req, res) => {
  ...
  try {
      ...
      if (updated && updated.status === "success" && updated.user_id) {
        try {
          await bot.telegram.sendMessage(
            updated.user_id,
            `Payment received. Thank you — your account is now active.`,
          );
    ...
    const tx_ref =
      payload.tx_ref || payload.reference || payload.checkoutRequestID || null;
    const status = payload.status || payload.result || null;
    const provider_tx_id =
      payload.provider_tx_id || payload.transaction_id || null;
