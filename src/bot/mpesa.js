...
async function getAccessToken({
  ...
  if (!key || !secret)
    throw new Error("Missing MPESA consumer key/secret in environment");
  const url =
...
export async function stkPush({
  amount = 300,
  phone,
  accountReference = "Betrix",
  transactionDesc = "Betrix access",
  env = DEFAULTS.env,
  consumerKey,
  consumerSecret,
  ...
  if (!shortcode || !passkey)
    throw new Error(
      "MPESA_SHORTCODE and MPESA_PASSKEY must be set in environment",
    );
