...
async function checkAzure() {
  ...
  const embDeploy =
    ...
    process.env.AZURE_OPENAI_EMBEDDINGS ||
    "text-embedding-3-large";
  console.log("\n=== Azure AI Check ===");
  if (!endpoint || !key || !deployment) {
    console.log(
      "Azure config missing: make sure AZURE_OPENAI_ENDPOINT, AZURE_OPENAI_KEY, AZURE_OPENAI_DEPLOYMENT are set",
    );
    return;
  }
  const s = new AzureAIService(
    ...
    { timeoutMs: 60000, logger: console },
  );
  ...
  try {
    ...
    console.log(
      "Embeddings success: length=",
      Array.isArray(emb) ? emb.length : "unknown",
    );
  } catch (e) {
...
async function checkHuggingFace() {
  console.log("\n=== HuggingFace Check ===");
  const models =
    process.env.HUGGINGFACE_MODELS || process.env.HUGGINGFACE_MODEL || "";
  const token =
    process.env.HUGGINGFACE_TOKEN || process.env.HUGGINGFACE_API_KEY || null;
  if (!models) {
    console.log(
      "No HuggingFace models configured (HUGGINGFACE_MODELS/HUGGINGFACE_MODEL)",
    );
    return;
  }
