...
async function testAnthropic() {
  ...
  try {
    const res = await fetch("https://api.anthropic.com/v1/models", {
      headers: { "x-api-key": key, "Anthropic-Version": anthropicVersion },
      signal,
    });
    clear();
    console.log("Status:", res.status);
...
async function testGROQ() {
  ...
  return await probeEndpoints("GROQ", key, endpoints, [
    "Authorization",
    "x-api-key",
  ]);
}
...
async function testDeepSeek() {
  ...
  const candidates = [
    ...
    { method: "GET", url: "https://deepseek.ai/api/v1/models" },
    {
      ...
      body: { prompt: "hello", max_tokens: 1 },
    },
    {
      method: "POST",
    ...
    {
      method: "POST",
      url: "https://deepseek.ai/api/v1/generate",
      body: { prompt: "hello", max_tokens: 1 },
    },
  ];
  ...
  for (const c of candidates) {
    try {
      ...
      clear();
      console.log(
        `DeepSeek -> tried ${c.url} ${c.method} status=${res.status}`,
      );
      const txt = await res.text();
      console.log("Body preview:", txt.slice(0, 1000));
  ...
  for (const c of candidates) {
    try {
      ...
      clear();
      console.log(
        `DeepSeek (x-api-key) -> tried ${c.url} ${c.method} status=${res.status}`,
      );
      const txt = await res.text();
      console.log("Body preview:", txt.slice(0, 1000));
      return { endpoint: c.url, status: res.status, body: txt };
...
async function testPostgres() {
  ...
  try {
    ...
    console.log("Postgres error:", e.message || e);
    try {
      await client.end();
    } catch {}
  }
...
async function testTelegram() {
  ...
  console.log("\n--- Telegram Bot ---");
  if (!token) {
    console.log("TELEGRAM_TOKEN not set");
    return;
  }
  const { signal, clear } = ac(timeoutMs);
  try {
