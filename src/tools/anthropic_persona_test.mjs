...
const version = process.env.ANTHROPIC_API_VERSION || process.env.ANTHROPIC_VERSION || '2023-06-01';
...
async function run() {
  ...
  try {
    const res = await fetch('https://api.anthropic.com/v1/complete', {
      ...
      body: JSON.stringify({ model, prompt, max_tokens: 300, temperature: 0.2 })
    });
