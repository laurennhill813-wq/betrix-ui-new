...
export async function runMediaAiTick() {
  ...
  const scored = await Promise.all(events.map(async (ev) => ({ ev, score: await scoreEvent(ev) || 0 })));
  ...
  for (const s of scored) {
    ...
    try {
      ...
      const priorities = JSON.parse(process.env.MEDIA_SPORT_PRIORITIES || '{"soccer":1,"nba":1.1,"nfl":1.05,"mlb":0.9,"nhl":0.9,"tennis":0.95}');
      const base = Number(priorities[sport] || 1);
      const hour = new Date().getHours();
      const isPrime = hour >= 18 && hour <= 23; // local prime hours
      const tod = isPrime ? Number(process.env.MEDIA_TOD_PRIME_MULTIPLIER || 1.15) : Number(process.env.MEDIA_TOD_OFF_MULTIPLIER || 1.0);
      s.score = Math.floor(s.score * base * tod);
    } catch (e) { /* ignore weighting errors */ }
  ...
  if (!image || !image.imageUrl) {
    console.warn('[MediaAiTicker] No valid image resolved — attempting text-only fallback');
    try {
      // attempt text-only post via broadcastText
      const captionOnly = (aiSummary && aiSummary.caption) ? aiSummary.caption : `**${chosen.home || 'Home'} vs ${chosen.away || 'Away'}**`;
      await broadcastText(captionOnly);
      await telemetry.incCounter('posts_fallback_text');
      console.info('[MediaAiTicker] Posted text-only fallback', { sport: chosen.sport, league: chosen.league, score: chosen._score });
      if (chosen._eventId) { try { await markEventPosted(chosen._eventId, { sport: chosen.sport, league: chosen.league, fallback: true }); } catch(e){} }
      lastPostedAt = now;
      return;
    } catch (e) {
      console.error('[MediaAiTicker] text-only fallback failed', e && e.message ? e.message : e);
