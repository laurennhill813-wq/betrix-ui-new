...
const POSTED_TTL_SECONDS = Number(process.env.BETRIX_POSTED_TTL_SECONDS || 60 * 60 * 24); // 24 hours
...
export function buildEventId(event = {}) {
  try {
    ...
    const s = String(event.sport || '') + '|' + String(event.home || '') + '|' + String(event.away || '') + '|' + String(event.time || '');
