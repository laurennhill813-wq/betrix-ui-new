  ...
  async function indexDocument(id, text, metadata = {}) {
    ...
    try {
      ...
      logger &&
        logger.warn &&
        logger.warn(
          "RAG indexDocument: embeddings failed",
  ...
  async function retrieveRelevant(namespaceOrUserId, query, { topK = 3 } = {}) {
    ...
    try {
      ...
      logger &&
        logger.warn &&
        logger.warn(
          "RAG retrieveRelevant: embeddings failed, skipping RAG retrieval",
          e && e.message ? e.message : e,
        );
      return [];
    }
    ...
    do {
      ...
      for (const k of keys) {
        try {
          ...
          logger &&
            logger.warn &&
            logger.warn("RAG: failed to parse vector for", k, e && e.message);
        }
      }
    } while (cursor !== "0");
