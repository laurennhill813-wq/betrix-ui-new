...
export class IntelligentMenuBuilder {
  ...
  async buildContextualMainMenu(userId, userData = {}, userStats = {}) {
    ...
    if (tier === "FREE") {
      keyboard.push([{ text: "💰 Upgrade to PRO", callback_data: "sub_pro" }]);
    } else if (tier === "PRO") {
      keyboard.push([
  ...
  buildLeagueBrowserMenu(sport = "football") {
    ...
    keyboard.push([
      { text: "🔙 Back to Sports", callback_data: "sport_football" },
    ]);
  ...
  buildMatchDetailMenu(matchId, leagueId, userTier = "FREE", hasOdds = true) {
    ...
    if (userTier !== "FREE") {
      actions.push([
        // Use upcoming token so analysis resolves scheduled fixtures reliably
        {
          text: "🤖 AI Analysis",
  ...
  buildErrorRecoveryMenu(errorType = "unknown") {
    ...
    const recovery = recoveryOptions[errorType] ||
