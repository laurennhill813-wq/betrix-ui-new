...
function createRedisAdapter(redis) {
  if (!redis) {
    logger.warn(
      "No redis provided to adapter; falling back to in-memory store",
    );
    const mem = new Map();
    return {
      async get(k) {
      ...
      async incr(k) {
        const v = Number(mem.get(k) || 0) + 1;
        mem.set(k, String(v));
        return v;
      },
      async lpush(k, v) {
        const arr = JSON.parse(mem.get(k) || "[]");
        arr.unshift(v);
        mem.set(k, JSON.stringify(arr));
        return arr.length;
      },
      async lpop(k) {
        const arr = JSON.parse(mem.get(k) || "[]");
  ...
  return {
    ...
    hset: (k, obj) =>
      redis.hSet ? redis.hSet(k, obj) : redis.set(k, JSON.stringify(obj)),
    ...
    zincrby: async (k, incr, member) => {
      ...
      if (idx === -1) {
        arr.push({ member, score: Number(incr) });
      } else {
        arr[idx].score = Number(arr[idx].score || 0) + Number(incr);
      }
      await redis.set(k, JSON.stringify(arr));
      return 1;
    },
    zrevrange: async (k, start, stop, withscores) => {
      ...
      const raw = await redis.get(k);
      const arr = raw ? JSON.parse(raw) : []; // sort by score desc
      arr.sort((a, b) => Number(b.score || 0) - Number(a.score || 0));
      if (withscores) {
        const slice = arr
          .slice(start, stop + 1)
          .flatMap((x) => [x.member, String(x.score)]);
        return slice;
      }
    ...
    sadd: async (k, v) => {
      ...
      return redis.set(k, JSON.stringify(arr));
    },
    ...
    zscore: async (k, member) => {
      ...
      const raw = await redis.get(k);
      const arr = raw ? JSON.parse(raw) : [];
      const found = arr.find((x) => x.member === member);
      return found ? String(found.score) : null;
    },
    incrby: async (k, n) => {
      if (redis.incrBy) return redis.incrBy(k, n);
      if (redis.incrby) return redis.incrby(k, n);
      const v = Number((await redis.get(k)) || 0) + Number(n);
      await redis.set(k, String(v));
      return v;
    },
    hincrbyfloat: async (k, field, n) => {
      ...
      obj[field] = Number(obj[field] || 0) + Number(n);
    ...
    srem: async (k, v) => {
      ...
      return redis.set(k, JSON.stringify(out));
    },
    smembers: async (k) => {
    ...
    setex: (k, ttl, v) =>
      redis.setEx ? redis.setEx(k, ttl, v) : redis.set(k, v),
    expire: (k, ttl) =>
      redis.expire ? redis.expire(k, ttl) : Promise.resolve(1),
    incr: (k) =>
      redis.incr
        ? redis.incr(k)
        : (async () => {
            const v = Number((await redis.get(k)) || 0) + 1;
            await redis.set(k, String(v));
            return v;
          })(),
    lpush: (k, v) =>
    ...
    ltrim: (k, start, stop) =>
      redis.lTrim ? redis.lTrim(k, start, stop) : Promise.resolve("OK"),
    rpush: (k, v) =>
      redis.rPush
        ? redis.rPush(k, v)
        : redis.rpush
    ...
    publish: async (channel, message) => {
      ...
      try {
        logger &&
          logger.warn &&
          logger.warn("redis.publish not available; publish is a no-op");
      } catch (e) {
        /* ignore */
      }
      return 0;
