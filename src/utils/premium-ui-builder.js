...
export function buildBetrixHeader(tier = "FREE", user = null) {
  ...
  return (
    `${BETRIX_BRAND.EMOJI} *${BETRIX_BRAND.NAME}* ${tierEmoji}\n` +
...
export function buildMatchCard(match, index = 1, includeOdds = true) {
  ...
  if ((home === "Home" || !home) && match.raw) {
    home =
      (match.raw.teams && match.raw.teams.home && match.raw.teams.home.name) ||
  ...
  if ((away === "Away" || !away) && match.raw) {
    away =
      (match.raw.teams && match.raw.teams.away && match.raw.teams.away.name) ||
      (match.raw.teams && match.raw.teams[1] && match.raw.teams[1].name) ||
      match.raw.visitor_team ||
      match.away ||
      "Away";
  }
...
export function buildMatchStats(match) {
  ...
  if (match.possession) {
    ...
    const homeBar =
      "█".repeat(Math.round(homePos / 5)) +
      "░".repeat(20 - Math.round(homePos / 5));
    const awayBar =
      "█".repeat(Math.round(awayPos / 5)) +
      "░".repeat(20 - Math.round(awayPos / 5));
    stats += `⚙️ Possession:\n${homeBar} ${homePos}%\n${awayBar} ${awayPos}%\n`;
  }
  ...
  if (match.stats) {
    ...
    stats += `📈 Stats:\n`;
    if (s.shots)
      stats += `🎯 Shots: ${s.shots.home || 0} - ${s.shots.away || 0}\n`;
    if (s.shotsOnTarget)
      stats += `🎯 On Target: ${s.shotsOnTarget.home || 0} - ${s.shotsOnTarget.away || 0}\n`;
    if (s.corners)
      stats += `🔃 Corners: ${s.corners.home || 0} - ${s.corners.away || 0}\n`;
    if (s.fouls)
      stats += `🚫 Fouls: ${s.fouls.home || 0} - ${s.fouls.away || 0}\n`;
    if (s.yellowCards)
      stats += `🟨 Yellow: ${s.yellowCards.home || 0} - ${s.yellowCards.away || 0}\n`;
    if (s.redCards)
      stats += `🔴 Red: ${s.redCards.home || 0} - ${s.redCards.away || 0}\n`;
  }
 ...
 */
export function buildMatchActionButtons(
  matchId,
  leagueId = null,
  userTier = "FREE",
) {
 ...
 */
export function buildFixturesDisplay(
  fixtures,
  league = "League",
  view = "upcoming",
) {
  ...
  fixtures.slice(0, 15).forEach((f, i) => {
    const status =
      f.status === "LIVE" || f.status === "live" || f.status === "IN_PLAY"
        ? "🔴"
        : "📅";
    ...
    let home = safeName(
      f.home ||
        f.homeTeam ||
        f.home_name ||
        (f.raw && f.raw.home && f.raw.home.name),
      "Home",
    );
    let away = safeName(
      f.away ||
        f.awayTeam ||
        f.away_name ||
        (f.raw && f.raw.away && f.raw.away.name),
      "Away",
    );
    if (!home) home = "TBA";
    if (!away) away = "TBA";
...
export function buildBetAnalysis(match, analysis = {}) {
  ...
  if (analysis.confidence || analysis.confidence === 0) {
    ...
    const bar =
      "█".repeat(Math.round(conf / 5)) + "░".repeat(20 - Math.round(conf / 5));
  ...
  const bets =
    analysis.preferredBets && analysis.preferredBets.length
      ? analysis.preferredBets
      : analysis.valueBets || [];
  if (bets && bets.length > 0) {
    ...
    let rows = "";
    bets.forEach((bet, i) => {
      const name = sanitize.escapeMarkdownV2(
        String(bet.option || bet.name || bet.title || ""),
      );
      const odds = sanitize.escapeMarkdownV2(
        String(bet.odds || bet.price || "-"),
      );
      const confNum =
      ...
      const stake = sanitize.escapeMarkdownV2(
        String(
          bet.suggested_stake_pct || bet.stake || bet.recommendedStake || "-",
        ),
      );
...
export function buildUpcomingFixtures(
  ...
  league = "",
  daysBefore = 7,
  opts = { showActions: false, userTier: "FREE", page: 1, pageSize: 20 },
) {
  if (!fixtures || fixtures.length === 0) {
    return {
      text: `📭 No upcoming fixtures in the next ${daysBefore} days.`,
      reply_markup: null,
    };
  }
  ...
  pageMatches.forEach((f, i) => {
    ...
    const matchId =
      ...
      f.fixtureId ||
      encodeURIComponent(
        `${home.replace(/\s+/g, "_")}_${away.replace(/\s+/g, "_")}_${idx}`,
      );
    const sportKey = league || "football";
    keyboard.push([
      {
        text: `🔎 ${home} vs ${away}`,
        callback_data: `match:${matchId}:${sportKey}`,
      },
    ]);
  ...
  if (currentPage > 1)
    navRow.push({
      text: "◀ Prev",
      callback_data: `sport:${sportKey}:upcoming:${currentPage - 1}`,
    });
  navRow.push({
    text: `🔄 Refresh`,
    callback_data: `sport:${sportKey}:upcoming:${currentPage}`,
  });
...
export function buildLiveMatchTicker(matches = []) {
  ...
  matches.slice(0, 8).forEach((m) => {
    ...
    let venue = null;
    if (m.venue || m.stadium || m.venue_name)
      venue = safeName(m.venue || m.stadium || m.venue_name, "");
...
export function buildTeamComparison(
  ...
  const stats = [
    ...
    },
    {
      key: "avgGoals",
      label: "⚽ Avg Goals",
      home: homeStats.avgGoals,
      away: awayStats.avgGoals,
    },
    {
      key: "winRate",
      label: "✅ Win Rate",
      home: homeStats.winRate,
      away: awayStats.winRate,
    },
    {
      key: "injuries",
      label: "🏥 Injuries",
      home: homeStats.injuries,
      away: awayStats.injuries,
    },
  ];
