 ...
 */
export function generateBetrixHeader(
  userTier = "FREE",
  userName = "User",
  includeStats = false,
  stats = {},
  ...
  const tierInfo =
    BETRIX_BRANDING.TIERS[userTier] || BETRIX_BRANDING.TIERS["FREE"];
 ...
 */
export function generateBetrixFooter(
...
export function formatNotification(type, data) {
  ...
  const notif = notifications[type] || {
    icon: BETRIX_BRANDING.ICONS.alert,
    text: data.message,
  };
...
export function formatLeagueName(name) {
  const flag =
    BETRIX_BRANDING.FLAGS[name.split(" ")[0]] || BETRIX_BRANDING.ICONS.league;
  return `${flag} *${name}*`;
}
