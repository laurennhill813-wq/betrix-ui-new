  ...
  async function allow(key, limit = 5, windowSeconds = 60) {
    try {
      if (redis && redis.incr) {
        ...
        if (count === 1) {
          try { await redis.expire(`rl:${key}`, windowSeconds); } catch (e) { /* ignore */ }
        }
        return count <= limit;
      }
      // Memory fallback
      ...
      if (now - entry.ts >= windowSeconds) {
        entry.ts = now; entry.count = 1; mem.set(key, entry); return true;
      }
      entry.count += 1; mem.set(key, entry); return entry.count <= limit;
    } catch (err) {
