...
export class FixturesManager {
      ...
      return allFixtures
        .filter((f) => {
          ...
          return (
            ...
            (f.sport || sport).toLowerCase() === sport.toLowerCase()
          );
        })
        .sort((a, b) => {
          const timeA = new Date(a.date || a.time || 0).getTime();
  ...
  formatMatch(match, includeStats = true) {
    ...
    const league = safeName(
      match.league || match.competition || (match.raw && match.raw.competition),
      "",
    );
  ...
  buildFixtureBrowserKeyboard(leagueId, currentView = "upcoming") {
    return {
      inline_keyboard: [
        [
          {
            text: currentView === "upcoming" ? "📅 Upcoming ✓" : "📅 Upcoming",
            callback_data: `fixtures_${leagueId}_upcoming`,
          },
          {
            text: currentView === "live" ? "🔴 Live ✓" : "🔴 Live",
            callback_data: `fixtures_${leagueId}_live`,
          },
        ],
        [
          {
            text:
              currentView === "completed" ? "✅ Completed ✓" : "✅ Completed",
            callback_data: `fixtures_${leagueId}_completed`,
          },
    ...
    try {
      ...
      Object.entries(byLeague).forEach(([league, matches]) => {
        ...
        matches.slice(0, 3).forEach((m) => {
          const time = new Date(m.date || m.time || 0).toLocaleTimeString(
            "en-US",
  ...
  formatMatchPreview(match) {
    const home = safeName(
      match.home || match.homeTeam || (match.raw && match.raw.home_team),
      "Home",
    );
    const away = safeName(
      match.away || match.awayTeam || (match.raw && match.raw.away_team),
      "Away",
    );
