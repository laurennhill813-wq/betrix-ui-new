 ...
 */
export function generateMatchDetailsKeyboard(
  matchId,
  userId,
  hasOdds = false,
  oddsCount = 0,
...
export function generateMatchUpdateNotification(oldMatch, newMatch) {
  ...
  if (oldMatch.score && newMatch.score) {
    if (
      ...
      updates.push(
        `⚽ GOAL! Score changed: ${oldMatch.score.home}-${oldMatch.score.away} → ${newMatch.score.home}-${newMatch.score.away}`,
      );
    }
  }
  ...
  if (oldMatch.stats?.yellowCards && newMatch.stats?.yellowCards) {
    const oldYellow =
      oldMatch.stats.yellowCards.home + oldMatch.stats.yellowCards.away;
    const newYellow =
      newMatch.stats.yellowCards.home + newMatch.stats.yellowCards.away;
