  ...
  for (const r of rows) {
    ...
    try {
      ...
      if (status && String(status).toLowerCase() === "success") {
        ...
        const { rows: updated } = await pool.query(upd, [
          providerId,
          providerId,
          providerId,
        ]);
        ...
        if (telegram && adminId) {
          try {
            await telegram.sendMessage(
              adminId,
              `✅ Reconciled payment ${providerId} -> marked SUCCESS (${updated.length} rows)`,
            );
          } catch (e) {
            void e;
          }
        ...
        if (telegram && adminId) {
          try {
            await telegram.sendMessage(
              adminId,
              `❌ Reconciled payment ${providerId} -> marked FAILED (${updated.length} rows)`,
            );
          } catch (e) {
            void e;
          }
