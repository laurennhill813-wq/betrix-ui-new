...
router.post("/ai-reply", express.json({ limit: "64kb" }), async (req, res) => {
  try {
    ...
    const convoId =
      body.convoId ||
      (body.message && body.message.from && String(body.message.from.id)) ||
      `anon-${Date.now()}`;
    const messageText = (body.message && body.message.text) || body.text || "";

    console.info("AI-REPLY incoming", {
      convoId,
      textPreview: messageText && messageText.slice(0, 120),
    });
    ...
    if (provider.type === "openrouter") {
      ...
      console.warn(
        "Using RapidAPI provider path is not yet implemented; falling back to stub.",
      );
    } else if (provider.type === "openai") {
      console.warn(
        "OpenAI provider configured but OpenRouter preferred; implement OpenAI path if needed.",
      );
    }
...
function ruleResponder(text) {
  ...
  if (/\b(hello|hi|hey)\b/.test(t))
    return "Hello! This is BETRIX AI (stub). How can I help you with sports today?";
  if (/\b(odds|fixture|match|score|next match)\b/.test(t))
    return "Stubbed matches: Team A vs Team B; Team C vs Team D.";
  if (/\b(recommend|tip|prediction)\b/.test(t))
    return "Tip (stub): check recent form and head-to-head. Gamble responsibly.";
  if (t.length < 40)
    return `You said "${text}". Tell me more and I can help with odds, fixtures, or tips.`;
