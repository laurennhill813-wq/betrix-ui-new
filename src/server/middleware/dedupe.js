...
async function initClient() {
  ...
  if (
    !rawUrl ||
    (process.env.DEDUPE_ENABLED &&
      process.env.DEDUPE_ENABLED.toLowerCase() === "false")
  ) {
    console.log(
      "DEDPUPE-MW: REDIS_URL not set or dedupe disabled; dedupe disabled",
    );
    client = null;
    ready = false;
    return;
  ...
  client.on("ready", () => {
    ready = true;
    console.log("DEDPUPE-MW: connected");
  });
  client.on("reconnecting", () => console.log("DEDPUPE-MW: reconnecting"));
  client.on("end", () => {
    ready = false;
    console.log("DEDPUPE-MW: connection ended");
  });
  ...
  try {
    ...
    console.error(
      "DEDPUPE-MW: connection failed; dedupe disabled",
      e && e.message,
    );
    client = null;
    ready = false;
