...
module.exports = function commandRouter(app) {
  ...
  app.post("/webhook/telegram", async (req, res) => {
    ...
    const text =
      update.message && update.message.text ? update.message.text.trim() : "";
    // quick ack
    ...
    (async () => {
      try {
        ...
        if (/^\/PING\b/i.test(text)) {
          ...
          mainMenuPayload.text =
            "BETRIX commands: /PING, /HELP, /BET <stake> <selection>";
          delete mainMenuPayload.reply_markup;
        } else if (/^\/BET\b/i.test(text)) {
          mainMenuPayload.text =
            "Received bet request. Processing... (this is a placeholder)";
          delete mainMenuPayload.reply_markup;
        }
