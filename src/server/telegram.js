  ...
  try {
    ...
    while (stack.length) {
      ...
      if (
        obj.chat &&
        obj.chat.id &&
        (typeof obj.chat.id === "number" || /^\d+$/.test(String(obj.chat.id)))
      )
        return obj.chat.id;
...
function logTelegramResolvedInfo(prefix, update) {
  try {
    ...
    console.log(
      `${prefix} TELEGRAM_RESOLVED_CHAT_ID ${typeof chatId === "undefined" ? "undefined" : chatId}`,
    );
    return chatId;
  } catch (e) {
    console.log(
      `${prefix} TELEGRAM_RESOLVE_ERROR ${e && e.stack ? e.stack : String(e)}`,
    );
    return undefined;
  }
}
