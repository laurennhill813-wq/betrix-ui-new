    ...
    try {
      if (fs.existsSync(c)) {
        ...
        if (
          mod &&
          mod.server &&
          typeof mod.server.createServer === "function"
        ) {
          return {
            mod: { createServer: mod.server.createServer },
            path: c,
            mode: "server.createServer",
          };
        }
      }
    } catch (e) {
...
try {
  ...
  if (found && found.mod && typeof found.mod.createServer === "function") {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "BOOT-WARN: no createServer implementation found in candidates; exporting minimal fallback createServer for safe boot.",
    );
  }
} catch (err) {
  exported = { createServer: fallbackCreateServer };
  // eslint-disable-next-line no-console
  console.error(
    "BOOT-ERROR: unexpected error while locating server implementation:",
    err && err.message,
  );
}
