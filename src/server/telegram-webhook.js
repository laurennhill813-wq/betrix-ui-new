...
async function getRedis() {
  ...
  if (global.__REDIS_CLIENT && global.__REDIS_CLIENT.isOpen) return getRedisAdapter();
...
module.exports = async function telegramWebhookHandler(req, res) {
  try {
    ...
    try {
      const { default: createRedisAdapter } = await import('../utils/redis-adapter.js');
    ...
    if (typeof client.rPush === 'function' || typeof client.rpush === 'function') {
      await (client.rPush ? client.rPush("telegram:updates", JSON.stringify(body)) : client.rpush("telegram:updates", JSON.stringify(body)));
    } else if (typeof client.rpush === 'undefined' && typeof client.lpush === 'function') {
    ...
    try {
      ...
      if (typeof client.rPush === 'function' || typeof client.rpush === 'function') {
        await (client.rPush ? client.rPush("telegram:webhook:queue", auditJob) : client.rpush("telegram:webhook:queue", auditJob));
        await (client.rPush ? client.rPush("webhooks:incoming", auditJob) : client.rpush("webhooks:incoming", auditJob));
