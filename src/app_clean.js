...
try {
  ...
  try {
    webhookRedis = createRedisAdapter(new MockRedis());
  } catch (_) {
    webhookRedis = null;
  }
}
...
app.post("/webhook/telegram", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "Telegram webhook: invalid secret token (telegram endpoint)",
      );
      return res.sendStatus(403);
    }
    if (!webhookRedis) {
      console.warn(
        "Telegram webhook: no Redis available to enqueue update (telegram endpoint)",
      );
      return res.sendStatus(503);
    }
    // Chat-id capture removed — was gated by ENABLE_CHAT_ID_LOG
    ...
    console.error(
      "Telegram webhook error (telegram endpoint)",
