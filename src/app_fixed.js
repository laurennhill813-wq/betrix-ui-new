  ...
  if (process.env.DATABASE_URL && pgSslMode && pgSslMode !== "disable") {
    if (pgSslMode === "verify-ca" || pgSslMode === "verify-full") {
      ...
      if (process.env.PGSSLROOTCERT) {
        try {
          poolConfig.ssl.ca = fs.readFileSync(
            process.env.PGSSLROOTCERT,
            "utf8",
          );
        } catch (e) {
          console.warn(
            "Could not read PGSSLROOTCERT:",
            e?.message || String(e),
          );
        }
      }
...
app.post("/webhook/mpesa", async (req, res) => {
  ...
  try {
    ...
    safeLog(
      "[webhook/mpesa] incoming=",
      incoming,
      "computedHexPrefix=",
      computedHex ? computedHex.slice(0, 16) : null,
    );
    ...
    try {
      ...
      await pool.query(
        "INSERT INTO webhooks(raw_payload, headers, incoming_signature, computed_hex, computed_b64) VALUES($1,$2,$3,$4,$5)",
        [req.body || {}, req.headers || {}, incoming, computedHex, computedB64],
      );
    } catch (e) {
      try {
        const rec = {
        ...
        safeLog(
          "DB insert failed; appended webhook to",
          logPath,
          "and",
          tmpPath,
        );
        ...
        safeLog(
          "DB insert failed and fallback file write failed:",
...
export function registerDataExposureAPI(sportsAggregator) {
  try {
    if (typeof DataExposureHandler !== "undefined")
