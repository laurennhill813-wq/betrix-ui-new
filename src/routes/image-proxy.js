...
router.get('/image-proxy', async (req, res) => {
  try {
    ...
    if (SECRET && !validSig(u, sig)) return res.status(401).json({ ok: false, error: 'invalid signature' });

    const img = await imageProxy.fetchAndCacheImage(u);
    if (!img || !img.path) return res.status(502).json({ ok: false, error: 'failed to fetch' });
...
export default function createImageProxyRouter() { return router; }
