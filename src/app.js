...
app.use((req, res, next) => {
  try {
    ...
    try {
      fs.appendFileSync(path.join(process.cwd(), "debug_requests.log"), rec, {
        encoding: "utf8",
      });
    } catch (e) {
      safeLog("debug log write failed:", e?.message || String(e));
    }
  } catch (e) {
    /* ignore */
  }
...
process.on("uncaughtException", (err) => {
  try {
    safeLog("UncaughtException:", err && err.stack ? err.stack : String(err));
  } catch (_) {
    /* ignore */
  }
});
  ...
  if (process.env.DATABASE_URL && mode && mode !== "disable") {
    if (mode === "verify-ca" || mode === "verify-full") {
      ...
      if (process.env.PGSSLROOTCERT) {
        try {
          cfg.ssl.ca = fs.readFileSync(process.env.PGSSLROOTCERT, "utf8");
        } catch (e) {
          safeLog("Could not read PGSSLROOTCERT:", e?.message || String(e));
        }
      }
    } else {
...
try {
  ...
  pool.on &&
    pool.on("error", (err) =>
      safeLog("Postgres pool error:", err?.message || String(err)),
    );
...
const redisUsername =
  process.env.REDIS_USERNAME || process.env.REDIS_USER || undefined;
const redisPassword =
  process.env.REDIS_PASSWORD || process.env.REDIS_PWD || undefined;
if (redisUrl) {
  try {
    ...
    redisClient = createClient({
      ...
      socket: { tls: String(redisUrl).startsWith("rediss://") },
    });
    redisClient.on("error", (err) =>
      safeLog("Redis error:", err?.message || String(err)),
    );
  } catch (e) {
    safeLog("Failed to create Redis client:", e?.message || String(e));
    redisClient = null;
  }
} else {
  safeLog("REDIS_URL not set; skipping Redis client initialization");
}
...
async function withRetries(fn, { attempts = 3, delayMs = 500 } = {}) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      lastErr = e;
      safeLog(`attempt ${i} failed:`, e?.message || String(e));
...
async function initServices({
  ...
  if (app.locals.rawRedis) {
    try {
      ...
      try {
        if (app.locals.rawRedis && app.locals.rawRedis.disconnect)
          await app.locals.rawRedis.disconnect();
      } catch (_) {
        /* ignore */
      }
...
app.use((req, res, next) => {
  try {
    ...
    safeLog("Request-time debug middleware error:", String(e));
  }
  return next();
});
...
export function registerDataExposureAPI(_sportsAggregator) {
  try {
    safeLog(
      "DATA_EXPOSURE: registerDataExposureAPI called (no-op in migration)",
    );
  } catch (e) {
...
app.use((req, res) => {
  try {
    ...
    try {
      fs.appendFileSync(path.join(process.cwd(), "debug_404.log"), rec, {
        encoding: "utf8",
      });
    } catch (e) {
      safeLog("debug_404 write failed:", e?.message || String(e));
    }
  } catch (e) {
    /* ignore */
  }
  ...
  (async () => {
    try {
      ...
      await Promise.race([
        initPromise,
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error("init timeout")), timeoutMs),
        ),
      ]).catch((e) => {
        safeLog(
          "Service init did not complete before timeout or failed:",
          e?.message || String(e),
        );
      });
      ...
      try {
        ...
        safeLog("Fallback start failed:", err?.message || String(err));
      }
    }
  })();
}
