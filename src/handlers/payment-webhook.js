...
async function alertAdmin(bot, subject, details = {}) {
  try {
    const adminId =
      process.env.ADMIN_TELEGRAM_ID || process.env.TELEGRAM_ADMIN_ID || null;
    if (!adminId) return;
    if (!bot || typeof bot.sendMessage !== "function") return;
    const payload = JSON.stringify(
      details,
      Object.keys(details).slice(0, 20),
      2,
    );
    ...
    try {
      await sendAdminNotification(subject, details);
    } catch (e) {
      logger.warn("sendAdminNotification failed", e?.message || String(e));
    }
  } catch (e) {
...
export async function handleMpesaCallback(req, redis, bot) {
  try {
    ...
    if (MPESA_SECRET) {
      const sig =
        req.headers["x-signature"] || req.headers["x-mpesa-signature"];
    ...
    const mpesaReceiptNumber =
      items.MpesaReceiptNumber || items.mpesaReceiptNumber || "";
    const phoneNumber = items.PhoneNumber || items.phoneNumber || "";
    const transactionDate =
      items.TransactionDate || items.transactionDate || "";
    ...
    if (orderResultCode === 0) {
      ...
      try {
        ...
        if (!orderId && mpesaReceiptNumber) {
          orderId = await redis.get(
            `payment:by_provider_ref:MPESA:${mpesaReceiptNumber}`,
          );
        }

        // Resolve order by quick mappings only (phone or provider reference)
        if (!orderId) {
          logger.warn("No quick mapping found for M-Pesa payment", {
            amount,
            phoneNumber,
          });
          await alertAdmin(bot, "M-Pesa mapping not found", {
            amount,
            phoneNumber,
            mpesaReceiptNumber,
        ...
        const subscription = await verifyAndActivatePayment(
          redis,
          orderId,
          mpesaReceiptNumber,
        );
        ...
        try {
          ...
          logger.warn(
            "Failed to update payments table after MPESA activation",
...
export async function handleSafaricomTillCallback(req, redis, bot) {
  try {
    ...
    const TILL_SECRET =
      process.env.SAFARICOM_TILL_SECRET ||
      process.env.PAYMENT_WEBHOOK_SECRET ||
      null;
    if (TILL_SECRET) {
    ...
    if (status === "completed") {
      try {
        ...
        if (req.body.reference) {
          orderId = await redis.get(
            `payment:by_provider_ref:SAFARICOM_TILL:${req.body.reference}`,
          );
        }
        ...
        if (!orderId && transaction_id) {
          orderId = await redis.get(
            `payment:by_provider_ref:SAFARICOM_TILL:${transaction_id}`,
          );
        }

        // Require quick mapping (reference or transaction id). Do not scan.
        if (!orderId) {
          logger.warn("No quick mapping found for Till payment", {
            amount,
            transaction_id,
          });
          await alertAdmin(bot, "Safaricom Till mapping not found", {
            amount,
            transaction_id,
        ...
        if (subscription && foundData.userId) {
          await bot.sendMessage(
            foundData.userId,
            `✅ *Safaricom Till Payment Confirmed*\n\nAmount: KES ${amount}\nTransaction: ${transaction_id}\n\nYour ${subscription.tier} subscription is now active!`,
            { parse_mode: "Markdown" },
          );
        }
...
export async function handlePayPalWebhook(req, redis, bot) {
  try {
    ...
    if (PAYPAL_CLIENT_ID && PAYPAL_CLIENT_SECRET && PAYPAL_WEBHOOK_ID) {
      ...
      if (!verified) {
        logger.warn(
          "PayPal webhook verification failed (provider verification)",
        );
      ...
      if (PAYPAL_SECRET) {
        const sig =
          req.headers["x-paypal-signature"] || req.headers["x-signature"];
      ...
      if (status === "COMPLETED") {
        ...
        try {
          ...
          for (const candidate of possibleOrderIds) {
            const mapped = await redis.get(
              `payment:by_provider_ref:PAYPAL:${candidate}`,
            );
            if (mapped) {
              foundOrderId = mapped;
              break;
        ...
        if (foundOrderId) {
          try {
            subscription = await verifyAndActivatePayment(
              redis,
              foundOrderId,
              id,
            );
            ...
            logger.warn(
              "verifyAndActivatePayment failed for mapped PayPal order",
              { foundOrderId, err: e.message },
            );
            subscription = null;
          }
        }
        ...
        if (!subscription) {
          logger.warn("PayPal webhook received but no mapped order found", {
            captureId: id,
          });
          await alertAdmin(bot, "PayPal mapping not found", {
            captureId: id,
            possibleOrderIds,
          });
          return { success: false, message: "Order mapping not found" };
...
export async function handleBinanceWebhook(req, redis, bot) {
  try {
    ...
    if (data.status === "SUCCESS") {
      ...
      try {
        const mapped = await redis.get(
          `payment:by_provider_ref:BINANCE:${transactionId}`,
        );
        if (mapped) {
          try {
            subscription = await verifyAndActivatePayment(
            ...
            logger.warn(
              "verifyAndActivatePayment failed for mapped Binance order",
              e,
            );
            subscription = null;
          }
        } else {
          logger.warn("No mapping found for Binance transaction", {
            transactionId,
          });
          await alertAdmin(bot, "Binance mapping not found", {
            transactionId,
            totalFeeInUSD,
          });
          return { success: false, message: "Order mapping not found" };
...
function verifySignature(req, secret, headerValue) {
  try {
    ...
    return (
      headerValue.toLowerCase().includes(hmac.toLowerCase()) ||
      headerValue.toLowerCase() === hmac.toLowerCase()
    );
  } catch (e) {
...
async function verifyPayPalWebhook(req, clientId, clientSecret, webhookId) {
  try {
    ...
    const transmissionId =
      ...
      req.headers["paypal-transmission-id".toLowerCase()] ||
      req.headers["paypal-transmission-id".toUpperCase()];
    const transmissionTime = req.headers["paypal-transmission-time"];
    const certUrl = req.headers["paypal-cert-url"];
    const authAlgo = req.headers["paypal-auth-algo"];
    const transmissionSig =
      req.headers["paypal-transmission-sig"] ||
      req.headers["paypal-transmission-sig".toLowerCase()];
    ...
    if (!verifyResp.ok) {
      logger.warn("PayPal verify-webhook-signature failed", {
    ...
    logger.info("PayPal verify result", {
      status: verifyJson.verification_status,
    });
    return (
      verifyJson.verification_status &&
      verifyJson.verification_status.toUpperCase() === "SUCCESS"
    );
  } catch (e) {
