  ...
  registerRoutes() {
    ...
    this.router.get("/api/data/match/:matchId", async (req, res) => {
      try {
        ...
        if (source) {
          ...
          const match = await this.aggregator.dataCache.getMatchDetail(
            matchId,
            source,
          );
          ...
          const match =
            await this.aggregator.dataCache.getFullMatchData(matchId);
          res.json(match);
        }
      } catch (e) {
    ...
    this.router.get("/api/data/standings/:leagueId", async (req, res) => {
      try {
        ...
        const standings = await this.aggregator.dataCache.getStandings(
          leagueId,
          source,
        );
        res.json({
    ...
    this.router.get("/api/data/cache-info", async (req, res) => {
      try {
        ...
        const totalSize = exportedData.entries.reduce(
          (sum, e) => sum + e.size,
          0,
        );
    ...
    this.router.get("/api/data/export", async (req, res) => {
      try {
        const summary = await this.aggregator.dataCache.getDataSummary();
        const smLive =
          await this.aggregator.dataCache.getLiveMatches("sportsmonks");
        const fdLive =
          await this.aggregator.dataCache.getLiveMatches("footballdata");
        const smLeagues =
          await this.aggregator.dataCache.getLeagues("sportsmonks");
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="sports-data-${Date.now()}.json"`,
        );
        res.json(exported);
      } catch (e) {
      ...
      const sanitize = (obj) => {
        ...
        if (typeof obj === "string")
          return obj.length > 1000
            ? obj.substring(0, 1000) + "...<truncated>"
            : obj;
                  ...
                  .map((it) =>
                    typeof it === "string"
                      ? it.length > 300
                        ...
                        : it
                      : typeof it === "object"
                        ? JSON.stringify(it).substring(0, 300)
                        : it,
                  );
              } else {
                out[k] = Object.keys(v)
                  ...
                  .reduce((acc, kk) => {
                    acc[kk] =
                      typeof v[kk] === "string" && v[kk].length > 300
                        ? v[kk].substring(0, 300) + "..."
                        : v[kk];
                    return acc;
                  }, {});
              }
