...
const ODDS_TICKER_ENABLED =
  String(process.env.ODDS_TICKER_ENABLED || "false").toLowerCase() === "true";
...
export async function runOddsTickerCycle() {
  ...
  try {
    if (!aggInstance || typeof aggInstance.getUpcomingWithOdds !== "function") {
      console.warn(
        "[OddsTicker] No aggregator available or method missing; skipping cycle",
      );
      return;
    }
