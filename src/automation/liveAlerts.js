...
const LIVE_ALERTS_ENABLED =
  String(process.env.LIVE_ALERTS_ENABLED || "false").toLowerCase() === "true";
...
async function getLiveMatchesWithEvents() {
  try {
    if (
      !aggInstance ||
      typeof aggInstance.getLiveMatchesWithEvents !== "function"
    )
      return [];
  ...
  try {
    ...
    for (const match of matches) {
      for (const ev of match.newEvents || []) {
        ...
        const text = [
          `${icon} <b>${match.home} ${match.scoreHome || 0} — ${match.scoreAway || 0} ${match.away}</b>`,
          ev.minute
            ? `⏱️ ${ev.minute}' ${ev.description || ev.type}`
        ...
        try {
          if (imageUrl) {
            const caption =
              ev && ev.caption ? `${ev.caption}\n\n${text}` : text;
            const res = await broadcastPhoto(imageUrl, caption);
            if (res) {
              console.log(
                "[LiveAlerts] Posted live event with photo",
                key,
                imageUrl,
              );
