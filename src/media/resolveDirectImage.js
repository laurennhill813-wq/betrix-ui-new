...
export async function resolveDirectImage(rawUrl) {
  ...
  try {
    ...
    if (ogImage) {
      ...
      try {
        const ogRes = await fetch(new URL(ogImage, finalUrl).toString(), { redirect: 'follow' });
        const ogFinal = ogRes.url || ogImage;
        const ogCT = (ogRes.headers && ogRes.headers.get ? ogRes.headers.get('content-type') : '') || '';
        if (looksLikeDirectImage(ogFinal) || (ogCT && ogCT.startsWith('image/'))) return ogFinal;
      } catch (_) { /* ignore */ }
    }
    ...
    if (twitterImage) {
      ...
      try {
        const tRes = await fetch(new URL(twitterImage, finalUrl).toString(), { redirect: 'follow' });
        const tFinal = tRes.url || twitterImage;
        const tCT = (tRes.headers && tRes.headers.get ? tRes.headers.get('content-type') : '') || '';
        if (looksLikeDirectImage(tFinal) || (tCT && tCT.startsWith('image/'))) return tFinal;
      } catch (_) { /* ignore */ }
    }

    // First <img> that looks like a direct image
    const imgs = $('img').map((i, el) => $(el).attr('src')).get().filter(Boolean);
    for (const src of imgs) {
      try {
        ...
        try {
          const head = await fetch(candidate, { method: 'HEAD', redirect: 'follow' });
          const hCT = (head.headers && head.headers.get ? head.headers.get('content-type') : '') || '';
    ...
    try { console.warn('[Media] resolveDirectImage failed', err && err.message ? err.message : String(err)); } catch (_) { }
    return null;
  }
}
