...
const DEFAULT_PLACEHOLDER = process.env.DEFAULT_MEDIA_URL || 'https://via.placeholder.com/1024x512.png?text=BETRIX+Media';

// Default provider priority (comma-separated ENV override allowed)
const DEFAULT_PRIORITY = 'getty,reuters,ap,imagn,sportradar';
const providerPriority = (process.env.MEDIA_PROVIDER_PRIORITY || DEFAULT_PRIORITY).split(',').map(s => s.trim()).filter(Boolean);
...
async function tryProvider(providerName, fnName, ctx) {
  try {
    ...
    if (result && typeof result === 'string') return { imageUrl: result, provider: providerName };
    if (result && result.imageUrl) return { imageUrl: result.imageUrl, provider: providerName, meta: result };
    return null;
  } catch (err) {
    logger.warn(`Provider ${providerName}.${fnName} failed: ${err?.message || String(err)}`);
    return null;
  }
}
...
export async function getBestImageForEvent({ event = {}, match = {} } = {}) {
  ...
  return { imageUrl: DEFAULT_PLACEHOLDER, provider: 'placeholder', reason: 'fallback' };
}
