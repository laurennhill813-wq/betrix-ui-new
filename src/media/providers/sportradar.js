  ...
  for (const host of HOST_PATTERNS) {
    for (const tpl of PATH_TEMPLATES) {
      for (const ext of EXTENSIONS) {
        let path = tpl.replace('{REGION}', REGION).replace('{LOCALE}', LOCALE).replace('{EXT}', ext);
        if (path.includes('{id}') && teamId) path = path.replace('{id}', encodeURIComponent(teamId));
        if (path.includes('{code}') && code) path = path.replace('{code}', encodeURIComponent(code));
...
export async function getImageForEvent(ctx = {}) {
  if (!API_KEY) return null;
  try {
    const teamId = ctx.homeTeamId || ctx.awayTeamId || ctx.home || ctx.away || ctx.event?.teamId || ctx.event?.team;
    const countryCode = ctx.countryCode || ctx.event?.countryCode || ctx.match?.countryCode;

    // Try team first
    if (teamId) {
      const img = await findImageFromTemplates({ teamId, countryCode: null });
      if (img) return { imageUrl: img, provider: 'sportradar', type: 'logo', teamId };
    }

    // Try flag
    if (countryCode) {
      const img = await findImageFromTemplates({ teamId: null, countryCode });
      if (img) return { imageUrl: img, provider: 'sportradar', type: 'flag', countryCode };
    }
...
export async function getImageForMatch({ match = {} } = {}) {
  ...
  try {
    ...
    if (home) {
      const img = await findImageFromTemplates({ teamId: home, countryCode: null });
      if (img) return { imageUrl: img, provider: 'sportradar', type: 'logo', teamId: home };
    }
    if (away) {
      const img = await findImageFromTemplates({ teamId: away, countryCode: null });
      if (img) return { imageUrl: img, provider: 'sportradar', type: 'logo', teamId: away };
    }
    if (country) {
      const img = await findImageFromTemplates({ teamId: null, countryCode: country });
      if (img) return { imageUrl: img, provider: 'sportradar', type: 'flag', countryCode: country };
    }
