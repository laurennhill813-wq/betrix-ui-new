...
export async function getImageForEvent({ event = {}, match = {} } = {}) {
  ...
  try {
    ...
    return { imageUrl: placeholderFor(subject), meta: { provider: 'reuters', scaffold: true } };
  } catch (err) {
    logger.warn(`reuters.getImageForEvent error: ${err?.message || String(err)}`);
    return null;
  }
}
...
export async function getImageForMatch({ match = {} } = {}) {
  ...
  try {
    ...
    logger.warn(`reuters.getImageForMatch error: ${err?.message || String(err)}`);
    return null;
  }
}
