...
export async function sendTelegramPhoto(chatId, photoUrl, caption, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (msg.includes('wrong type of the web page content') || msg.includes('failed to get http url') || msg.includes("can't parse entities")) {
        ...
        if (imgRes.ok && ct && ct.startsWith('image/')) {
          ...
          const tmp = require('path').join(require('os').tmpdir(), `${Date.now()}-${filename}`);
          const fs = await import('fs');
          await fs.promises.writeFile(tmp, buf);
          formData.append('photo', fs.createReadStream(tmp), { filename, contentType: ct });

          const uploadUrl = `${TELEGRAM_BASE}/sendPhoto`;
          const headers = formData.getHeaders ? formData.getHeaders() : {};
          const uploadResp = await fetch(uploadUrl, { method: 'POST', body: formData, headers });
          try { await fs.promises.unlink(tmp); } catch (e) {}
          const uploadJson = await uploadResp.json().catch(() => null);
          if (!uploadJson || !uploadJson.ok) {
            console.error('[Telegram] sendPhoto upload failed', uploadResp.status, uploadJson);
            return null;
          }
          return uploadJson;
        }
      ...
      try { (await import('../brain/telemetry.js')).default.incCounter('upload_fallback_failures'); } catch(e){}
    }
