...
function writeLocalLog(line) {
  try {
    ...
    console.error("writeLocalLog failed", e);
  }
}
...
export async function betrixIngest(payload) {
  const record = {
    ...
    provider_event_id:
      (payload.data && (payload.data.id || payload.data.transaction_id)) ||
      null,
  ...
  if (!supabase) {
    writeLocalLog(
      `${new Date().toISOString()} NO_SUPABASE ${JSON.stringify(record)}`,
    );
    return null;
  }
  ...
  try {
    ...
    if (payErr) {
      ...
      writeLocalLog(
        `${new Date().toISOString()} PAYMENTS_INSERT_ERROR ${JSON.stringify(payErr)}`,
      );
      throw payErr;
    }
    return payData;
    ...
    try {
      ...
      if (error) {
        writeLocalLog(
          `${new Date().toISOString()} WEBHOOKS_INSERT_ERROR ${JSON.stringify(error)}`,
        );
        throw error;
      }
      return data;
      ...
      writeLocalLog(
        `${new Date().toISOString()} INGEST_FAILED ${JSON.stringify(err2)}`,
      );
