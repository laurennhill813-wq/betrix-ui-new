// Minimal, single-definition RapidAPI fetcher module.
// Exports: `RapidApiFetcher` and `normalizeRedisKeyPart`.
export class RapidApiFetcher {
  ...
  _buildHeaders(host) {
    ...
    try {
      ...
      // If calling The Odds API directly, prefer its `x-api-key` header.
      if (hostNorm.includes("the-odds-api.com") || hostNorm.includes("api.the-odds-api.com")) {
        ...
        if (theOddsKey) headers["x-api-key"] = theOddsKey;
      }
    } catch (e) {
      // swallow
    }
  ...
  async fetchRapidApi(host, endpoint, opts = {}) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        if (timeoutHandle) clearTimeout(timeoutHandle);
        try {
          console.warn("[rapidapi-fetcher-error]", err && err.message ? err.message : String(err));
        } catch (e) {}
