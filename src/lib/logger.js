...
module.exports = {
  info: (obj) => {
    try {
      console.log(
        JSON.stringify(
          Object.assign({ level: "info", ts: new Date().toISOString() }, obj),
        ),
      );
    } catch (e) {
      console.log("log-err", e && e.message);
    }
  },
  error: (obj) => {
    try {
      console.error(
        JSON.stringify(
          Object.assign({ level: "error", ts: new Date().toISOString() }, obj),
        ),
      );
    } catch (e) {
      console.error("log-err", e && e.message);
    }
  },
  inc: (metric) => {
