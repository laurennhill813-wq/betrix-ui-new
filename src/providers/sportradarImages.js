...
export async function fetchSportradarEventAssets(eventId) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Sportradar fetch failed ${res.status}: ${txt.slice ? txt.slice(0,200) : txt}`);
  }
  ...
  const urls = Array.isArray(assets) ? assets.map(a => (typeof a === 'string' ? a : (a && (a.url || a.href || a.path) || null))).filter(Boolean) : [];
