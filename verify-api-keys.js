...
console.log(
  `  SportMonks:    ${status.sportmonks.configured ? "✅ CONFIGURED" : "❌ NOT CONFIGURED"}`,
);
console.log(
  `  Football-Data: ${status.footballdata.configured ? "✅ CONFIGURED" : "❌ NOT CONFIGURED"}`,
);
...
if (!status.sportmonks.configured && !status.footballdata.configured) {
  ...
  console.error(
    "Please set SPORTSMONKS_API and FOOTBALL_DATA_API environment variables.",
  );
  process.exit(1);
}
