...
export async function signSportradarAsset(url) {
  ...
  if (!response.ok) throw new Error(`Sportradar rejected request: ${response.status}`);

  const arr = await response.arrayBuffer();
  const buffer = Buffer.from(arr);

  if (buffer.length > config.limits.maxAssetBytes) throw new Error('Asset exceeds MAX_ASSET_MB');

  const ext = guessExt(response.headers.get('content-type')) || 'jpg';
  const key = `sportradar/${crypto.randomUUID()}.${ext}`;

  await uploadTempBuffer(buffer, key, response.headers.get('content-type') || 'application/octet-stream');
