...
assert(
  normalizePaymentMethod("MPESA") === "MPESA",
  "MPESA should normalize to MPESA",
);
assert(
  normalizePaymentMethod("PAYPAL") === "PAYPAL",
  "PAYPAL should normalize to PAYPAL",
);
assert(
  normalizePaymentMethod("SAFARICOM_TILL") === "SAFARICOM_TILL",
  "SAFARICOM_TILL should normalize to SAFARICOM_TILL",
);
assert(
  normalizePaymentMethod("BINANCE") === "BINANCE",
  "BINANCE should normalize to BINANCE",
);
assert(
  normalizePaymentMethod("SWIFT") === "SWIFT",
  "SWIFT should normalize to SWIFT",
);
assert(
  normalizePaymentMethod("BITCOIN") === "BITCOIN",
  "BITCOIN should normalize to BITCOIN",
);
...
assert(
  normalizePaymentMethod("safaricom") === "SAFARICOM_TILL",
...
assert(
  normalizePaymentMethod("till") === "SAFARICOM_TILL",
  "till -> SAFARICOM_TILL",
);
assert(
  normalizePaymentMethod("TILL") === "SAFARICOM_TILL",
...
assert(
  normalizePaymentMethod("binance_pay") === "BINANCE",
  "binance_pay -> BINANCE",
);
assert(
  normalizePaymentMethod("BINANCE_PAY") === "BINANCE",
...
assert(
  normalizePaymentMethod("bank_transfer") === "SWIFT",
  "bank_transfer -> SWIFT",
);
assert(
  normalizePaymentMethod("BANK_TRANSFER") === "SWIFT",
...
assert(normalizePaymentMethod("BTC") === "BITCOIN", "BTC -> BITCOIN");
assert(
  normalizePaymentMethod("eth") === "BITCOIN",
  "eth -> BITCOIN (maps to BITCOIN)",
);
...
assert(
  normalizePaymentMethod("\tpaypal\n") === "PAYPAL",
  "Tabs/newlines should be trimmed",
);
...
assert(
  normalizePaymentMethod("unknown_method") === null,
  "Unknown method should return null",
);
assert(
  normalizePaymentMethod("fake_provider") === null,
  "Fake provider should return null",
);
...
assert(
  normalizePaymentMethod(undefined) === null,
  "undefined input should return null",
);
...
assert(
  normalizePaymentMethod("SaFaRiCoM_TiLl") === "SAFARICOM_TILL",
  "Mixed case SaFaRiCoM_TiLl -> SAFARICOM_TILL",
);
...
for (const [key, _] of Object.entries(PAYMENT_PROVIDERS)) {
  ...
  assert(
    normalized === key,
    `${key.toLowerCase()} should normalize to ${key}, got ${normalized}`,
  );
}
