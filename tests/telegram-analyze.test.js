...
test('handleAnalyzeMatch: basic upcoming fixture analysis', async () => {
  ...
  const mockAgg = {
    ...
    getHeadToHead: async () => ({ totalMatches: 0, homeWins: 0, awayWins: 0, draws: 0 }),
    getRecentForm: async () => [{ starting_at: '2025-11-01', result: '2-1', score: '2-1' }],
    getStandings: async () => ([
  ...
  assert.ok(/Match Analysis|BETRIX/i.test(res.text), 'expected analysis header to be present');
  // Either editMessageText (when short) or sendMessage when too long/truncated
  assert.ok(res.method === 'editMessageText' || res.method === 'sendMessage', 'unexpected telegram method');
});
