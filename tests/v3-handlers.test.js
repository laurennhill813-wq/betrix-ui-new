...
test("v3 - commands-v3.js loads and exports", async () => {
  const { handleCommand, handleStart } =
    await import("../src/handlers/commands-v3.js");
  assert(
    typeof handleCommand === "function",
    "handleCommand should be a function",
  );
...
test("v3 - handleStart returns welcome message", async () => {
  ...
  assert(
    result.text.includes("Welcome to BETRIX"),
    "should include welcome text",
  );
...
test("v3 - handleMenu returns main menu", async () => {
  ...
  assert(
    result.reply_markup.inline_keyboard.length === 4,
    "should have 4 rows",
  );
});
...
test("v3 - message-handler-v3.js loads and exports", async () => {
  const { handleMessage, classifyIntent } =
    await import("../src/handlers/message-handler-v3.js");
  assert(
    typeof handleMessage === "function",
    "handleMessage should be a function",
  );
  assert(
    typeof classifyIntent === "function",
    "classifyIntent should be a function",
  );
});
...
test("v3 - classifyIntent detects signup intent", async () => {
  ...
  assert(
    classifyIntent("I want to join") === "signup",
    'should detect "I want to join"',
  );
...
test("v3 - callbacks-v3.js loads and exports", async () => {
  const { handleCallbackQuery } =
    await import("../src/handlers/callbacks-v3.js");
  assert(
    typeof handleCallbackQuery === "function",
    "handleCallbackQuery should be a function",
  );
});
...
test("v3 - betting-sites.js loads and exports", async () => {
  const { getBettingSitesForCountry, formatBettingSites } =
    await import("../src/handlers/betting-sites.js");
  assert(
    typeof getBettingSitesForCountry === "function",
...
test("v3 - getBettingSitesForCountry returns Kenya sites", async () => {
  const { getBettingSitesForCountry } =
    await import("../src/handlers/betting-sites.js");
  ...
  assert(
    sites.some((s) => s.id === "betika"),
    "should include Betika",
  );
  assert(
    sites.some((s) => s.id === "sportpesa"),
    "should include SportPesa",
  );
});

test("v3 - data-models.js loads and exports", async () => {
  const { createUserProfile, getUserProfile, createPaymentRecord } =
    await import("../src/handlers/data-models.js");
  assert(
    typeof createUserProfile === "function",
    "should export createUserProfile",
  );
  assert(typeof getUserProfile === "function", "should export getUserProfile");
  assert(
    typeof createPaymentRecord === "function",
    "should export createPaymentRecord",
  );
});
...
test("v3 - calculateVVIPExpiry sets correct dates", async () => {
  const { calculateVVIPExpiry } =
    await import("../src/handlers/data-models.js");
  const now = Date.now();

  const daily = new Date(calculateVVIPExpiry("daily")).getTime();
  assert(
    daily > now && daily < now + 24 * 60 * 60 * 1000 + 1000,
    "daily should be ~24h",
  );

  const weekly = new Date(calculateVVIPExpiry("weekly")).getTime();
  assert(
    weekly > now && weekly < now + 7 * 24 * 60 * 60 * 1000 + 1000,
    "weekly should be ~7d",
  );

  const monthly = new Date(calculateVVIPExpiry("monthly")).getTime();
  assert(
    monthly > now && monthly < now + 30 * 24 * 60 * 60 * 1000 + 1000,
    "monthly should be ~30d",
  );
});
