...
function loadAzureAIServiceWithFetch(mockFetch) {
  ...
  src = src.replace(
    /import\s+fetch\s+from\s+['"]node-fetch['"];?/,
  ...
  try {
    ...
    console.error(
      "VM execution failed while loading AzureAI service:",
      vmErr && vmErr.message,
    );
    try {
      const cp = require("child_process");
      // Run the ESM test file with the current node executable
      cp.execFileSync(process.execPath, ["tests/azure-ai.smoke.node.js"], {
        stdio: "inherit",
      });
