...
test('handleAnalyzeMatch: prefers Azure AI output when available', async () => {
  ...
  const mockAgg = {
    ...
    getFixtures: async () => ([{ id: 537950, home: 'Leeds United FC', away: 'Crystal Palace FC', status: 'TIMED', time: '2025-12-21T17:00:00Z', competition: 'Premier League', venue: 'Elland Road', raw: { competition: { id: 2 } } }]),
    getHeadToHead: async () => ({ totalMatches: 2, homeWins: 1, awayWins: 1, draws: 0 }),
    getRecentForm: async () => ([{ starting_at: '2025-11-01', result: '1-0', score: '1-0' }]),
    getStandings: async () => ([]),
    getOdds: async () => []
  };
  ...
  const mockAzure = {
    ...
    chat: async (prompt, opts) => {
      ...
      if (!/Leeds United FC/.test(prompt) || !/Crystal Palace FC/.test(prompt)) return '';
  ...
  const res = await handleAnalyzeMatch('analyze_match_upcoming_537950', 11111, 22222, redis, services);

  assert.ok(res && typeof res === 'object', 'expected object response');
  assert.ok(res.text && res.text.includes('AI GENERATED ANALYSIS'), 'expected AI text from azure mock');
});
