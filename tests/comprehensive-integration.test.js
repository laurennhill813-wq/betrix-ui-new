...
test("Command handlers - /start shows welcome message with buttons", async () => {
  ...
  assert(
    result.reply_markup.inline_keyboard.length > 0,
...
test("Command handlers - /help returns comprehensive FAQs", async () => {
  ...
  assert(
    result.text.includes("Help") || result.text.includes("FAQ"),
...
test("Betting sites - Kenya sites available with all info", async () => {
  ...
  console.log(
    `✅ Kenya betting sites: ${sites.length} sites with complete info`,
  );
});
...
test("UI/UX - VVIP response with tier info", async () => {
  ...
  assert(
    result.text.includes("VVIP") || result.text.includes("Premium"),
    "Missing tier info",
  );
...
test("UI/UX - Analysis response has structure", async () => {
  ...
  assert(
    ...
    "Missing action buttons when no ID provided",
  );
...
test("Data models - currency formatting (KES)", () => {
  ...
  assert(
    formatted.includes("KES") || formatted.includes("Sh"),
    "Missing currency symbol",
  );
  assert(
    formatted.includes("1") && formatted.includes("500"),
    "Wrong amount formatted",
  );
...
test("Edge cases - handling missing services gracefully", async () => {
  ...
  assert(
    result.text.includes("Failed") || result.text.includes("not found"),
    ...
    if (response.reply_markup) {
      ...
      response.reply_markup.inline_keyboard.forEach((row) => {
        ...
        row.forEach((btn) => {
          ...
          assert(
            btn.callback_data || btn.url,
            `${cmd}: button missing callback or URL`,
          );
        });
      });
    }
