...
async function run() {
  ...
  assert(
    res && (res.method === "editMessageText" || res.method === "sendMessage"),
  );
  ...
  const fakeMessageUpdate = {
    message: { chat: { id: 9999 }, from: { id: 424242 }, text: "Arsenal" },
  };
  ...
  assert(
    Array.isArray(stored.favorites) && stored.favorites.includes("Arsenal"),
  );
