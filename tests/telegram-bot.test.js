...
async function runTests() {
  ...
  try {
    ...
    const start = await handleCommand(
      "/start",
      chatId,
      userId,
      redis,
    ...
    assert(
      start && (start.chat_id === chatId || start.chatId === chatId),
      "/start should return chat id",
    );

    const menu = await handleCommand(
      "/menu",
      chatId,
      userId,
      redis,
    ...
    assert(
      menu && menu.reply_markup && menu.reply_markup.inline_keyboard,
      "/menu should include inline keyboard",
    );

    const help = await handleCommand(
      "/help",
      chatId,
      userId,
      redis,
    ...
    const menuCb = await handleCallback(
      "menu_main",
      chatId,
      userId,
      redis,
    ...
    assert(
      menuCb && menuCb.reply_markup,
      "menu_main callback should return reply_markup",
    );
