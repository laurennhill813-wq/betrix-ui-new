...
import {
  getNewsHeadlines,
  getRedditHeadlines,
} from "./src/services/news-provider.js";
...
async function run() {
  ...
  try {
    ...
    const sbResult = await scorebat
      .freeFeed()
      .catch((e) => ({ error: e.message }));
    console.log(
      "ScoreBat result:",
      Array.isArray(sbResult) ? `items=${sbResult.length}` : sbResult,
    );
  } catch (e) {
    console.error("ScoreBat test failed", e);
  }

  try {
    console.log("Fetching live matches from aggregator (league 39)...");
    const matches = await agg
      .getLiveMatches(39)
      .catch((e) => ({ error: e.message }));
    console.log(
      "Aggregator live matches:",
      Array.isArray(matches) ? `matches=${matches.length}` : matches,
    );
    if (Array.isArray(matches) && matches.length > 0)
      console.dir(matches.slice(0, 3), { depth: 2 });
  } catch (e) {
    console.error("Aggregator live test failed", e);
  }

  try {
    console.log('Fetching Google News RSS for "football"...');
    const news = await getNewsHeadlines({ query: "football", max: 5 }).catch(
      (e) => ({ error: e.message }),
    );
