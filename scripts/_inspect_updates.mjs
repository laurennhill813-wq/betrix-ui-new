...
(async ()=>{
  try{
    ...
    items.forEach((it, i)=>{
      try{ console.log(i, JSON.parse(it)); } catch(e){ console.log(i, it); }
    });
    const processing = await r.lrange('telegram:processing', 0, 9);
    console.log('telegram:processing (first 10):', processing.length);
    processing.forEach((it,i)=>{ try{ console.log('proc',i,JSON.parse(it)); } catch(e){ console.log('proc',i,it); } });
  }catch(e){ console.error('ERR',e); }
  finally{ await r.quit(); process.exit(0); }
})();
