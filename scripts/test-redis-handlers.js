...
try {
  ...
  await redis.del(
    `payment:${orderId}`,
    `payment:by_user:${userId}:pending`,
    userKey,
  );
} catch (error) {
...
try {
  ...
  const topCommands = await redis.zrevrange(
    `cmd:monthly:${currentMonth}`,
    0,
    4,
    "WITHSCORES",
  );
...
try {
  ...
  const topPredictors = await redis.zrevrange(
    "predictions:accuracy",
    0,
    2,
    "WITHSCORES",
