...
class MockRedis {
  ...
  async hget(hash, field) {
    ...
    return h[field] === undefined ? null : h[field];
  }
  ...
  async lpush(key, val) {
    const arr = this.store.get(key) ? JSON.parse(this.store.get(key)) : [];
    arr.unshift(val);
...
async function run() {
  try {
    ...
    const { handleMessage, handleCallbackQuery } =
      await import("../src/handlers/telegram-handler-v2.js");
    const redisClient = new MockRedis(); // use mock directly in e2e
    const mockUpdate = {
      message: { chat: { id: 9999 }, from: { id: 424242 }, text: "/live" },
    };
    console.log("--- Running /live simulation ---");
    const res = await handleMessage(mockUpdate, redisClient, {
      apiFootball: {
        getLive: async () => ({
          response: [
            {
              fixture: { id: 11111, status: { short: "LIVE", elapsed: 12 } },
              teams: {
                home: { name: "Home FC" },
    ...
    const cb = {
      ...
      data: "bet_fixture_11111",
    };
    const cbRes = await handleCallbackQuery(cb, redisClient, {
      apiFootball: {
        getFixture: async (id) => ({
          response: [
            {
    ...
    try {
      ...
      const instr = await payment.getPaymentInstructions(
        redisClient,
        order.orderId,
        order.paymentMethod,
      );
      console.log(
        ...
        instr && instr.description ? instr.description : instr,
      );
      const result = await payment.simulatePaymentComplete(
        redisClient,
        order.orderId,
      );
