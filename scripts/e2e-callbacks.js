...
class MockRedis {
  ...
  async rpush(k, v) {
    const arr = this.store.get(k) ? JSON.parse(this.store.get(k)) : [];
    arr.push(v);
    this.store.set(k, JSON.stringify(arr));
    return arr.length;
  }
}
...
async function run() {
  ...
  const callbacks = [
    ...
    },
    {
      id: "cb-sport",
      from: { id: 424242 },
      message: { chat: { id: 9999, message_id: 3 } },
      data: "sport_football",
    },
    {
      id: "cb-league",
      from: { id: 424242 },
      message: { chat: { id: 9999, message_id: 4 } },
      data: "league_39",
    },
    {
      id: "cb-standings",
      ...
      data: "league_standings_39",
    },
    {
      id: "cb-odds",
      from: { id: 424242 },
      message: { chat: { id: 9999, message_id: 6 } },
      data: "menu_odds",
    },
  ];
...
run().catch((e) => {
  console.error("e2e-callbacks error", e);
  process.exit(1);
});
