...
if (!BOT_TOKEN || !TEST_CHAT) {
  console.error(
    "Please set TELEGAM_BOT_TOKEN (or TELEGRAM_BOT_TOKEN) and TEST_CHAT_ID (or BOT_BROADCAST_CHAT_ID)",
  );
...
(async function run() {
  try {
    ...
    await sendPhotoWithCaption({
      chatId: TEST_CHAT,
      photoUrl: good,
      caption: "Test image — good URL",
    });
    console.log("Good URL send attempted (check chat).");

    console.log(
      "Sending test: bad image URL (expected to trigger upload fallback when Telegram rejects)",
    );
    const bad = process.env.BAD_URL || "https://example.com/";
    await sendPhotoWithCaption({
      chatId: TEST_CHAT,
      photoUrl: bad,
      caption: "Test image — bad URL (should attempt upload fallback)",
    });
