...
async function main() {
  ...
  const endpoint = process.env.SPORTRADAR_SIGN_ENDPOINT || (await ask('Signing endpoint (SPORTRADAR_SIGN_ENDPOINT): '));
  if (!endpoint) {
    console.error('No signing endpoint provided. Set SPORTRADAR_SIGN_ENDPOINT or enter it now.');
    process.exit(2);
  }
  ...
  try {
    const res = await fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ url: asset }) });
    ...
    const r2 = await fetch(signed, { redirect: 'follow', headers: { 'User-Agent': 'BetrixBot/1.0 (+https://betrix.example)' } });
    console.log('Fetch status:', r2.status, r2.statusText);
    if (r2.ok) {
      console.log('Signed URL appears fetchable. Content-Type:', r2.headers.get('content-type'));
    } else {
