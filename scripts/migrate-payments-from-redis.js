...
async function main() {
  ...
  for (const k of keys) {
    try {
      ...
      if (!raw) continue;
      let obj = null;
      try { obj = JSON.parse(raw); } catch (e) { console.warn('Skipping non-JSON payment log', k); continue; }
      // Map to payment shape expected by createPaymentRecord
      const payment = {
        userId: obj.user_id || obj.userId || null,
        provider: obj.provider || null,
        providerRef: obj.provider_ref || obj.providerRef || obj.provider_ref || null,
      ...
      if (dry) {
        console.log('[DRY RUN] Would migrate', k, '->', payment.providerRef || '(no provider_ref)', 'amount:', payment.totalAmount);
      } else {
        const inserted = await createPaymentRecord(dbClient, r, payment);
        console.log('Migrated', k, '->', inserted && (inserted.id || inserted._redis_key || inserted), 'OK');
      }
  ...
  const byKeys = (await (r.keys ? r.keys('payment:by_provider_ref:*') : [])) || [];
  ...
  for (const bk of byKeys) {
    try {
      ...
      if (typeof parsed === 'string' && parsed.startsWith('payments:log:')) {
        ...
        if (!logRaw) continue;
        let obj = null;
        try { obj = JSON.parse(logRaw); } catch (e) { continue; }
        const payment = {
          userId: obj.user_id || obj.userId || null,
          provider: obj.provider || null,
          providerRef: obj.provider_ref || obj.providerRef || obj.provider_ref || null,
        ...
        if (dry) {
          console.log('[DRY RUN] Would migrate mapping', bk, '->', payment.providerRef || '(no provider_ref)');
        } else {
          const inserted = await createPaymentRecord(dbClient, r, payment);
          console.log('Migrated mapping', bk, '->', inserted && (inserted.id || inserted._redis_key || inserted));
        }
        // Optionally update mapping to point to id
        // await r.setex(bk, 30*24*60*60, JSON.stringify({ id: inserted.id }));
      }
    } catch (e) {
      console.error('Failed migrating mapping', bk, e && e.message ? e.message : e);
    }
  }
...
main().catch(err => { console.error('Migration error', err); process.exit(2); });
