#!/usr/bin/env node
/*
CommonJS variant of the Sportradar probe script for projects with "type":"module".
Usage:
  SPORTRADAR_KEY=yourkey node scripts/probe-sportradar.cjs [YYYY-MM-DD]
*/
const fs = require("fs");
...
async function run() {
  ...
  for (const [name, items] of Object.entries(out.probes)) {
    ...
    items.forEach((it) => {
      const s =
        it.result && it.result.status
          ? `${it.result.status}`
          : (it.result && it.result.error) || "unknown";
