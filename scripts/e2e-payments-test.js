...
class MockRedis {
  ...
  async hget(hash, field) {
    return (this.hashes.get(hash) || new Map()).get(field) || null;
  }
  async hgetall(hash) {
  ...
  async hdel(hash, field) {
    return this.hashes.get(hash)?.delete(field) ? 1 : 0;
  }
  async lpush(key, val) {
  ...
  async lrange(key, start, end) {
    return (this.store.get(key) || []).slice(start, end + 1);
  }
...
async function runPaymentTest() {
  ...
  if (tierResponse && tierResponse.reply_markup) {
    ...
    console.log(
      `✓ Payment methods shown: ${paymentButtons.map((row) => row[0].text).join(", ")}`,
    );
  ...
  try {
    ...
    if (paymentResponse && paymentResponse.text) {
      ...
      console.log(
        `  Response text preview: ${paymentResponse.text.substring(0, 80)}...`,
      );
    } else if (
      paymentResponse &&
      paymentResponse.method === "answerCallbackQuery" &&
  ...
  try {
    ...
    if (invalidResponse && invalidResponse.method === "answerCallbackQuery") {
      console.log(
        `✓ Gracefully handled invalid method: ${invalidResponse.text}`,
      );
    } else {
      console.log(
        "❌ Should have returned answerCallbackQuery with error",
        invalidResponse,
      );
    }
  } catch (err) {
