...
(async ()=>{
  ...
  for (const i of imgs) {
    try {
      ...
      const res = await fetch(i.url, { method: 'GET', redirect: 'manual', headers: { 'User-Agent': 'BetrixBot/1.0' }, timeout: 15000 });
      console.log(' -> status', res.status, 'ok?', res.ok, 'ct=', res.headers.get('content-type'));
    } catch (e) {
