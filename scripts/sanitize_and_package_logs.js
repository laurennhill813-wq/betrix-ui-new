...
function redactPhone(value) {
  ...
  if (phoneRegex.test(value))
    return value.replace(phoneRegex, "<REDACTED_MSISDN>");
  return value;
}
  ...
  if (typeof obj === "object") {
    ...
    for (const k of Object.keys(obj)) {
      ...
      if (typeof v === "string") {
        ...
        out[k] =
          redactUserId(k, v) === "<REDACTED_USER_ID>"
            ? "<REDACTED_USER_ID>"
            : v;
      } else {
        out[k] = deepRedact(v);
...
function sanitizeText(content) {
  ...
  let out = content.replace(
    /(\+?2547\d{8}|2547\d{8}|0?7\d{8}|\+?\d{8,15})/g,
    "<REDACTED_MSISDN>",
  );
  ...
  out = out.replace(
    /(postgres(?:ql)?:\/\/[^\s"']+|mysql:\/\/[^\s"']+|mongodb(?:\+srv)?:\/\/[^\s"']+|redis:\/\/[^\s"']+|mssql:\/\/[^\s"']+)/gi,
    "<REDACTED_DSN>",
  );
  // redact API keys-like tokens (very coarse)
  out = out.replace(/([A-Za-z0-9_-]{20,})/g, (m) => {
    if (m.length > 40) return "<REDACTED_SECRET>";
    return m;
  });
  return out;
}
...
async function main() {
      ...
      if (base.endsWith(".json")) {
        ...
        if (parsed) {
          ...
          const outPath = path.join(
            dest,
            base.replace(".json", ".sanitized.json"),
          );
  ...
  if (created.length === 0) {
    console.log(
      "No artifacts found to sanitize. Looked for:",
      sources.join(", "),
    );
    return;
  }
  ...
  console.log(
    "Compress-Archive -Path .\\artifacts\\sanitized\\* -DestinationPath .\\artifacts\\lipana_artifacts_YYYYMMDD_HHMMSS.zip",
  );
}
