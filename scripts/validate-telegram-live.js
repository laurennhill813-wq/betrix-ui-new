...
try {
  ...
  console.log(
    `✅ SportMonks API responded with ${matches.length} live matches\n`,
  );

  if (matches.length === 0) {
    console.warn(
      "⚠️  No live matches found. This is normal if there are no ongoing matches.\n",
    );
  } else {
    // Show sample matches
    ...
    matches.slice(0, 3).forEach((match, idx) => {
      const home =
        match.teams?.find((t) => t.pivot?.role === "home")?.name || "Unknown";
      const away =
        match.teams?.find((t) => t.pivot?.role === "away")?.name || "Unknown";
  ...
  if (err.code === "ENOTFOUND") {
    console.error(
      "   Network issue or DNS failure. Check your internet connection.",
    );
  } else if (err.code === "UNABLE_TO_VERIFY_LEAF_SIGNATURE") {
    console.error(
      "   TLS certificate verification failed. Install proxy CA or allowlist api.sportmonks.com",
    );
    console.error("   Or temporarily set: export SPORTSMONKS_INSECURE=true");
  } else if (err.response?.status === 401) {
    console.error(
      "   SportMonks API returned 401. Check SPORTSMONKS_API token.",
    );
  }
  console.error();
  process.exit(1);
...
// ============================================================================
console.log(
  "📋 Step 4: Testing handler response (simulating /live command)...",
);
...
try {
  ...
  console.warn(
    "   Skipping handler test. Verify handler file exists at src/handlers/telegram-handler-v2-clean.js\n",
  );
  handler = null;
}
...
if (handler && handler.handleLive) {
  try {
    ...
    const result = await handler.handleLive(
      TEST_CHAT_ID,
      TEST_MESSAGE_ID,
      botContext,
    );
    ...
    if (result) {
      ...
      console.log(
        `   Text (first 100 chars): ${result.text?.substring(0, 100)}...`,
      );
      ...
      if (result.text && !result.text.includes("Unknown")) {
        ...
        result.text?.includes("No matches") ||
        result.text?.includes("unavailable")
      ) {
        console.log(
          '⚠️  Handler returned "no matches" (normal if no live matches)',
        );
      } else {
