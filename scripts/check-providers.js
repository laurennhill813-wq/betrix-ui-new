...
async function checkPayPal() {
  ...
  if (!id || !secret)
    return { ok: false, reason: "PAYPAL_CLIENT_ID/SECRET missing" };
  const base =
    mode === "live"
  ...
  try {
    ...
    if (json.error) {
      return {
        ok: false,
        reason: `${json.error} - ${json.error_description || ""}`.trim(),
      };
    }
    return {
      ok: !!json.access_token,
      info: json.access_token
        ? "token_acquired"
        : `no_token: ${JSON.stringify(json).substring(0, 100)}`,
    };
  } catch (err) {
...
async function checkApiFootball() {
  ...
  if (!key)
    return { ok: false, reason: "API_FOOTBALL_KEY/API_SPORTS_KEY missing" };
  try {
...
async function checkSportsData() {
  ...
  try {
    ...
    return {
      ok:
        res && (res.status === 200 || res.status === 401 || res.status === 403),
      status: res ? res.status : "no_resp",
    };
  } catch (err) {
    return { ok: false, reason: err.message };
  }
...
async function runChecks() {
  ...
  console.log(
    ...
    results.redis.ok
      ? "OK"
      : `FAIL - ${results.redis.reason || results.redis.status}`,
  );
  await wait(200);
  ...
  console.log(
    ...
    results.telegram.ok
      ? "OK"
      : `FAIL - ${results.telegram.reason || "unknown"}`,
  );
  await wait(200);
  ...
  console.log(
    "PAYPAL:",
    results.paypal.ok
      ? `OK (${results.paypal.info || "token"})`
      : `FAIL - ${results.paypal.reason}`,
  );
  await wait(200);
  results.apiFootball = await checkApiFootball();
  console.log(
    "API_FOOTBALL:",
    results.apiFootball.ok
      ? `OK (status ${results.apiFootball.status})`
      : `FAIL - ${results.apiFootball.reason}`,
  );
  await wait(200);
  results.allSports = await checkAllSports();
  console.log(
    "ALLSPORTS (RapidAPI):",
    results.allSports.ok
      ? `OK (status ${results.allSports.status})`
      : `FAIL - ${results.allSports.reason}`,
  );
  await wait(200);
  results.sportsData = await checkSportsData();
  console.log(
    "SPORTSDATA:",
    results.sportsData.ok
      ? `OK (status ${results.sportsData.status})`
      : `FAIL - ${results.sportsData.reason}`,
  );
