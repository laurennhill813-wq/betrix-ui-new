...
async function getPendingPayments() {
  ...
  try {
    const res = await client.query(
      "SELECT id, tx_ref, metadata->>'provider_checkout_id' AS provider_checkout_id FROM payments WHERE status = 'pending' ORDER BY created_at ASC LIMIT 50",
    );
...
async function updatePaymentStatusInDb(
  client,
  tx_ref,
  status,
  tx_id,
  ...
  for (const p of rows) {
    ...
    try {
      ...
      if (
        ...
        const updated = await updatePaymentStatusInDb(
          client,
          ref,
          status.toLowerCase(),
          tx_id,
