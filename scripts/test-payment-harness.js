...
(async () => {
  ...
  if (!redisUrl) {
    ...
    console.error(
      "Create a `.env` file in the repo root (copy `.env.example`) and add your Redis connection string as:",
    );
  ...
  try {
    ...
    if (order.providerRef) {
      const byRef = await redis.get(
        `payment:by_provider_ref:${paymentMethod}:${order.providerRef}`,
      );
    ...
    if (e && e.message && e.message.includes("NOAUTH")) {
      ...
      console.error(
        "Double-check the `REDIS_URL` in your environment or `.env` file and ensure it includes the password.",
      );
    }
