...
import { getUpcomingFixtures, formatMatchShort, setAggregator } from '../src/bot/football.js';
...
(async function(){
  ...
  console.log('\n=== /odds football (NFL) ===');
  const fakeCtxOdds = { from: { id: 9999 }, reply: (msg, opts) => { console.log(msg); return Promise.resolve(); } };
  ...
  const ps = spawn(process.execPath, ['./scripts/prefetch-status-local.mjs'], { stdio: 'inherit' });
