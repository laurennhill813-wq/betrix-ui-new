...
const service =
  serviceArgIndex >= 0 && args[serviceArgIndex + 1]
    ? args[serviceArgIndex + 1]
    : "web";
...
async function checkRedis() {
  ...
  if (!connection) {
    fatal(
      "Redis connection could not be established by the canonical factory. Check REDIS_URL and ensure provider is BullMQ-compatible.",
    );
  }
  try {
    ...
    fatal(
      "Redis PING failed: " + (err && err.message ? err.message : String(err)),
    );
  }
}
...
(async function main() {
  if (process.env.NODE_ENV !== "production") {
    console.warn(
      "[start-with-health] NODE_ENV is not production. This wrapper is intended for Render production start.",
    );
  }
  await checkRedis();
  if (service === "worker") {
