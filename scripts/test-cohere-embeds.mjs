    ...
    if (j) {
      if (Array.isArray(j.embeddings)) {
        ...
        if (emb && Array.isArray(emb.embedding))
          return { ok: true, dim: emb.embedding.length };
      }
      if (Array.isArray(j.data)) {
        const d0 = j.data[0];
        if (d0 && Array.isArray(d0.embedding))
          return { ok: true, dim: d0.embedding.length };
        if (d0 && Array.isArray(d0.embeddings))
          return { ok: true, dim: d0.embeddings[0].length };
      }
      if (j.embedding && Array.isArray(j.embedding))
        return { ok: true, dim: j.embedding.length };
    }
  ...
  for (const model of models) {
    for (const endpoint of endpoints) {
      process.stdout.write(
        `Trying Cohere endpoint ${endpoint} model ${model}... `,
      );
