...
if (!BOT_TOKEN || !CHAT || !URL) {
  console.error('Missing TELEGRAM_BOT_TOKEN, TEST_CHAT_ID/BOT_BROADCAST_CHAT_ID, or TEST_IMAGE_URL env vars');
  process.exit(1);
}
...
(async ()=>{
  try {
    ...
    if (!r.ok) { console.error('Failed to download', r.status); process.exit(1); }
    ...
    form.append('photo', fs.createReadStream(tmp), { filename: path.basename(tmp), contentType: ct });
    form.append('caption', 'Direct multipart upload test');
    const headers = form.getHeaders();
    console.log('Uploading to Telegram...');
    const res = await fetch(url, { method: 'POST', body: form, headers });
    const text = await res.text().catch(()=>'');
    console.log('Telegram response', res.status, text.slice ? text.slice(0,400) : text);
