...
(async () => {
  try {
    const svc = new AzureAIService(process.env.AZURE_OPENAI_ENDPOINT, process.env.AZURE_OPENAI_KEY, process.env.AZURE_OPENAI_DEPLOYMENT, process.env.AZURE_OPENAI_API_VERSION);
    ...
    console.log('Embeddings returned', v.length, 'vectors. First vector length:', Array.isArray(v[0]) ? v[0].length : typeof v[0]);
    process.exit(0);
  } catch (err) {
