  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(liveMatches) && liveMatches.length > 0) {
        logger.info(
          `✓ Retrieved ${liveMatches.length} live matches from SportMonks`,
        );
        logger.info("Sample match (first 3):");
        liveMatches.slice(0, 3).forEach((m, i) => {
          logger.info(
            `  [${i + 1}] ${m.name || m.home} vs ${m.away || "?"} (ID: ${m.id})`,
          );
        });
      } else {
        logger.warn(
          "⚠ SportMonks returned 0 matches (likely TLS cert issue on this network)",
        );
    ...
    try {
      ...
      const result = await v2Handler.handleMessage(update, redis, {
        sportsAggregator: aggregator,
      });
      ...
      if (result && result.method === "sendMessage") {
        ...
        if (result.text) {
          ...
          const hasMatches =
            result.text.includes("vs") || result.text.includes("No live");
          ...
          logger.info(
            `  ${result.text.substring(0, 150).replace(/\n/g, " ")}...`,
          );
        }
    ...
    try {
      ...
      const result = await v2Handler.handleCallbackQuery(cbUpdate, redis, {
        sportsAggregator: aggregator,
      });
    ...
    try {
      ...
      if (result && result.method) {
        ...
        if (result.text) {
          logger.info(
            `✓ Response text preview: ${result.text.substring(0, 100)}...`,
          );
        }
      } else {
        logger.info(
          "✓ Match details callback handled (service unavailable or match not found)",
        );
      }
    } catch (e) {
