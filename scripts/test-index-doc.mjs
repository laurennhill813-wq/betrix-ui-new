...
(async function(){
  try {
    ...
    const azure = new AzureAIService(process.env.AZURE_OPENAI_ENDPOINT, process.env.AZURE_OPENAI_KEY, process.env.AZURE_OPENAI_DEPLOYMENT, process.env.AZURE_OPENAI_API_VERSION, { embeddingsDeployment: process.env.AZURE_EMBEDDINGS_DEPLOYMENT, logger: console });
    const rag = createRag({ redis, azure, logger: console });
    const id = 'manual-test-' + Date.now();
    const text = 'Hello from manual RAG index test at ' + new Date().toISOString();
