...
const configuredBase = process.env.SPORTSGAMEODDS_BASE || 'https://api.sportsgameodds.com/v1';
...
const endpoints = [
  '/', '/v1', '/api', '/health', '/status', '/docs', '/openapi.json',
  '/events', '/events/upcoming', '/events/list',
  '/fixtures', '/fixtures/upcoming', '/fixtures/list',
  '/odds', '/odds/events', '/odds/list'
];
...
(async () => {
    ...
    for (const ep of endpoints) {
      ...
      for (const q of queryVariants) {
        for (const h of headerVariants) {
          ...
          try {
            const resp = await axios.get(url, { headers, params: q, timeout: 8000, validateStatus: null });
            const status = resp.status;
            const body = resp.data;
            results.push({ url, status, header: h, query: q, body: shortBody(body) });
            ok = true;
  ...
  for (const [k, r] of seen.entries()) {
    console.log(`${r.status || 'ERR'} ${r.url} -> ${r.body ? r.body.substring(0,120) : (r.error || '')}`);
  }

  // Save a detailed JSON file for inspection
  try {
    const fs = await import('fs');
    await fs.promises.writeFile(path.resolve(process.cwd(), 'scripts/probe-sgo-results.json'), JSON.stringify(results, null, 2), 'utf8');
