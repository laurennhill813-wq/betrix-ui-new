...
const mockAgg = {
  ...
  getFixtures: async () => [ { id: '537952', home: 'Newcastle', away: 'Chelsea', kickoff: new Date().toISOString(), competition: 'EPL' } ],
...
(async () => {
  ...
  const res = await handleAnalyzeMatch(token, 1111, 2222, redis, { sportsAggregator: mockAgg, azureAI: { isHealthy: () => false } });
