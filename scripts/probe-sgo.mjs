...
(async function () {
  try {
    ...
    const key =
      (CONFIG.SPORTSGAMEODDS && CONFIG.SPORTSGAMEODDS.KEY) ||
    ...
    for (const c of candidates) {
      ...
      for (const h of headerVariants) {
        try {
          ...
          const status =
            e && e.response && e.response.status ? e.response.status : "ERR";
    ...
    try {
      await redis.set(keyRedis, JSON.stringify(payload));
      await redis.expire(keyRedis, 3600);
      console.log("Wrote health to", keyRedis);
    } catch (e) {
