...
if (!REDIS_URL) {
  ...
  logTest(
    ...
    "PASS",
    `Configured: ${REDIS_URL.substring(0, 30)}...`,
  );
}
...
try {
  ...
  logTest(
    ...
    "PASS",
    `Protocol: ${url.protocol}, Host: ${url.hostname}:${url.port}`,
  );
} catch (e) {
...
redis.on("error", (err) => {
  if (err.message.includes("NOAUTH")) {
    ...
    logTest(
      ...
      "FAIL",
      "Cannot connect to host - check hostname/port",
    );
...
async function runOperationsTest() {
  if (!connectionSucceeded) {
    logTest(
      ...
      "FAIL",
      "Connection not established - skipping operations",
    );
    await redis.quit();
    return;
  }
  ...
  try {
    ...
    if (zrange.length === 3) {
      ...
      logTest(
        "ZRANGE command",
        "FAIL",
        `Expected 3 members, got ${zrange.length}`,
      );
    }
    ...
    if (smembers.length === 3) {
      logTest(
        "SMEMBERS command",
        "PASS",
        `Retrieved ${smembers.length} set members`,
      );
    } else {
      logTest(
        "SMEMBERS command",
        "FAIL",
        `Expected 3 members, got ${smembers.length}`,
      );
    }
    ...
    await redis.del(
      "__betrix_test__",
      "__betrix_hash_test__",
      "__betrix_list_test__",
      "__betrix_zset_test__",
      "__betrix_counter__",
      "__betrix_set_test__",
    );
  ...
  try {
    ...
    if (userData.tier === "FREE") {
      logTest(
        ...
        "PASS",
        `Retrieved user tier: ${userData.tier}`,
      );
    } else {
      logTest(
        "User profile HGETALL",
        "FAIL",
        "User data not retrieved correctly",
      );
    }
    ...
    await redis.setex(
      ...
      JSON.stringify({
        ...
        matches: [
          ...
          { id: 2, home: "ManCity", away: "Liverpool", odds: 2.1 },
        ],
        totalOdds: 3.885,
        stake: 100,
      }),
    ...
    if (favorites.length === 3) {
      logTest(
        ...
        "PASS",
        `Retrieved ${favorites.length} favorites`,
      );
    } else {
      logTest(
        "Favorites set retrieval",
        "FAIL",
        `Expected 3, got ${favorites.length}`,
      );
    }
    ...
    logTest(
      ...
      "PASS",
      "Usage metric recorded with expiry",
    );
  ...
  try {
    ...
    if (factoryRedis) {
      logTest(
        ...
        "PASS",
        `Returns ${factoryRedis.constructor.name} instance`,
      );
    } else {
      logTest(
        "getRedis() factory function",
        "FAIL",
        "Did not return an instance",
      );
    }
...
async function main() {
  ...
  if (failed > 0) {
    ...
    console.log(
      "✨ Your BETRIX bot is ready to connect with the Azure Redis instance.\n",
    );
    process.exit(0);
  }
}
