...
const CALLBACK =
  ...
  "https://betrix-ui.onrender.com/webhook/mpesa";
...
(async function main() {
  ...
  try {
    ...
    console.log(
      "Create response:",
      created.status,
      JSON.stringify(created.body || created, null, 2),
    );
    ...
    while (Date.now() < deadline) {
      ...
      const status =
        item && (item.status || item.state) ? item.status || item.state : null;
      if (JSON.stringify(item) !== JSON.stringify(last)) {
        console.log(
          ...
          status,
          "item=",
          JSON.stringify(item || q.body || {}, null, 2),
        );
        last = item || q.body;
      }
      if (
        status &&
        ["success", "failed", "timeout", "cancelled"].includes(
