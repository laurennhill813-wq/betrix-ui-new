...
async function tryFetch(insecure = false) {
  try {
    ...
    if (insecure) {
      ...
      console.warn(
        "Attempting insecure TLS fetch (rejectUnauthorized=false) - for debugging only",
      );
    }
...
(async function () {
  ...
  if (!ok) {
    console.warn(
      "Normal fetch failed. You can retry insecurely for debugging by setting DEBUG_SPORTSMONKS_INSECURE=true in env (not recommended in production).",
    );
    if (process.env.DEBUG_SPORTSMONKS_INSECURE === "true") {
      console.warn(
        "DEBUG_SPORTSMONKS_INSECURE is true — attempting insecure fetch (this will accept self-signed certs).",
      );
      await tryFetch(true);
    }
  }
