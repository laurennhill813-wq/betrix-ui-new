...
(async () => {
  try {
    ...
    const { default: SportGameOddsService } = await import('../src/services/sportsgameodds.js');

    console.log('CONFIG.SPORTSGAMEODDS:', JSON.stringify(CONFIG.SPORTSGAMEODDS, null, 2));
    ...
    console.log('Fetched items count:', Array.isArray(items) ? items.length : 0);
    ...
    for (const it of sample) {
      ...
      console.log('ITEM:', it.id, '| homeType:', homeType, (homeType === 'object' ? Object.keys(it.home || {}) : it.home), '| awayType:', awayType, (awayType === 'object' ? Object.keys(it.away || {}) : it.away), '| start:', it.start);
    }

    // If no items returned, perform a few diagnostic requests to discover available endpoints
    if (!Array.isArray(items) || items.length === 0) {
      const axios = (await import('axios')).default;
      const candidates = ['','/v1','/health','/status','/docs','/openapi.json','/events','/fixtures','/odds','/odds/events','/events/upcoming','/fixtures/upcoming'];
      console.log('No items; running diagnostics against candidate endpoints...');
      for (const c of candidates) {
        const url = svc.base.replace(/\/$/, '') + c;
        try {
          const resp = await axios.get(url, { headers: svc._headers(), timeout: 10000 });
          ...
          const status = e.response && e.response.status ? e.response.status : 'ERR';
