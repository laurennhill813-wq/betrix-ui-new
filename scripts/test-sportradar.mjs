...
(async function run() {
  try {
    ...
    // Build full URL
    let imageUrl = href.startsWith('http') ? href : `${BASE.replace(/\/+$/,'')}${href}`;
    ...
    await sendPhotoWithCaption({ chatId: CHAT, photoUrl: imageUrl, caption: 'Sportradar test image (flags manifest)' });
