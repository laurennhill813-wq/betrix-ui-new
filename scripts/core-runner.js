...
async function start() {
  ...
  try {
    ...
    try {
      ...
      logger.info("Redis connected (core-runner)");
    } catch (e) {
      logger.warn("Redis ping failed, using MockRedis");
      redis = new MockRedis();
    }
  } catch (e) {
    logger.warn("Redis init failed, using MockRedis");
    redis = new MockRedis();
  }
  ...
  setInterval(async () => {
    try {
      ...
      await redis.expire("worker:heartbeat", 30);
    } catch (e) {
      logger.warn("Heartbeat write failed", e && e.message ? e.message : e);
    }
  }, 10 * 1000);
  ...
  try {
    ...
    if (typeof sub.subscribe !== "function") {
      logger.warn(
        "Prefetch subscriber skipped: redis client has no subscribe()",
      );
    } else {
    ...
    logger.warn("Prefetch subscriber failed", e && e.message ? e.message : e);
  }
...
start().catch((e) => {
  console.error("core-runner failed", e);
  process.exit(1);
});
