...
const BASE = process.env.SPORTSGAMEODDS_API_BASE || process.env.SPORTSGAMEODDS_BASE || 'https://api.sportsgameodds.com/v2';

async function probe(path) {
  const url = `${BASE}${path}`;
  console.log('Requesting', url);
  const res = await fetch(url, { headers: { 'X-API-Key': KEY, Accept: 'application/json' }, timeout: 20000 });
