...
try {
  ...
  process.exit = function (code) {
    try {
      console.error(
        "DBG process.exit called with code=",
        code,
        "\ncallstack:\n",
        new Error().stack,
      );
...
function containsNodeTest(filepath) {
  try {
    ...
    if (
      txt.includes("from 'node:assert'") ||
      txt.includes('from "node:assert"')
    )
      return true;
    // Detect plain 'assert' imports (common in node-script style tests)
    if (
      /\bimport\s+assert\b/.test(txt) ||
      txt.includes("from 'assert'") ||
      txt.includes('from "assert"')
    )
      return true;
    ...
    if (
      txt.includes("require.main === module") ||
      txt.includes("import.meta.url")
    )
      return true;
    if (
      txt.includes("process.exit(") ||
      txt.includes("runtests(") ||
      txt.includes("runtests()") ||
      txt.includes("runtests().catch")
    )
      return true;
    ...
    if (
      (/\bassert\s*\(/.test(txt) || /\bassert\./.test(txt)) &&
      !/\bdescribe\s*\(|\btest\s*\(/.test(txt)
    )
      return true;
    return false;
  } catch (e) {
...
function isCommonJS(filepath) {
  try {
    const txt = readFileSync(filepath, "utf8");
    return (
      txt.includes("require(") ||
      txt.includes("module.exports") ||
      txt.includes("exports.")
    );
  } catch (e) {
    return false;
  }
...
async function main() {
  ...
  let nodeExit = 0;
  // Print environment diagnostics useful for CI debugging
  try {
    const nodeVer = process.version;
    let npmVer = "unknown";
    try {
      const npmOut = spawnSync("npm", ["--version"], { encoding: "utf8" });
      if (npmOut && npmOut.stdout) npmVer = String(npmOut.stdout).trim();
    } catch (e) {}
    console.log(
      "Environment: node",
      nodeVer,
  ...
  if (nodeTestFiles.length) {
    ...
    nodeExit =
      r.status !== null && r.status !== undefined ? r.status : r.error ? 1 : 0;
  } else {
  ...
  if (nodeScriptFiles.length) {
    ...
    for (const f of nodeScriptFiles) {
      ...
      console.log("Script result for", f, ":", {
        status: r.status,
        signal: r.signal,
        error: r.error ? String(r.error) : null,
      });
      ...
      if (r.status !== 0 || r.error) {
        ...
        r = spawnSync(process.execPath, [f], {
          stdio: "pipe",
          encoding: "utf8",
        });
        console.error(
  ...
  let jestExit = 0;
  // CI-mode: when true, runner should fail hard on missing tooling
  const ciMode =
    process.env.CI === "true" ||
    process.env.CI === "1" ||
    process.env.CI === "yes";
  ...
  if (jestFiles.length) {
    console.log(
      "Running Jest (single invocation) on",
      jestFiles.length,
      "files...",
    );
    const args = [
    ...
    // Prefer running the local jest binary directly to avoid relying on `npx`.
    const isWin = process.platform === "win32";
    const jestBin = isWin
      ? join(process.cwd(), "node_modules", ".bin", "jest.cmd")
      : join(process.cwd(), "node_modules", ".bin", "jest");
    const jestArgs = [
      "--passWithNoTests",
      "--runInBand",
      "--verbose",
    ...
    // Try running the local jest script with Node first (more portable),
    // then fall back to the platform .bin executable or npx.
    const jestScript = join(
      process.cwd(),
      "node_modules",
      "jest",
      "bin",
    ...
    // Capture logs into CI-friendly files for later upload
    const logDir = join(process.cwd(), "ci-logs");
    const ts = Date.now();
    const outLog = join(logDir, `jest-out-${ts}.log`);
    const errLog = join(logDir, `jest-err-${ts}.log`);
    let r = null;
    try {
      r = spawnSync(process.execPath, [jestScript, ...jestArgs], {
        stdio: "inherit",
        env: jestEnv,
      });
    } catch (e) {
    ...
    // If that fails because the script isn't present, try the .bin shim (jest.cmd/jest)
    if (
      (r && r.error && r.error.code === "ENOENT") ||
      (r &&
        r.status === 1 &&
        r.stderr &&
        String(r.stderr).includes("Cannot find module"))
    ) {
      try {
        r = spawnSync(jestBin, jestArgs, { stdio: "inherit", env: jestEnv });
      } catch (e) {
        r = e;
      }
    }
    console.log("Jest process result:", {
      status: r.status,
      signal: r.signal,
      error: r.error ? String(r.error) : null,
    });
    if ((r && r.status !== 0) || (r && r.error)) {
      // If direct exe fails (missing bin), fall back to npx if available.
      if (r && r.error && r.error.code === "ENOENT") {
        console.warn("Local jest binary not found.");
        if (!ciMode) {
          console.warn("Falling back to npx (non-CI mode).");
          try {
            r = spawnSync(
              "npx",
              [
              ...
              { stdio: "inherit", env: jestEnv },
            );
          } catch (e) {
            r = e;
          }
        } else {
          console.error("CI mode enabled: failing due to missing Jest.");
        }
      }

      if ((r && r.status !== 0) || (r && r.error)) {
        console.error(
          "Jest returned non-zero; capturing output for diagnosis...",
        );
        // Try to capture stdout/stderr for diagnostics
        try {
          const captured = spawnSync(jestBin, jestArgs, {
            stdio: "pipe",
            encoding: "utf8",
            env: jestEnv,
          });
          // ensure ci-logs exist
          try {
            require("fs").mkdirSync(logDir, { recursive: true });
          } catch (e) {}
          require("fs").writeFileSync(outLog, captured.stdout || "");
          require("fs").writeFileSync(errLog, captured.stderr || "");
          console.error("Captured logs written to", outLog, errLog);
          console.error(
            "--- Captured stdout (jest) ---\n",
            captured.stdout || "(none)",
          );
          console.error(
            "--- Captured stderr (jest) ---\n",
            captured.stderr || "(none)",
          );
          r = captured;
        } catch (e) {
          console.error(
            "Failed to capture jest output:",
            e && e.stack ? e.stack : e,
          );
        }
      }
    ...
    jestExit =
      r && r.status !== null && r.status !== undefined
        ...
        : r && r.error
          ? 1
          : 0;
    // If jest isn't installed and we're not in CI, allow runner to continue but warn
    const stderr = r && r.stderr ? String(r.stderr) : "";
    if (
      jestExit !== 0 &&
      (stderr.includes("Cannot find module") ||
        (r &&
          r.error &&
          (r.error.code === "ENOENT" || r.error.code === "MODULE_NOT_FOUND")))
    ) {
      if (ciMode) {
        console.error("Jest missing in CI mode — failing the test run.");
      } else {
        console.warn(
          "Jest appears not to be installed or failed to run; skipping Jest failures (non-CI).",
        );
        jestExit = 0;
      }
    }
