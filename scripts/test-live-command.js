...
async function testLiveCommand() {
  try {
    ...
    const result = await handler.handleMessage(mockContext, redis, {
    ...
    const sportsMonksMatches =
      await aggregator._getLiveFromSportsMonks("football");
    console.log(
      `✅ SportMonks returned ${sportsMonksMatches.length} live matches\n`,
    );
    ...
    if (sportsMonksMatches.length > 0) {
      ...
      console.log(
        "⚠️  No live matches from SportMonks (likely TLS cert issue on this network)",
      );
      console.log(
        "    But fallback to prefetch cache or demo mode is working ✓",
      );
    }
    ...
    if (cachedKey) {
      ...
      if (cached.length > 0) {
        ...
        cached.slice(0, 3).forEach((m, idx) => {
          console.log(
            `  ${idx + 1}. ${m.home_team || m.homeTeam} vs ${m.away_team || m.awayTeam}`,
          );
        });
      }
    } else {
