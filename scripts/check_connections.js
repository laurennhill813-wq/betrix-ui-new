...
async function checkRedis() {
  try {
    ...
    return {
      ok: pong === "PONG" || String(pong).toLowerCase().includes("pong"),
    };
  } catch (e) {
...
async function checkSportsAggregator() {
  try {
    ...
    const live = await agg.getAllLiveMatches().catch((e) => {
      throw e;
    });
    console.log(
      "[SPORTSAGGREGATOR] getAllLiveMatches -> count:",
      Array.isArray(live) ? live.length : typeof live,
    );
    if (Array.isArray(live) && live.length > 0) {
      console.log(
        "[SPORTSAGGREGATOR] Sample match:",
        JSON.stringify(live[0], null, 2).slice(0, 1000),
      );
    }
    // Try to fetch upcoming fixtures (light call)
    const fixtures = await agg.getFixtures().catch((e) => {
      console.warn("[SPORTSAGGREGATOR] getFixtures failed", e.message || e);
      return [];
    });
    console.log(
      "[SPORTSAGGREGATOR] getFixtures -> count:",
      Array.isArray(fixtures) ? fixtures.length : typeof fixtures,
    );
    return {
      ok: true,
      liveCount: Array.isArray(live) ? live.length : 0,
