...
async function run() {
  ...
  const res1 = await handleCallbackQuery(cbPayload.data, chatId, userId, adapter, {} , cbPayload).catch(e => ({ error: String(e) }));
  ...
  const msgUpdate = { message: { chat: { id: chatId }, text: 'Alice Example', from: { id: userId } } };
  ...
  const res2 = await handleMessage(msgUpdate, adapter, {}).catch(e => ({ error: String(e) }));
  console.log('Message handler result:', res2);

  console.log('\n--- Simulate: Press Talk to BETRIX AI (mod_ai_chat) ---');
  const cb2 = { data: 'mod_ai_chat' };
  const res3 = await handleCallbackQuery(cb2.data, chatId, userId, adapter, {} , cb2).catch(e => ({ error: String(e) }));
  console.log('AI Callback result:', res3);
  console.log('onboarding key after AI callback:', await adapter.get(onboardKey));

  console.log('\n--- Simulate: Ask a question to BETRIX AI via chat message ---');
  const questionUpdate = { message: { chat: { id: chatId }, text: 'What are the best value bets today?', from: { id: userId } } };
  const res4 = await handleMessage(questionUpdate, adapter, { aiService: {} }).catch(e => ({ error: String(e) }));
  console.log('AI message handler result:', res4);
}

run().then(() => console.log('\nSimulation complete')).catch(err => { console.error('Sim failed', err); process.exit(1); });
