...
async function run() {
  ...
  if (!url) { console.error('Please set REDIS_URL env var'); process.exit(2); }
  const r = new Redis(url);
  const patterns = ['telegram*', 'queue*', '*telegram*', 'betrix*'];
  try {
    for (const p of patterns) {
      console.log('PATTERN', p);
      const keys = await r.keys(p);
      if (!keys || keys.length === 0) { console.log('  none'); continue; }
  ...
  } finally { try{ await r.quit(); }catch(e){try{r.disconnect()}catch{}} }
}

run().catch(e=>{ console.error(e && e.message||e); process.exit(1); });
