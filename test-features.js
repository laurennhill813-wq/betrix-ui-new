...
test(
  "Main menu has Odds & Analysis button",
  JSON.stringify(mainMenu.reply_markup).includes("Odds"),
);
test(
...
test(
  "Main menu has Subscribe button",
  JSON.stringify(mainMenu.reply_markup).includes("VVIP"),
);
test(
  "Main menu has Profile button",
  JSON.stringify(mainMenu.reply_markup).includes("Profile"),
);
test(
  "Main menu has Help button",
  JSON.stringify(mainMenu.reply_markup).includes("Help"),
);
...
test(
  "SIGNUP has features",
  TIERS["SIGNUP"].features && TIERS["SIGNUP"].features.length > 0,
);
test(
  "VVIP has features",
  TIERS["VVIP"].features && TIERS["VVIP"].features.length > 0,
);
...
test(
  "SAFARICOM_TILL provider configured",
...
test(
  "MPESA accepts KES",
  PAYMENT_PROVIDERS["MPESA"].currencies.includes("KES"),
);
test(
  "SAFARICOM_TILL accepts KES",
  PAYMENT_PROVIDERS["SAFARICOM_TILL"].currencies.includes("KES"),
);
...
test(
  "Returning user welcome function exists",
  typeof welcomeReturningUser === "function",
);
const newUserWelcome = welcomeNewUser();
test(
  "New user welcome has content",
  newUserWelcome && newUserWelcome.length > 0,
);
const returningUserWelcome = welcomeReturningUser({
  name: "John",
  tier: "VVIP",
});
test(
  "Returning user welcome has content",
  returningUserWelcome && returningUserWelcome.length > 0,
);
test(
  "Returning user welcome includes name",
  returningUserWelcome.includes("John"),
);
test(
  "Returning user welcome includes tier",
  returningUserWelcome.includes("VVIP"),
);
...
test(
  "Menu has all sport categories",
  mainMenu.reply_markup.inline_keyboard.length > 3,
);
test(
  "Subscription menu has Free tier",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .some((b) => b.text.includes("Free")),
);
test(
  "Subscription menu has Pro tier",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .some((b) => b.text.includes("Pro")),
);
test(
  "Subscription menu has VVIP tier",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .some((b) => b.text.includes("VVIP")),
);
test(
  "Subscription menu has Plus tier",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .some((b) => b.text.includes("Plus")),
);
test(
  "Subscription menu has payment methods",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .some((b) => b.text.includes("Till") || b.text.includes("Pesa")),
);

// TEST 8: FLOW INTEGRITY
console.log("\n🔄 FLOW INTEGRITY");
test(
  "Main menu callback data valid",
  mainMenu.reply_markup.inline_keyboard
    .flat()
    .every((b) => b.callback_data && typeof b.callback_data === "string"),
);
test(
  "Subscription menu callback data valid",
  subscriptionMenu.reply_markup.inline_keyboard
    .flat()
    .every((b) => b.callback_data && typeof b.callback_data === "string"),
);
test(
  "Sports menu callback data valid",
  sportsMenu.reply_markup.inline_keyboard
    .flat()
    .every((b) => b.callback_data && typeof b.callback_data === "string"),
);
