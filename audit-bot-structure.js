...
const auditConfig = {
  apis: {
    "API-Football (Primary)": {
      ...
      endpoint:
        "https://api-football-v3.p.rapidapi.com/fixtures?live=all&status=LIVE",
  ...
  services: [
    {
      ...
      methods: ["getLiveMatches", "getOdds", "getStandings", "getLeagues"],
    },
    {
      name: "OddsAnalyzer",
      file: "src/services/odds-analyzer.js",
      methods: ["analyzeMatch", "formatForTelegram", "getOddsForMatch"],
    },
    {
      name: "MultiSportAnalyzer",
      file: "src/services/multi-sport-analyzer.js",
      methods: ["analyzeMatch", "formatForTelegram"],
    },
    {
  ...
  handlers: [
    ...
    {
      name: "Telegram Handler (v2)",
      file: "src/handlers/telegram-handler-v2.js",
      methods: ["handleMessage", "handleCallbackQuery"],
    },
    {
      name: "Multi-Sport Handler",
      file: "src/handlers/multi-sport-handler.js",
      methods: ["handleMultiSportAnalyze"],
    },
    {
      name: "Payment Handler",
      file: "src/handlers/payment-handler.js",
      methods: ["handlePaymentFlow"],
    },
    {
      name: "Callbacks",
      file: "src/handlers/callbacks-v3.js",
      callbacks: ["menu_main", "odds_*", "analyze_*", "signup_*"],
    },
  ],
...
async function auditEnvironment() {
  ...
  const requiredEnv = ["TELEGRAM_TOKEN", "REDIS_URL", "GEMINI_API_KEY"];

  const apiEnv = ["API_FOOTBALL_KEY", "RAPIDAPI_KEY", "SPORTSDATA_API_KEY"];
  ...
  for (const env of apiEnv) {
    if (process.env[env]) {
      const masked =
        ...
        "..." +
        process.env[env].substring(process.env[env].length - 4);
      pass(`${env}: ${masked}`);
      apiCount++;
    } else {
  ...
  if (process.env.API_FOOTBALL_KEY) {
    try {
      ...
      const response = await fetch(
        "https://api-football-v3.p.rapidapi.com/fixtures?live=all&status=LIVE&limit=3",
        {
          headers: {
      ...
      if (response.ok) {
        ...
        if (data.response && data.response.length > 0) {
          pass(
            `API-Football: ✅ Working (${data.response.length} live matches found)`,
          );
          results.apiFootball = {
            status: "active",
    ...
    try {
      ...
      const response = await fetch(
        "https://api.football-data.org/v4/competitions",
        {
          headers: {
            "X-Auth-Token":
              process.env.FOOTBALL_DATA_API || process.env.FOOTBALLDATA_API_KEY,
          },
          timeout: 5000,
        },
      );
      ...
      if (response.ok) {
        const data = await response.json();
        pass(
          `Football-Data.org: ✅ Working (${data.competitions?.length || 0} competitions available)`,
        );
  ...
  for (const service of auditConfig.services) {
    try {
      const filePath = new URL(
        `file:///${process.cwd()}/${service.file}`.replace(/\\/g, "/"),
      );
      ...
      if (methodsFound === service.methods.length) {
        ...
        warn(
          `${service.name}: Only ${methodsFound}/${service.methods.length} methods found`,
        );
      }
    } catch (err) {
...
async function auditHandlers() {
  ...
  for (const handler of auditConfig.handlers) {
    try {
      const filePath = new URL(
        `file:///${process.cwd()}/${handler.file}`.replace(/\\/g, "/"),
      );
      const content = await fs.readFile(filePath, "utf8");
      ...
      if (itemsFound.length > 0) {
        pass(
          `${handler.name}: ${itemsFound.length}/${items.length} items implemented`,
        );
      } else {
...
async function testLiveSports() {
  ...
  try {
    ...
    try {
      const SportsAggregator =
        await import("./src/services/sports-aggregator.js");
...
function generateSummary() {
  ...
  if (errors.length === 0) {
    log(
      "\n" +
        chalk.bold.green(
          "🎉 ALL CHECKS PASSED - BOT STRUCTURE IS TOP NOTCH!\n",
        ),
    );
    return true;
  } else {
    log(
      "\n" +
        chalk.bold.red(
          `⚠️  ${errors.length} issues found that need attention\n`,
        ),
    );
    return false;
  }
