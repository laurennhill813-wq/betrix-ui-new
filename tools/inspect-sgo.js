...
async function inspect(leagues) {
  ...
  for (const league of leagues) {
    try {
      ...
      const events = await sportsgameodds.fetchEvents({ league, redis, forceFetch: true });
      console.log(JSON.stringify(events, null, 2).slice(0, 4000));

      const firstEventId = (Array.isArray(events) && events[0] && (events[0].id || events[0].eventId)) ? (events[0].id || events[0].eventId) : null;
      if (firstEventId) {
        console.log(`\n--- Fetching odds for first event (${firstEventId}) ---`);
        const odds = await sportsgameodds.fetchOdds({ league, eventId: firstEventId, redis, forceFetch: true });
...
inspect(leagues).then(() => process.exit(0)).catch((e) => { console.error(e); process.exit(2); });
