...
async function main() {
  ...
  try {
    if (what === 'events') {
      const all = await sgo.fetchAllEvents({ league, limit, redis, forceFetch: true });
      console.log(`Fetched events: ${all.length}`);
      console.log(JSON.stringify(all.slice(0,5), null, 2));
    } else if (what === 'teams') {
      const all = await sgo.fetchAllTeams({ leagueID: league, limit, redis, forceFetch: true });
      console.log(`Fetched teams: ${all.length}`);
      console.log(JSON.stringify(all.slice(0,5), null, 2));
    } else if (what === 'odds') {
      const all = await sgo.fetchAllOdds({ league, limit, redis, forceFetch: true });
