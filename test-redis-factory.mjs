...
async function test(){
  ...
  await r.rpush('myq','a');
  await r.rpush('myq','b');
  console.log('rpoplpush from myq to myq2 ->', await r.rpoplpush('myq','myq2'));
  console.log('brpoplpush from myq to myq2 ->', await r.brpoplpush('myq','myq2',1));
  await r.publish('channel','hi');
  console.log('publish done');
  if (r.quit) { await r.quit(); console.log('quit ok'); }
}
