...
test("SportMonks API is initialized with correct API key", () => {
  const api = new SportMonksAPI(
    "zUdIC2auUmiG6bUS5v7Mc53IxJwqiQ2gBMyFqsTI9KnnBJJQMM5eExZsPh42",
  );
  assert(
    api.apiKey ===
      "zUdIC2auUmiG6bUS5v7Mc53IxJwqiQ2gBMyFqsTI9KnnBJJQMM5eExZsPh42",
...
test("SportsData API is initialized with correct API key", () => {
  ...
  assert(
    api.apiKey === "abdb2e2047734f23b576e1984d67e2d7",
    "API key not set correctly",
  );
  assert(api.enabled === true, "API should be enabled with key");
});
...
test("SIGNUP tier has correct pricing (KES 150)", () => {
  assert(
    TIERS.SIGNUP.price === 150,
    `SIGNUP price should be 150, got ${TIERS.SIGNUP.price}`,
  );
  assert(
    TIERS.SIGNUP.billingPeriod === "one-time",
    `SIGNUP billing should be one-time`,
  );
  assert(
    Array.isArray(TIERS.SIGNUP.features),
    "SIGNUP features should be array",
  );
...
test("Payment Router exports createPaymentOrder function", () => {
  assert(
    typeof createPaymentOrder === "function",
    "createPaymentOrder should be exported",
  );
  assert(
    typeof getPaymentInstructions === "function",
    "getPaymentInstructions should be exported",
  );
});
...
test("Telegram Handler V2 imports payment functions", () => {
  ...
  assert(
    content.includes("createPaymentOrder"),
    "createPaymentOrder not imported",
  );
  assert(
    content.includes("getPaymentInstructions"),
    "getPaymentInstructions not imported",
  );
...
test("Telegram Handler has safe user data retrieval function", () => {
  ...
  assert(
    content.includes("safeGetUserData"),
    "safeGetUserData helper not found",
  );
  assert(
    content.includes("WRONGTYPE"),
    "Redis WRONGTYPE error handling not found",
  );
});
...
test("Live Games handler uses new API integrations", () => {
  ...
  assert(
    (content.includes("sportMonks") && content.includes("getLiveMatches")) ||
      content.includes("sportMonks.*getLiveMatches"),
    "SportMonks integration missing",
  );
  // Check for SportsData integration
  assert(
    (content.includes("sportsData") && content.includes("getLiveGames")) ||
      content.includes("sportsData.*getLiveGames"),
    "SportsData integration missing",
  );
});
...
test("Odds handler uses betting odds APIs", () => {
  ...
  assert(
    content.includes("getBettingOdds"),
    "getBettingOdds integration missing",
  );
...
test("Standings handler fetches from multiple sources", () => {
  ...
  assert(
    (content.includes("sportMonks") && content.includes("getStandings")) ||
      (content.includes("sportsData") && content.includes("getStandings")),
...
test("Worker initializes API instances", () => {
  const content = fs.readFileSync("./src/worker-final.js", "utf8");
  assert(
    content.includes("const sportMonksAPI = new SportMonksAPI"),
    "SportMonks instance not created",
  );
  assert(
    content.includes("const sportsDataAPI = new SportsDataAPI"),
    "SportsData instance not created",
  );
});
...
test("Live games endpoint handles fallback gracefully", () => {
  ...
  assert(
    content.includes("Arsenal") ||
      content.includes("Chelsea") ||
      content.includes("Manchester"),
