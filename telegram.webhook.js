...
async function initRedisClient() {
  try {
    ...
    console.warn(
      "[telegram.webhook] Failed to load redis adapter, falling back to native client:",
      e && e.message,
    );
  }
...
const logger = {
  info: (m, meta) => wlog("INFO", m, meta),
  warn: (m, meta) => wlog("WARN", m, meta),
  error: (m, meta) => wlog("ERROR", m, meta),
};
...
function parseRedisOptsFromEnv() {
  if (process.env.REDIS_URL) {
    try {
      ...
      if (url.password)
        opts.password = decodeURIComponent(url.password.replace(/^:/, ""));
      return opts;
    } catch (err) {
...
logger.info("WEBHOOK_REDIS_OPTS", {
  host: redisOpts.socket.host,
  port: redisOpts.socket.port,
  tls: !!redisOpts.socket.tls,
  ...
  app.post("/telegram/:token", async (req, res) => {
    try {
      ...
      if (!expected) {
        ...
        logger.warn("WEBHOOK_TOKEN_MISMATCH", {
      ...
      try {
        ...
        logger.error("WEBHOOK_ENQUEUE_FAILED", err && err.message, {
          jobId: job.jobId,
        });
      }
    } catch (err) {
