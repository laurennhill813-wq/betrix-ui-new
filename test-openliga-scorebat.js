...
(async () => {
  ...
  try {
    ...
    const scorebat = new ScoreBatService(null, cache, {
      retries: 3,
      cacheTtlSeconds: 30,
    });
    ...
    try {
      ...
      console.log(
        `  ✅ ScoreBat free feed: ${Array.isArray(feed) ? feed.length : "N/A"} items`,
      );
    } catch (e) {
    ...
    try {
      ...
      console.log(
        `  ✅ ScoreBat featured: ${Array.isArray(featured) ? featured.length : "N/A"} items`,
      );
    } catch (e) {
    ...
    console.log(
      `    Input: { Team1.TeamName: "${mockOpenLigaMatch.Team1.TeamName}", Team2.TeamName: "${mockOpenLigaMatch.Team2.TeamName}" }`,
    );

    const normalized = agg._normalizeOpenLigaMatch(mockOpenLigaMatch);
    console.log(`  ✅ Normalized output:`);
    console.log(
      `    - home: "${normalized.home}" (type: ${typeof normalized.home})`,
    );
    console.log(
      `    - away: "${normalized.away}" (type: ${typeof normalized.away})`,
    );
