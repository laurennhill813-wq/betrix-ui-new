...
class intelligentMenus {
  constructor() {}
  buildContextualMainMenu() {
    return null;
  }
  buildMatchDetailMenu() {
    return null;
  }
}
...
const premiumUI = {
  buildMatchCard: (m) => `${teamNameOf(m.home)} vs ${teamNameOf(m.away)}`,
  buildSubscriptionComparison: () => "Subscription comparison",
};
  ...
  try {
    ...
    const keyboard = limited.map((m, i) => {
      ...
      return [
        {
          text: `🔎 Analyze ${i + 1}`,
          callback_data: validateCallbackData(`analyze_match_${leagueId}_${i}`),
        },
        {
      ...
      [
        {
          text: "🤖 Analyze Match",
          callback_data: validateCallbackData(
            `analyze_match_${leagueId || "live"}_${idx}`,
          ),
        },
      ],
      [
  ...
  try {
    ...
    if (!match) {
      return {
        method: "sendMessage",
        chat_id: chatId,
        text: "⚠️ Match not found or not live.",
        parse_mode: "Markdown",
      };
    }
    ...
    if (recentFormHome && recentFormHome.length > 0) {
      ...
      analysisText +=
        recentFormHome
          ...
          .map(
            (m) =>
              `${m.starting_at || m.date || m.date_time || m.utcDate || "N/A"}: ${m.result || (m.score ? JSON.stringify(m.score) : "N/A")}`,
          )
          .join("\n") + "\n\n";
    }
    ...
    if (standings && standings.length > 0) {
      ...
      if (homeStanding || awayStanding) {
        ...
        if (homeStanding)
          analysisText += `${homeLabel}: #${homeStanding.position || "N/A"} (${homeStanding.points || 0} pts)\n`;
        if (awayStanding)
          analysisText += `${awayLabel}: #${awayStanding.position || "N/A"} (${awayStanding.points || 0} pts)\n`;
        analysisText += "\n";
      }
    }
