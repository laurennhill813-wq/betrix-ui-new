...
async function initRedis() {
  ...
  try {
    ...
    client.on("error", (err) => {
      safeLog("redis error", err && err.message ? err.message : err);
      redisReady = false;
    });
    ...
    safeLog(
      "Failed to connect to Redis for dedupe:",
    ...
    try {
      ...
      const authHint =
        req.headers && req.headers.authorization
          ? String(req.headers.authorization).slice(0, 16)
          : "";
      ...
      const setResult = await redisClient.set(key, Date.now().toString(), {
        NX: true,
        EX: ttl,
      });
      ...
      safeLog("dedupe error", err && err.message ? err.message : err);
      return next();
    }
  };
};
