...
try {
  ...
  if (analyses && analyses.length > 0) {
    ...
    analyses.forEach((analysis, idx) => {
      ...
      if (analysis.prediction) {
        console.log(
          `  Prediction: ${analysis.prediction.outcome.replace(/_/g, " ")}`,
        );
  ...
  const matchAnalysis = await oddsAnalyzer.analyzeMatch(
    "Manchester United",
    "Liverpool",
  );
  ...
  if (matchAnalysis.status !== "error") {
    ...
    console.log(
      `Odds: 1=${matchAnalysis.odds.home} X=${matchAnalysis.odds.draw} 2=${matchAnalysis.odds.away}`,
    );
    console.log(
    ...
    console.log(
      `  Has Value: ${matchAnalysis.value.hasValue ? "✅ YES" : "❌ NO"}`,
    );
  } else {
  ...
  formatted.split("\n").forEach((line) => {
    ...
    console.log(
      "│ " +
        displayLine +
        " ".repeat(Math.max(0, 66 - displayLine.length)) +
        "│",
    );
  });
  console.log("└" + "─".repeat(68) + "┘");
  ...
  console.log("\n📊 TEST 5: Odds Comparison\n");
  const comparison = await oddsAnalyzer.compareOdds(
    "Manchester United",
    "Liverpool",
  );
